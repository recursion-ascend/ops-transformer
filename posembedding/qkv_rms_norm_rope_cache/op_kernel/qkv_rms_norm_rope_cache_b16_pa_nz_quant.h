...
class KernelQkvRmsNormRopeCacheQuantB16PANZ : public KernelQkvRmsNormRopeCacheCutBSN<QKV_DTYPE, K_DTYPE, V_DTYPE> {
public:
    ...
    __aicore__ inline void Init(GM_ADDR qkv, GM_ADDR q_gamma, GM_ADDR k_gamma, GM_ADDR cos, GM_ADDR sin, GM_ADDR index,
                                GM_ADDR q_out, GM_ADDR k_cache, GM_ADDR v_cache, GM_ADDR k_scale, GM_ADDR v_scale,
                                GM_ADDR k_offset, GM_ADDR v_offset, GM_ADDR q_out_proto, GM_ADDR k_cache_proto,
                                GM_ADDR v_cache_proto)
    {
        this->InitSharedData();
        ...
        this->isV = (GetBlockIdx() < (this->tilingData_->blockDimK + this->tilingData_->blockDimV) &&
                     GetBlockIdx() >= this->tilingData_->blockDimK) ?
                        1 :
                        0;

        if (GetBlockIdx() ==
            ((this->tilingData_->blockDimK + this->tilingData_->blockDimV + this->tilingData_->blockDimQ - 1))) {
            currentBlockFactorQ =
                this->tilingData_->batchSize * this->tilingData_->seqLength * this->tilingData_->numHeadQ -
                (this->tilingData_->blockDimQ - 1) * this->tilingData_->blockFactorQ;
        }

        if (GetBlockIdx() == (this->tilingData_->blockDimK - 1)) {
            currentBlockFactorK =
                this->tilingData_->batchSize * this->tilingData_->seqLength * this->tilingData_->numHeadK -
        ...
        if (GetBlockIdx() == (this->tilingData_->blockDimK + this->tilingData_->blockDimV - 1)) {
            currentBlockFactorV =
                this->tilingData_->batchSize * this->tilingData_->seqLength * this->tilingData_->numHeadV -
        ...
        // init global memory
        this->qkvGm.SetGlobalBuffer((__gm__ QKV_DTYPE *)qkv);        // [B*S, N*D]
        ...
        if (this->isV) {
            this->pipe_->InitBuffer(
                this->inQueueX, NUM_TWO,
                this->ubFactorV * this->rmsNormLength * sizeof(QKV_DTYPE)); // 2 * ubFactorV * 128 * 2 / 1024 = 32K
            this->pipe_->InitBuffer(this->locBuf0, this->ubFactorV * this->rmsNormLength *
                                                       sizeof(float)); // ubFactorV * 128 * 4 / 1024 = 32K
            this->pipe_->InitBuffer(this->locBuf1, this->ubFactorV * this->rmsNormLength *
                                                       sizeof(float)); // ubFactorV * 128 * 4 / 1024 = 32K
            this->pipe_->InitBuffer(this->locBuf2, this->ubFactorV * this->rmsNormLength *
                                                       sizeof(float)); // ubFactorV * 128 * 4 / 1024 = 32K
            this->pipe_->InitBuffer(this->locBuf3, this->ubFactorV * this->rmsNormLength *
                                                       sizeof(float)); // ubFactorV * 128 * 4 / 1024 = 32K
            this->pipe_->InitBuffer(
                this->outQueue, NUM_TWO,
                this->ubFactorV * this->rmsNormLength * sizeof(QKV_DTYPE)); // 2 * ubFactorV * 128 * 2 / 1024 = 32K
        } else {
            this->pipe_->InitBuffer(this->inQueueX, NUM_TWO,
                                    this->tilingData_->ubFactor * this->rmsNormLength *
                                        sizeof(QKV_DTYPE)); // 2 * ubFactor * 128 * 2 / 1024 = 21K
            this->pipe_->InitBuffer(this->inQueueY, NUM_TWO,
                                    this->tilingData_->ubFactor * this->rmsNormLength *
                                        sizeof(QKV_DTYPE)); // 2 * ubFactor * 128 * 2 / 1024 = 21K
            this->pipe_->InitBuffer(this->locBuf0, NUM_TWO * this->tilingData_->ubFactor * this->rmsNormLength *
                                                       sizeof(float)); // ubFactor * 128 * 4 * 2 / 1024 = 42K
            this->pipe_->InitBuffer(this->locBuf1, this->tilingData_->ubFactor * this->rmsNormLength *
                                                       sizeof(float)); // ubFactor * 128 * 4 / 1024 = 21K
            this->pipe_->InitBuffer(this->locBuf2, NUM_TWO * this->tilingData_->ubFactor * this->rmsNormLength *
                                                       sizeof(float)); // ubFactor * 128 * 4 * 2 / 1024 = 42K
            this->pipe_->InitBuffer(this->locBuf3, this->tilingData_->ubFactor * this->rmsNormLength *
                                                       sizeof(float)); // ubFactor * 128 * 4 / 1024 = 21K
            this->pipe_->InitBuffer(
                this->gammaBuf, this->rmsNormLength *
                                    sizeof(float)); // q_gamma or k_gamma                       // 128 * 4 / 1024 = 0.5K
            this->pipe_->InitBuffer(this->outQueue, NUM_TWO,
                                    this->tilingData_->ubFactor * this->rmsNormLength *
                                        sizeof(QKV_DTYPE)); // 2 * ubFactor * 128 * 2 / 1024 = 21K
        }
    }
    ...
    __aicore__ inline void ProcessK()
    {
        ...
        LocalTensor<QKV_DTYPE> gammaLocalK = gammaLocalFp32K.template ReinterpretCast<QKV_DTYPE>()[this->rmsNormLength];
        ...
        for (int64_t loopIdx = 0; loopIdx < this->ubLoopK; ++loopIdx) {
            int64_t startIdx = GetBlockIdx() * this->tilingData_->blockFactorK +
                               loopIdx * this->ubFactorK; // startIdx表示：Cut K BSN时，当次循环所计算的K的128的起始偏移
            // Calc K
            // SplitK
            ...
            RmsNorm<QKV_DTYPE>(xLocalFp32, xSquareLocal, xSumLocal, dstLocalFp32, gammaLocalFp32K, this->ubFactorK,
                               this->rmsNormLength);
            // CopyIn Cos Sin
            ...
            if (this->tilingData_->isKQuant) {
                ...
                CopyInQuantFac(scaleLocal, offsetLocal, this->kScaleGm, this->kOffsetGm, this->ubFactorK, startIdx,
                               this->numHeadK, isAsymmetricKQuant);
                LocalTensor<int8_t> quantOutLocal = outLocal.template ReinterpretCast<int8_t>();
                Quant<QKV_DTYPE>(quantOutLocal, dstLocalFp32, scaleLocal, offsetLocal, isAsymmetricKQuant,
                                 this->rmsNormLength, this->ubFactorK);
                ...
                ScatterUpdateNZWithQuant<int8_t, D1_SIZE4_PA_NZ_QUANT, D0_SIZE32_PA_NZ_QUANT>(
                    (GlobalTensor<int8_t> &)this->kCacheGm, quantOutLocal, startIdx, this->ubFactorK,
                    this->rmsNormLength, this->numHeadK);
            } else {
        ...
        if (this->ubTailK > 0) {
            int64_t startIdx =
                GetBlockIdx() * this->tilingData_->blockFactorK +
                this->ubLoopK * this->ubFactorK; // startIdx表示：Cut K BSN时，当次循环所计算的K的128的起始偏移
            // Calc K
            // Split K
            LocalTensor<QKV_DTYPE> xLocal = this->inQueueX.template AllocTensor<QKV_DTYPE>();
            ...
            LocalTensor<float> xSquareLocal = this->locBuf1.template Get<float>();
            LocalTensor<float> xSumLocal = this->locBuf2.template Get<float>();
            RmsNorm<QKV_DTYPE>(xLocalFp32, xSquareLocal, xSumLocal, dstLocalFp32, gammaLocalFp32K, this->ubTailK,
                               this->rmsNormLength);
            // CopyIn Cos Sin
            ...
            if (this->tilingData_->isKQuant) {
                LocalTensor<float> scaleLocal = this->locBuf1.template Get<float>();
                LocalTensor<float> offsetLocal = this->locBuf2.template Get<float>();
                CopyInQuantFac(scaleLocal, offsetLocal, this->kScaleGm, this->kOffsetGm, this->ubTailK, startIdx,
                               this->numHeadK, isAsymmetricKQuant);
                LocalTensor<int8_t> quantOutLocal = outLocal.template ReinterpretCast<int8_t>();
                Quant<QKV_DTYPE>(quantOutLocal, dstLocalFp32, scaleLocal, offsetLocal, isAsymmetricKQuant,
                                 this->rmsNormLength, this->ubTailK);
                this->outQueue.EnQue(outLocal);
                outLocal = this->outQueue.template DeQue<QKV_DTYPE>();
                quantOutLocal = outLocal.template ReinterpretCast<int8_t>();
                // Scatter Update kCache
                ScatterUpdateNZWithQuant<int8_t, D1_SIZE4_PA_NZ_QUANT, D0_SIZE32_PA_NZ_QUANT>(
                    (GlobalTensor<int8_t> &)this->kCacheGm, quantOutLocal, startIdx, this->ubTailK, this->rmsNormLength,
                    this->numHeadK);
            } else {
                // Cast and Scatter Update kCache
                LocalTensor<K_DTYPE> outputLocal = outLocal.template ReinterpretCast<K_DTYPE>();
    ...
    __aicore__ inline void ProcessV()
    {
        for (int64_t loopIdx = 0; loopIdx < this->ubLoopV; ++loopIdx) {
            int64_t startIdx = (GetBlockIdx() - this->tilingData_->blockDimK) * this->tilingData_->blockFactorV +
                               loopIdx * this->ubFactorV;
            // Calc: V
            // SplitV
            LocalTensor<QKV_DTYPE> xLocal = this->inQueueX.template AllocTensor<QKV_DTYPE>();
            ...
            if (this->tilingData_->isVQuant) {
                LocalTensor<float> scaleLocal = this->locBuf1.template Get<float>();
                LocalTensor<float> offsetLocal = this->locBuf2.template Get<float>();
                CopyInQuantFac(scaleLocal, offsetLocal, this->vScaleGm, this->vOffsetGm, this->ubFactorV, startIdx,
                               this->numHeadV, isAsymmetricVQuant);
                LocalTensor<int8_t> quantOutLocal = outLocal.template ReinterpretCast<int8_t>();
                Quant<QKV_DTYPE>(quantOutLocal, xLocalFp32, scaleLocal, offsetLocal, isAsymmetricVQuant,
                                 this->rmsNormLength, this->ubFactorV);
                this->outQueue.EnQue(outLocal);
                outLocal = this->outQueue.template DeQue<QKV_DTYPE>();
                quantOutLocal = outLocal.template ReinterpretCast<int8_t>();
                // Scatter Update vCache
                ScatterUpdateNZWithQuant<int8_t, D1_SIZE4_PA_NZ_QUANT, D0_SIZE32_PA_NZ_QUANT>(
                    (GlobalTensor<int8_t> &)this->vCacheGm, quantOutLocal, startIdx, this->ubFactorV,
                    this->rmsNormLength, this->numHeadV);
            } else {
                // Cast
        ...
        if (this->ubTailV > 0) {
            ...
            if (this->tilingData_->isVQuant) {
                LocalTensor<float> scaleLocal = this->locBuf1.template Get<float>();
                LocalTensor<float> offsetLocal = this->locBuf2.template Get<float>();
                CopyInQuantFac(scaleLocal, offsetLocal, this->vScaleGm, this->vOffsetGm, this->ubTailV, startIdx,
                               this->numHeadV, isAsymmetricVQuant);
                LocalTensor<int8_t> quantOutLocal = outLocal.template ReinterpretCast<int8_t>();
                Quant<QKV_DTYPE>(quantOutLocal, xLocalFp32, scaleLocal, offsetLocal, isAsymmetricVQuant,
                                 this->rmsNormLength, this->ubTailV);
                this->outQueue.EnQue(outLocal);
                outLocal = this->outQueue.template DeQue<QKV_DTYPE>();
                quantOutLocal = outLocal.template ReinterpretCast<int8_t>();
                // Scatter Update vCache
                ScatterUpdateNZWithQuant<int8_t, D1_SIZE4_PA_NZ_QUANT, D0_SIZE32_PA_NZ_QUANT>(
                    (GlobalTensor<int8_t> &)this->vCacheGm, quantOutLocal, startIdx, this->ubTailV, this->rmsNormLength,
                    this->numHeadV);
            } else {
                // Cast
                LocalTensor<V_DTYPE> outputLocal = outLocal.template ReinterpretCast<V_DTYPE>();
    ...
    __aicore__ inline void ProcessQ()
    {
        ...
        LocalTensor<QKV_DTYPE> gammaLocalQ = gammaLocalFp32Q.template ReinterpretCast<QKV_DTYPE>()[this->rmsNormLength];
        copyParamsContinguous.blockLen = this->rmsNormLength * sizeof(QKV_DTYPE);
        ...
        for (int64_t loopIdx = 0; loopIdx < this->ubLoopQ; ++loopIdx) {
            int64_t startIdx = (GetBlockIdx() - this->tilingData_->blockDimK - this->tilingData_->blockDimV) *
                                   this->tilingData_->blockFactorQ +
                               loopIdx * this->ubFactorQ;
            // Calc: Q
            // SplitQ
            LocalTensor<QKV_DTYPE> xLocal = this->inQueueX.template AllocTensor<QKV_DTYPE>();
            ...
            LocalTensor<float> xSquareLocal = this->locBuf1.template Get<float>();
            LocalTensor<float> xSumLocal = this->locBuf2.template Get<float>();
            RmsNorm<QKV_DTYPE>(xLocalFp32, xSquareLocal, xSumLocal, dstLocalFp32, gammaLocalFp32Q, this->ubFactorQ,
                               this->rmsNormLength);
            // CopyIn Cos Sin
        ...
        if (this->ubTailQ > 0) {
            ...
            LocalTensor<float> xSquareLocal = this->locBuf1.template Get<float>();
            LocalTensor<float> xSumLocal = this->locBuf2.template Get<float>();
            RmsNorm<QKV_DTYPE>(xLocalFp32, xSquareLocal, xSumLocal, dstLocalFp32, gammaLocalFp32Q, this->ubTailQ,
                               this->rmsNormLength);
            // CopyIn Cos Sin
    ...
    template <typename T>
    __aicore__ inline void SplitQkv(const LocalTensor<T> &xLocal, int64_t rows, int64_t startIdx, int64_t numHeadBefore,
                                    int64_t numHeads)
    {
        DataCopyPadExtParams<T> padParams{false, 0, 0, 0};
        DataCopyExtParams copyParams{/* blockCount */ 1,
        ...
        for (int64_t i = 0; i < rows; ++i) {
            int64_t offset = batchIdx * this->tilingData_->seqLength * this->numHead * this->rmsNormLength +
                             seqIdx * this->numHead * this->rmsNormLength + numHeadBefore * this->rmsNormLength +
                             nIdx * this->rmsNormLength;
    ...
    __aicore__ inline void CopyInQuantFac(const LocalTensor<float> &scaleLocal, const LocalTensor<float> &offsetLocal,
                                          const GlobalTensor<float> &scaleGm, const GlobalTensor<float> &offsetGm,
    ...
    template <typename T>
    __aicore__ inline void CopyInRopeFac(const LocalTensor<float> &cosSinLocalFp32, int64_t rows, int64_t ropeLen,
                                         int64_t numHead, int64_t startIdx)
    {
        DataCopyPadExtParams<T> padParams{false, 0, 0, 0};
        DataCopyExtParams copyParamsContinguous;
        ...
        while (totalMask > 0) {
            ...
            Div(buf0[offset], srcTensor[offset], scaleLocal[offset],
                ...
                {1, 1, 1, static_cast<uint8_t>(headSize / NUM_EIGHT), static_cast<uint8_t>(headSize / NUM_EIGHT),
                 static_cast<uint8_t>(headSize / NUM_EIGHT)});
            if (ifAsymmetricQuant) {
                PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline void ScatterUpdateNZWithQuant(const GlobalTensor<T1> &dst, const LocalTensor<T1> &outLocal,
                                                    int64_t startIdx, int64_t rows, int64_t headSize, int64_t numHead)
    {
        DataCopyExtParams copyParamsNz{
            /*burstNum*/ static_cast<uint16_t>(D1),                 // 4
        ...
        for (int64_t i = 0; i < rows; i++) {
            ...
            if (pageOffset >= 0) {
                ...
                SToMTE3Sync();
                DataCopyPad(dst[gmOffsetNz], outLocal[ubOffset],
                            copyParamsNz); // outLocal.shape=[ubFactor, headSize] -> [BSNk, 128]
    ...
    template <typename T>
    __aicore__ inline void RmsNorm(const LocalTensor<float> &xLocalFp32, const LocalTensor<float> &xSquareLocal,
                                   const LocalTensor<float> &xSumLocal, const LocalTensor<float> &dstLocalFp32,
    ...
    template <typename T>
    __aicore__ inline void Rope(const LocalTensor<float> &srcLocalFp32, const LocalTensor<float> &cosSinLocalFp32,
                                int64_t rows, int64_t ropeLen)
    {
        ...
        Copy<float, true>(
            realLocalFp32, srcLocalFp32, (ropeLen >> 1), rows,
            {1, 1, static_cast<uint8_t>((ropeLen >> 1) / NUM_EIGHT), static_cast<uint8_t>(ropeLen / NUM_EIGHT)});
        PipeBarrier<PIPE_V>();
        Copy<float, true>(
            imagLocalFp32, srcLocalFp32[(ropeLen >> 1)], (ropeLen >> 1), rows,
            {1, 1, static_cast<uint8_t>((ropeLen >> 1) / NUM_EIGHT), static_cast<uint8_t>(ropeLen / NUM_EIGHT)});
        PipeBarrier<PIPE_V>();
        // step #3: y0 = (realLocalFp32, imagLocalFp32) * cosLocalFp32
        Mul(y0, realLocalFp32, cosLocalFp32, /*mask*/ (ropeLen >> 1), /*repeat*/ rows,
            {1, 1, 1, static_cast<uint8_t>(ropeLen / NUM_EIGHT), static_cast<uint8_t>((ropeLen >> 1) / NUM_EIGHT),
             static_cast<uint8_t>(ropeLen / NUM_EIGHT)});
        ...
        PipeBarrier<PIPE_V>();
        Copy<float, true>(
            cosSinLocalFp32, srcLocalFp32, (ropeLen >> 1), rows * NUM_TWO,
            {1, 1, static_cast<uint8_t>((ropeLen >> 1) / NUM_EIGHT), static_cast<uint8_t>((ropeLen >> 1) / NUM_EIGHT)});
        PipeBarrier<PIPE_V>();
    }

    template <typename T>
    __aicore__ inline void CopyoutProto(const LocalTensor<float> &srcLocalFp32, const GlobalTensor<T> &dstNd,
                                        int64_t startIdx, int64_t rows, int64_t headSize)
    {
        ...
        DataCopyPadExtParams<T> padParams{false, 0, 0, 0};
        DataCopyExtParams copyParamsNd{/* blockCount */ static_cast<uint16_t>(rows),
