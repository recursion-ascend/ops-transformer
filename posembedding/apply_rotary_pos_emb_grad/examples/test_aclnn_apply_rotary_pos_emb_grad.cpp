...
int main()
{
    ...
    ret = CreateAclTensor(gradQEmbedHostData, gradQEmbedShape, &gradQEmbedDeviceAddr, aclDataType::ACL_FLOAT,
                          &gradQueryEmbed);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建gradKeyEmbed aclTensor
    ret = CreateAclTensor(gradKEmbedHostData, gradKEmbedShape, &gradKEmbedDeviceAddr, aclDataType::ACL_FLOAT,
                          &gradKeyEmbed);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建cos aclTensor
    ...
    ret = CreateAclTensor(gradQueryOutHostData, gradQueryOutShape, &gradQueryOutDeviceAddr, aclDataType::ACL_FLOAT,
                          &gradQueryOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建gradKeyOut aclTensor
    ret = CreateAclTensor(gradKeyOutHostData, gradKeyOutShape, &gradKeyOutDeviceAddr, aclDataType::ACL_FLOAT,
                          &gradKeyOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建gradCosOut aclTensor
    ret = CreateAclTensor(std::vector<float>(128, 0.0f), gradCosOutShape, &gradCosOutDeviceAddr, aclDataType::ACL_FLOAT,
                          &gradCosOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建gradSinOut aclTensor
    ret = CreateAclTensor(std::vector<float>(128, 0.0f), gradSinOutShape, &gradSinOutDeviceAddr, aclDataType::ACL_FLOAT,
                          &gradSinOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = aclnnApplyRotaryPosEmbGradGetWorkspaceSize(gradQueryEmbed, gradKeyEmbed, cos, sin, query, key,
                                                     const_cast<char *>(rotaryModeOptional), layout, gradQueryOut,
                                                     gradKeyOut, gradCosOut, gradSinOut, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnApplyRotaryPosEmbGradGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
