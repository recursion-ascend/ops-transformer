...
__aicore__ inline void RotaryPositionEmbeddingABAAndBA<T, IsBBoardcast>::InitAllBuffer()
{
    ...
    this->dAlign_ =
        ops::CeilAlign<int64_t>(tilingData_->sliceLength / dSplitCoef_, BLOCK_TYPE_SIZE / sizeof(T)) * dSplitCoef_;
    if (tilingData_->rotaryMode == static_cast<int64_t>(InplacePartialRotaryPosEmbeddingMode::DEEPSEEK_INTERLEAVE)) {
        ...
        this->ubCopyInStride = (this->dAlign_ * sizeof(T) -
                                ops::CeilAlign<int64_t>(tilingData_->sliceLength * sizeof(T), BLOCK_TYPE_SIZE)) /
                               BLOCK_TYPE_SIZE;
    }
                                                                                 ...
                                                                                 int64_t nTotalSize)
{
    ...
    copyPadExtparams.paddingValue = 0;
    DataCopyPad(target,
                source[bStart * nTotalSize * tilingData_->S * D_ + nStart * tilingData_->S * D_ + sStart * D_ +
                                                                                 ...
                                                                                 int64_t bLength, int64_t nLength)
{
    ...
    if (tilingData_->rotaryMode == static_cast<int64_t>(InplacePartialRotaryPosEmbeddingMode::HALF)) {
        BatchHalfAlignVF<T, IsBBoardcast>((__ubuf__ T *)inUb.GetPhyAddr(), (__ubuf__ T *)cos.GetPhyAddr(),
                                          ...
                                          bLength, nLength, tilingData_->sliceLength, dAlign_, ubFactorS_, ubFactorN_);
        ...
        BatchInterleaveModeVF<T, IsBBoardcast>((__ubuf__ T *)inUb.GetPhyAddr(), (__ubuf__ T *)cos.GetPhyAddr(),
                                               ...
                                               sLength, bLength, nLength, tilingData_->sliceLength, dAlign_, ubFactorS_,
                                               ubFactorN_);
    } else if (tilingData_->rotaryMode == static_cast<int64_t>(InplacePartialRotaryPosEmbeddingMode::QUARTER)) {
        BatchQuarterAlignVF<T, IsBBoardcast>((__ubuf__ T *)inUb.GetPhyAddr(), (__ubuf__ T *)cos.GetPhyAddr(),
                                             (__ubuf__ T *)sin.GetPhyAddr(), (__ubuf__ T *)outUb.GetPhyAddr(), sLength,
                                             bLength, nLength, tilingData_->sliceLength, dAlign_, ubFactorS_,
                                             ubFactorN_);
    } else {
