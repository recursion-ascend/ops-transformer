...
// check nullptr
ACLNN_API bool CombineArnCheckNotNull(const aclTensor *expandX, const aclTensor *expertIds,
                                      const aclTensor *assistInfoForCombine, const aclTensor *epSendCounts,
                                      [[maybe_unused]] const aclTensor *tpSendCounts, const aclTensor *expertScales,
                                      const char *groupEp, [[maybe_unused]] const char *groupTp, aclTensor *x);

// 入参校验
ACLNN_API aclnnStatus CombineArnCheckParams(const aclTensor *expandX, const aclTensor *expertIds,
                                            const aclTensor *expandIdx, const aclTensor *epSendCounts,
                                            const aclTensor *tpSendCounts, const aclTensor *expertScales,
                                            const char *groupEp, const char *groupTp, aclTensor *x, bool is910B);
...
ACLNN_API aclnnStatus aclnnMoeDistributeCombineAddRmsNormBase(void *workspace, uint64_t workspaceSize,
