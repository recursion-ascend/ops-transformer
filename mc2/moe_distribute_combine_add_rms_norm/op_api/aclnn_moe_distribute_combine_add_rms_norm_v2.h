 ...
 * @param [in] constExpertAlpha1Optional:
 计算可选输入，Tensor，数据类型float16，bfloat16，int32，必须为1维，数据格式支持ND，支持非连续的Tensor。数据类型必须与expandX保持一致。
    当constExpertNum不为0时必须传入有效输入。
 * @param [in] constExpertAlpha2Optional:
 计算可选输入，Tensor，数据类型float16，bfloat16，int32，必须为1维，数据格式支持ND，支持非连续的Tensor。数据类型必须与expandX保持一致。
    当constExpertNum不为0时必须传入有效输入。
 * @param [in] constExpertVOptional:
 计算可选输入，Tensor，数据类型float16，bfloat16，int32，必须为1维，数据格式支持ND，支持非连续的Tensor。数据类型必须与expandX保持一致。
 ...
 * @param [in] zeroExpertNum:
 计算输入，int64_t。取值范围[0,MAX_INT32]，合法的零专家ID值是[moeExpertNum,moeExpertNum+zeroExpertNum)。
 * @param [in] copyExpertNum:
 计算输入，int64_t。取值范围[0,MAX_INT32]，合法的零专家ID值是[moeExpertNum+zeroExpertNum,moeExpertNum+zeroExpertNum+copyExpertNum)。
 * @param [in] constExpertNum:
 计算输入，int64_t。取值范围[0,MAX_INT32]，合法的零专家ID值是[moeExpertNum+zeroExpertNum+copyExpertNum,moeExpertNum+zeroExpertNum+copyExpertNum+constExpertNum)。
 ...
 *
 */
ACLNN_API aclnnStatus aclnnMoeDistributeCombineAddRmsNormV2GetWorkspaceSize(
 ...
 */
ACLNN_API aclnnStatus aclnnMoeDistributeCombineAddRmsNormV2(void *workspace, uint64_t workspaceSize,
