...
# -----------------------------------------------------------------------------------------------------------
# run.sh — apace QuantMatmulReduceScatter ubmem ST
#
...
run_single() {
    ...
    echo ""
    echo "=========================================="
    echo "apace QuantMatmulReduceScatter ubmem ST"
    ...
    if [ "$VERIFY_ONLY" -eq 0 ]; then
        if [ "$SKIP_BUILD" -eq 0 ]; then
            ...
            cmake --build "$BUILD_DIR" --target apace_qmm_rs_ubmem_st --parallel 4 || { echo "ERROR: build failed"; return 1; }
        else
            [ -x "$BUILD_DIR/quant_matmul_reduce_scatter/ubmem/apace_qmm_rs_ubmem_st" ] || { echo "ERROR: --skip-build but binary missing"; return 1; }
            echo "[2/4] Skip build (--skip-build)"
        fi

        # ---- 3. 多 rank 运行 ----
        echo "[3/4] NPU run ($RANK_NUM ranks)..."
        local EXE_PATH="$BUILD_DIR/quant_matmul_reduce_scatter/ubmem/apace_qmm_rs_ubmem_st"
