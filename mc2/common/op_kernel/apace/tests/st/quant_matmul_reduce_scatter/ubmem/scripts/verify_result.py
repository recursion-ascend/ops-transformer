...
"""
Verify QuantMatmulReduceScatter operator output.

Operator semantics: MXFP8 MatMul + AlltoAll + ReduceAdd.

Each rank j produces output y_j = sum_i(C_i)[j*M/R : (j+1)*M/R, :] -> [M/R, N] (BF16).

This is DIFFERENT from AllGatherQuantMatmul:
  - AllGather: each rank produces [rankNum*M, N] (full gathered output)
  - ReduceScatter: each rank produces [M/rankNum, N] (scattered slice of reduced output)
...
def verify_single_rank(m_total, n, rank_id, rank_num, base_dir="./output"):
    """Check NPU output for a single rank.

    Each rank's output is [M/rank_num, N] (the scattered slice of the reduced result).

    Returns: (pass_bool, diff_max, diff_mean, message)
    """
    m_per_rank = m_total // rank_num
    ...
    if output.size != expected_size:
        print(
            f"[WARN] Rank {rank_id}: output size mismatch ({output.size} vs {expected_size})"
        )
        return False, None, None, f"size mismatch: {output.size} != {expected_size}"

    npu_tensor = torch.from_numpy(output).view(torch.bfloat16).reshape(m_per_rank, n)
    print(f"[Rank {rank_id}] Output shape: ({m_per_rank}, {n})")
    print(
        f"[Rank {rank_id}] Output range: [{npu_tensor.float().min():.4f}, {npu_tensor.float().max():.4f}]"
    )
    ...
    golden_tensor = torch.from_numpy(golden).view(torch.bfloat16).reshape(m_per_rank, n)
    diff = (npu_tensor.float() - golden_tensor.float()).abs()
    max_diff = diff.max().item()
    mean_diff = diff.mean().item()

    # Raw uint16 comparison for 1-ULP tolerance.
    # Large-K matmuls can produce elements that differ by 1 ULP in bfloat16
    ...
    print(
        f"[Rank {rank_id}] raw uint16 diff: max={raw_max}, diff_elems={raw_diff_count}, one_ulp_elems={raw_one_ulp_count}"
    )
    ...
    if max_diff == 0 and mean_diff == 0:
        ...
        print(
            f"[Rank {rank_id}] [PASS] Within 1 ULP ({raw_diff_count}/{expected_size} elems, {raw_one_ulp_count} at 1 ULP)"
        )
        return True, max_diff, mean_diff, None
    else:
        close_count = (diff < 0.2).sum().item()
        print(
            f"[Rank {rank_id}] [FAIL] Raw diff max={raw_max} > 1 ULP ({close_count}/{expected_size} within 0.2)"
        )
...
def verify_result(m, n, rank_num, base_dir="./output"):
    ...
    m is the total M dimension. Each rank's output is [M/rank_num, N].

    Returns: (all_pass: bool)
    """
    m_per_rank = m // rank_num

    print(f"\n{'=' * 60}")
    print(f"QuantMatmulReduceScatter — Output Check for {rank_num} ranks")
    print(f"Input: M={m} (total), N={n}")
    print(f"Per-rank output shape: ({m_per_rank}, {n})")
    print(f"{'=' * 60}")

    all_pass = True
    failed_ranks = []
    for rank_id in range(rank_num):
        ok, max_d, mean_d, msg = verify_single_rank(m, n, rank_id, rank_num, base_dir)
        if not ok:
            all_pass = False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Verify QuantMatmulReduceScatter output"
    )
    parser.add_argument(
        "m", type=int, help="matrix M dimension (total, must be divisible by rank_num)"
    )
    parser.add_argument("n", type=int, help="matrix N dimension")
    parser.add_argument("rank_num", type=int, help="number of ranks")
    parser.add_argument(
        "base_dir", nargs="?", default="./output", help="output directory"
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if m % rank_num != 0:
        print(f"Error: m={m} is not divisible by rank_num={rank_num}")
        sys.exit(1)
