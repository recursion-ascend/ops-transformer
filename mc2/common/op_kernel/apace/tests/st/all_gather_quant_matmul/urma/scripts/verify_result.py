...
def verify_single_rank(m_total, n_per_rank, rank_id, base_dir="./output"):
    ...
    npu_tensor = (
        torch.from_numpy(output).view(torch.bfloat16).reshape(m_total, n_per_rank)
    )
    ...
    golden_tensor = (
        torch.from_numpy(golden).view(torch.bfloat16).reshape(m_total, n_per_rank)
    )
...
def verify_result(m, n, rank_num, base_dir="./output"):
    ...
    for rank_id in range(rank_num):
        ok, max_d, mean_d, msg = verify_single_rank(
            m_total, n_per_rank, rank_id, base_dir
        )
        if not ok:
            all_pass = False
