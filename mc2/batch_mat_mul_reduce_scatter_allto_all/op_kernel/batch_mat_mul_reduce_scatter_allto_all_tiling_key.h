...
ASCENDC_TPL_ARGS_DECL(
    ...
    ASCENDC_TPL_UINT_DECL( // 按枚举值给出，0表示输出在H维度按tp分片，1表示输出在C维度按tp分片。
        BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_Y_SHARD, ASCENDC_TPL_2_BW, ASCENDC_TPL_UI_LIST, 0, 1),
...
// 模板参数组合
ASCENDC_TPL_SEL(
    ASCENDC_TPL_ARGS_SEL(ASCENDC_TPL_UINT_SEL(BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_Y_SHARD, ASCENDC_TPL_UI_LIST, 1),
                         ASCENDC_TPL_BOOL_SEL(BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_WEIGHT_TRANSPOSE, 0, 1),
                         ASCENDC_TPL_BOOL_SEL(BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_IS_BIAS, 0, 1),
                         ASCENDC_TPL_BOOL_SEL(BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_LITE, 0, 1)),
    ASCENDC_TPL_ARGS_SEL(ASCENDC_TPL_UINT_SEL(BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_Y_SHARD, ASCENDC_TPL_UI_LIST, 0),
                         ASCENDC_TPL_BOOL_SEL(BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_WEIGHT_TRANSPOSE, 0, 1),
                         ASCENDC_TPL_BOOL_SEL(BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_IS_BIAS, 0, 1),
                         ASCENDC_TPL_BOOL_SEL(BATCH_MM_REDUCE_SCATTER_ALLTO_ALL_LITE, 0)), );
