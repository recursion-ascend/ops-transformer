...
"""Standalone cascade worker for MatmulAlltoAll.

Launched as a subprocess (python3 cascade_worker.py ...) so it does not depend
on pickle of dynamically-loaded spec modules. Mirrors TTK's E2E worker pattern.
...
def main():
    ...
    try:
        ...
        np.savez(rank_file, **{f"cascade_did{rank}": out_cpu.numpy()})
