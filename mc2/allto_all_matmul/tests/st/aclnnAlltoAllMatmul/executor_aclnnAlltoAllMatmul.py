...
class AllToAllMatmul(BaseApi):
                ...
                for src_rank in range(self.world_size):
                    if src_rank != self.rank:
                        dist.recv(
                            ...
                            src=src_rank,  # 源rank
                            tag=src_rank * 1000
                            + self.rank,  # 匹配发送方的标签（源rank*1000+当前rank）
                        )
            else:
                # 发给其他rank
                dist.send(
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        if self.name == "cpu":
            ...
            x_splits_from_ranks = [
                torch.empty((m, k), dtype=x.dtype, device=x.device)
                for _ in range(world_size)
            ]
            self.manual_all_to_all(
                x_splits_from_ranks, [x[m * i : m * (i + 1)] for i in range(world_size)]
            )
...
class AclnnAllToAllMatmul(AclnnBaseApi):
    ...
    def init_by_input_data(self, input_data: InputDataset):
        ...
        input_data.kwargs["group"] = self.group._get_backend(
            torch.device("npu")
