...
"""Standalone cascade worker for AllGatherMatmul.

Args: rank world_size port input_path result_path error_path
      is_trans_b m_dim k_dim n_dim is_gather_output
"""
...
def main():
    ...
    k_dim = int(k_dim)
    n_dim = int(n_dim)
    is_gather_output = is_gather_output == "True"
    try:
        ...
        gathered = torch.empty(
            world_size * m_dim, k_dim, dtype=x1.dtype, device=f"npu:{rank}"
        )
        ...
        if any(d in w_dtype_str for d in ("float8", "hifloat8", "hif8", "fp8", "int8")):
            mm_out = torch.matmul(gathered.float(), x2.float())
        else:
            mm_out = torch.matmul(gathered, x2)
        ...
        if is_gather_output:
            ...
            np.savez(a2a_file, **{f"cascade_a2a_did{rank}": gathered_cpu.numpy()})
