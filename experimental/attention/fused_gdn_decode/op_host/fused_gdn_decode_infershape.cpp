...
static ge::graphStatus InferShapeFusedGdnDecode(InferShapeContext *context)
{
    ...
    if (mixedShape == nullptr || stateShape == nullptr || outShape == nullptr || stateOutShape == nullptr) {
        OP_LOGE("FusedGdnDecode", "infershape null shape: mixed=%p state=%p out=%p stateOut=%p", mixedShape, stateShape,
                outShape, stateOutShape);
        return ge::GRAPH_FAILED;
    }
...
IMPL_OP_INFERSHAPE(FusedGdnDecode).InferShape(InferShapeFusedGdnDecode).InferDataType(InferDataTypeFusedGdnDecode);
