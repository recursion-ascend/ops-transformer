                                ...
                                aclOpExecutor *executor)
{
    ...
    auto ret = INFER_SHAPE(FusedGdnDecode, OP_INPUT(mixedQkv, a, b, aLog, dtBias, stateRef, ssmStateIndices),
                           OP_OUTPUT(out, stateOut), OP_ATTR(scale, softplusThreshold));
    OP_CHECK_INFERSHAPE(ret != ACLNN_SUCCESS, return nullptr, "FusedGdnDecode InferShape failed.");

    ret = ADD_TO_LAUNCHER_LIST_AICORE(FusedGdnDecode, OP_INPUT(mixedQkv, a, b, aLog, dtBias, stateRef, ssmStateIndices),
                                      OP_OUTPUT(out, stateRef), OP_ATTR(scale, softplusThreshold));
