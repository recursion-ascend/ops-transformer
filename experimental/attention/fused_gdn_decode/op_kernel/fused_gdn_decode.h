...
constexpr uint32_t INT32_NUM_PER_BLOCK = 8;
...
template <typename InType, typename StateType, bool STATE_FP32>
class KernelFusedGdnDecode {
public:
    __aicore__ inline KernelFusedGdnDecode() {}

    __aicore__ inline void Init(GM_ADDR mixedQkv, GM_ADDR a, GM_ADDR b, GM_ADDR aLog, GM_ADDR dtBias, GM_ADDR state,
                                GM_ADDR stateIndices, GM_ADDR out, GM_ADDR stateOut,
                                const FusedGdnDecodeTilingData *tilingData, TPipe *pipe)
    {
        ...
        dtBiasGm_.SetGlobalBuffer(reinterpret_cast<__gm__ InType *>(dtBias));
    ...
    __aicore__ inline void Process()
    {
        ...
        indexStartBatch_ = taskStart / td_->hv;
        const uint32_t indexEndBatch = (taskStart + taskCount - 1) / td_->hv;
        LoadStateIndices(indexStartBatch_, indexEndBatch - indexStartBatch_ + 1);
        ...
        while (taskOffset < taskCount) {
            ...
            const uint32_t segmentSize = MinU32(FP32_NUM_PER_BLOCK, MinU32(taskCount - taskOffset, remainingInBatch));
            ProcessBatchSegment(firstTask, segmentSize);
            taskOffset += segmentSize;
        }
        indexQueue_.FreeTensor(stateIndicesUb_);
    }
...
private:
    ...
    __aicore__ inline void InitLocalBuffers()
    {
        ...
        pipe_->InitBuffer(indexQueue_, BUFFER_NUM, td_->indexBufferElems * sizeof(int32_t));
        const uint32_t stateSlotBytes = alignK_ * alignBV_ * sizeof(StateType) + alignBV_ * sizeof(InType);
    ...
    __aicore__ inline void LoadStateIndices(uint32_t startBatch, uint32_t count)
    {
        auto indexLocal = indexQueue_.AllocTensor<int32_t>();
        const uint32_t alignedCount = Ceil(count, INT32_NUM_PER_BLOCK) * INT32_NUM_PER_BLOCK;
        DataCopyExtParams indexParams{1, static_cast<uint32_t>(count * sizeof(int32_t)), 0, 0, 0};
        DataCopyPadExtParams<int32_t> indexPad{true, 0, static_cast<uint8_t>(alignedCount - count), 0};
        DataCopyPad(indexLocal, stateIndicesGm_[startBatch], indexParams, indexPad);
        indexQueue_.EnQue(indexLocal);
        stateIndicesUb_ = indexQueue_.DeQue<int32_t>();
    }
    ...
    __aicore__ inline void LoadGatingInputs(uint32_t batch, uint32_t firstHv, uint32_t groupSize)
    {
        ...
        DataCopyPadExtParams<InType> abPad{true, 0, static_cast<uint8_t>(BF16_NUM_PER_BLOCK - groupSize), 0};
        ...
        DataCopyExtParams aLogParams{1, static_cast<uint32_t>(groupSize * sizeof(float)), 0, 0, 0};
        DataCopyPadExtParams<float> floatPad{true, 0, static_cast<uint8_t>(FP32_NUM_PER_BLOCK - groupSize), 0.0f};
        DataCopyPad(paramsLocal, aLogGm_[firstHv], aLogParams, floatPad);
        qQueue_.EnQue(paramsLocal);
        paramsLocal = qQueue_.DeQue<float>();
        DataCopy(scalarUb_[GATING_A_LOG_OFFSET], paramsLocal, FP32_NUM_PER_BLOCK);
        qQueue_.FreeTensor(paramsLocal);
        PipeBarrier<PIPE_V>();

        auto dtBiasLocal = qQueue_.AllocTensor<InType>();
        DataCopyExtParams dtBiasParams{1, static_cast<uint32_t>(groupSize * sizeof(InType)), 0, 0, 0};
        DataCopyPadExtParams<InType> dtBiasPad{true, 0, static_cast<uint8_t>(BF16_NUM_PER_BLOCK - groupSize), 0};
        DataCopyPad(dtBiasLocal, dtBiasGm_[firstHv], dtBiasParams, dtBiasPad);
        qQueue_.EnQue(dtBiasLocal);
        dtBiasLocal = qQueue_.DeQue<InType>();
        Cast(scalarUb_[GATING_DT_BIAS_OFFSET], dtBiasLocal, RoundMode::CAST_NONE, BF16_NUM_PER_BLOCK);
        qQueue_.FreeTensor(dtBiasLocal);
        PipeBarrier<PIPE_V>();
    }

    __aicore__ inline void PrefetchState(int64_t stateIdx, uint32_t batch, uint32_t hv, uint32_t vStart, uint32_t curBV)
    {
        ...
        DataCopyExtParams stateParams{static_cast<uint16_t>(curBV), static_cast<uint32_t>(td_->k * sizeof(StateType)),
                                      0, 0, 0};
        ...
        const uint64_t vOffset =
            mixedBase + static_cast<uint64_t>(2 * td_->h * td_->k) + static_cast<uint64_t>(hv) * td_->v + vStart;
                                          ...
                                          const LocalTensor<float> &scalarBlock, uint32_t count)
    {
        ...
        if (offset < count) {
            Mul(dst[offset], src[offset], scalarBlock, count - offset, 1, {1, 1, 0, repeatStride, repeatStride, 0});
        }
    }
                                     ...
                                     LocalTensor<float> &dst, uint32_t rows)
    {
        ...
        for (uint32_t i = 0; i < alignK_; i += REPEAT_LEN) {
            ...
            Mul(dst[i], matrix[i], vec[i], mask, repeatTime, {1, 1, 1, repeatStride, repeatStride, 0});
        }
    }
                                         ...
                                         const LocalTensor<float> &rowScalars, uint32_t rows)
    {
        ...
        for (uint32_t i = 0; i < alignK_; i += REPEAT_LEN) {
            const uint64_t mask = MinU32(REPEAT_LEN, alignK_ - i);
            MulAddDst(dst[i], vec[i], rowScalars, mask, static_cast<uint8_t>(rows), {1, 1, 0, dstRepeatStride, 0, 1});
        }
    }
                                 ...
                                 LocalTensor<StateType> stateLocal)
    {
        ...
        const uint64_t outOffset =
            static_cast<uint64_t>(batch) * td_->outBatchStride + static_cast<uint64_t>(hv) * td_->v + vStart;
        DataCopyExtParams outParams{1, static_cast<uint32_t>(curBV * sizeof(InType)), 0, 0, 0};
    ...
    __aicore__ inline void ProcessBatchSegment(uint32_t firstTask, uint32_t segmentSize)
    {
        ...
        const int32_t stateIdx = stateIndicesUb_.GetValue(batch - indexStartBatch_);
        if (stateIdx <= 0) {
        ...
        while (segmentOffset < segmentSize) {
            ...
            const uint32_t groupSize = MinU32(segmentSize - segmentOffset, remainingInH);
            LoadQK(batch, h);
            PrepareQK();
            for (uint32_t groupOffset = 0; groupOffset < groupSize; ++groupOffset) {
                ProcessValueHead(batch, hv + groupOffset, stateIdx, segmentOffset + groupOffset);
            }
    ...
    uint32_t indexStartBatch_;
    TQue<QuePosition::VECIN, 1> qQueue_;
    TQue<QuePosition::VECIN, 1> indexQueue_;
    ...
    LocalTensor<int32_t> stateIndicesUb_;
};
