...
def build_attention_mask(
    ...
    global_window_size = (
        GLOBAL_WINDOW_SIZE if global_window_size is None else int(global_window_size)
    )
    if global_window_size < 0:
        raise ValueError(
            f"GLOBAL_WINDOW_SIZE must be non-negative, got {global_window_size}"
        )
...
if __name__ == "__main__":
    ...
    logger.info(
        "SPARSE_MODE=%d, PRE_TOKENS=%d, NEXT_TOKENS=%d, GLOBAL_WINDOW_SIZE=%d",
        SPARSE_MODE,
        PRE_TOKENS,
        NEXT_TOKENS,
        GLOBAL_WINDOW_SIZE,
