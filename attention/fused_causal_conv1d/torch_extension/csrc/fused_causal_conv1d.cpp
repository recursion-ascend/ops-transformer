...
{
    at::Tensor y = at::empty(x.sizes(), x.options().dtype(x.dtype()));
    ...
    ACLNN_CMD(aclnnFusedCausalConv1dV2, x, weight, conv_states, query_start_loc, cache_indices, initial_state_mode,
              bias, num_accepted_tokens, num_computed_tokens, block_idx_first_scheduled_token,
              block_idx_last_scheduled_token, initial_state_idx, activation_value, pad_slot_id_value, run_mode_value,
              max_query_len_value, residual_connection_value, block_size_value, conv_mode_value, max_draft_tokens_value,
              y);
    return y;
}
...
{
    m.def("fused_causal_conv1d", &FusedCausalConv1d, "fused_causal_conv1d");
}
} // namespace op_api
