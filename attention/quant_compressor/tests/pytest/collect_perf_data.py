...
def extract_quant_compressor_row(prof_folder):
    ...
    target = df[df["Op Name"] == "QuantCompressor"]
    if target.empty:
        return None
...
def collect_and_save_incremental(prof_folder, test_result_path):
    ...
    row_data = extract_quant_compressor_row(prof_folder)
    if row_data is None:
        return None
...
def collect_all(
    ...
    for i in range(df_b.shape[0]):
        ...
        row_data = extract_quant_compressor_row(prof)
        ...
        if row_data is not None:
            ...
            print(f"  [{prof_idx + 1}] {case_name}: 未找到QuantCompressor数据")
    ...
    if is_compare:
        perf_threshold = 10
        perf_fail_list = []
        df_b["quant_compressor_perf_diff"] = ""
        df_b["perf_result"] = ""
        ...
        for i in perf_rows.keys():
            try:
                ...
                df_b.at[i, "quant_compressor_perf_diff"] = sas_diff
