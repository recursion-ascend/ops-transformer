...
# -----------------------------------------------------------------------------------------------------------
"""Input customization for standalone MixedQuantSparseFlashMlaMetadata cases."""
...
def load_sidecar_values(kwargs):
    metadata_input = load_metadata_protocol().load_metadata_inputs(
        "mixed_quant_sparse_flash_mla", kwargs.get("testcase_name")
    )
...
def copy_values(target, values, name, attributes):
    ...
    if values is None:
        if name not in _TOPK_NAMES:
            raise ValueError(
                f"MixedQuantSparseFlashMlaMetadata requires {name}_values for an active tensor"
            )
    ...
    if tuple(source.shape) != tuple(target.shape):
        raise ValueError(
            f"MixedQuantSparseFlashMlaMetadata {name} shape mismatch: "
...
def generate_mixed_quant_sparse_flash_mla_metadata_inputs(
    num_heads_q,
    num_heads_kv,
    head_dim,
    quant_mode,
    ...
    **kwargs,
    ...
    for name, tensor in zip(_VECTOR_NAMES, tensors):
        ...
        attributes.setdefault("cmp_topk", attributes.get("topk"))
