...
"""Opt-in QSMLA deterministic-level-3 batch-consistency tests."""
...
result_compare_method = load_operator_module(
    "qsmla_consistency_result_compare",
    PYTEST_PATH / "result_compare_method.py",
)
quant_sparse_flash_mla_process = load_operator_module(
    "qsmla_consistency_process",
    PYTEST_PATH / "batch/quant_sparse_flash_mla_process.py",
)


class QuantSparseFlashMlaExecutor:
    """Normalize the ordinary QSMLA eager pytest tuple for the shared runner."""

    def __call__(self, data):
        device_id = int(os.environ.get("QSMLA_DEVICE_ID", "0"))
...
TEST_MATRIX = collect_case_matrix("QSMLA", "qsmla_testcase")
INDEPENDENT_RELATIONS = collect_independent_relation_matrix("QSMLA", "qsmla")

pytestmark = pytest.mark.consistency


@pytest.mark.parametrize("case_path,mode", TEST_MATRIX)
def test_quant_sparse_flash_mla_batch_consistency(case_path, mode):
    device_id = int(os.environ.get("QSMLA_DEVICE_ID", "0"))
    run_pytest_case(
        case_path,
        mode,
...
def test_quant_sparse_flash_mla_independent_batch_consistency(relation):
    device_id = int(os.environ.get("QSMLA_DEVICE_ID", "0"))
    run_independent_pytest_case(
        relation,
        "qsmla",
