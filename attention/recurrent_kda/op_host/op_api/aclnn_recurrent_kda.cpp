...
static const std::initializer_list<op::DataType> GATE_TYPE_SUPPORT_LIST = {
...
static bool CheckCuSeqlensShape(const aclTensor *cuSeqlens, const char *opName)
{
    ...
    if (Rank(cuSeqlens) != 1 || Dim(cuSeqlens, DIM0) < 2) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "%s: cuSeqlensOptional must be a 1D tensor with at least two elements.",
                opName);
        return false;
    }
    return true;
...
static bool CheckShape(const RecurrentKdaParams &params, RecurrentKdaLayout layout)
{
    ...
    if (layout == RecurrentKdaLayout::TND) {
        ...
        if (Dim(params.value, DIM0) != totalTokens || Dim(params.gate, DIM0) != totalTokens ||
            Dim(params.beta, DIM0) != totalTokens || Dim(params.gate, DIM1) != hv || Dim(params.beta, DIM1) != hv ||
            Dim(params.gate, DIM2) != kDim) {
        ...
        if (Dim(params.value, DIM0) != batch || Dim(params.value, DIM1) != denseSeqLen ||
            Dim(params.gate, DIM0) != batch || Dim(params.gate, DIM1) != denseSeqLen || Dim(params.gate, DIM2) != hv ||
            Dim(params.gate, DIM3) != kDim || Dim(params.beta, DIM0) != batch ||
            Dim(params.beta, DIM1) != denseSeqLen || Dim(params.beta, DIM2) != hv) {
    ...
    if (kDim != 128 || (vDim != 128 && vDim != 256)) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                "npu_recurrent_kda: K/V currently support only K=128,V=128 or K=128,V=256, but K=%ld,V=%ld.", kDim,
                vDim);
        return false;
    }
    ...
    int64_t seqNum = params.cuSeqlensOptional == nullptr ? ((layout == RecurrentKdaLayout::BSND) ? batch : 1) :
    ...
    bool stateTailMatches =
        params.stateVFirst ? (Dim(params.initialStateRef, DIM2) == vDim && Dim(params.initialStateRef, DIM3) == kDim) :
    ...
    if (!params.useGateInKernel &&
        (params.safeGate || params.aLogOptional != nullptr || params.dtBiasOptional != nullptr)) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID,
...
static aclnnStatus PreProcess(RecurrentKdaParams &params, aclOpExecutor *executor)
{
    ...
    if (params.ssmStateIndicesOptional == nullptr && params.numAcceptedTokensOptional != nullptr) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "npu_recurrent_kda: num_accepted_tokens requires ssm_state_indices.");
    ...
    aclOpExecutor **executor)
{
    ...
    RecurrentKdaParams params{query,
                              key,
                              value,
                              gate,
                              beta,
    ...
    if (!IsContiguous(finalStateForKernel)) {
        finalStateForKernel = executorPtr->CreateView(
            ...
            finalStateForKernel->GetViewStrides(), finalStateForKernel->GetViewOffset());
    ...
    auto result = l0op::RecurrentKda(params.query, params.key, params.value, params.gate, params.beta,
                                     ...
                                     params.layout, params.scale, params.outputFinalState, params.inplaceFinalState,
                                     params.useQkL2normInKernel, params.useGateInKernel, params.useBetaSigmoidInKernel,
                                     params.allowNegEigval, params.safeGate, params.lowerBound, params.stateVFirst,
                                     params.attnOut, finalStateForKernel, executorPtr);
    CHECK_RET(result[0] != nullptr && result[1] != nullptr && result[2] != nullptr, ACLNN_ERR_INNER_NULLPTR);
    if (params.inplaceFinalState && params.outputFinalState && params.finalState != params.initialStateRef) {
