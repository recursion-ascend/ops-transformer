...
#define INVOKE_PFA_KVANTIQUANT_OP_IMPL(templateClass, ...) \
    ...
    do { \
        ...
        op.Init(query, key, value, pseShift, attenMask, actualSeqLengths, actualSeqLengthsKV, blocktable, \
                queryPaddingSize, kvPaddingSize, keySharedPrefix, valueSharedPrefix, actualSharedPrefixLen, \
                attentionOut, softmaxLse, user, tiling_data, tiling, &tPipe); \
        op.InitKvAntiquant(antiquant_scale, antiquant_offset, key_antiquant_scale, key_antiquant_offset, \
...
#define INVOKE_PFA_GENERAL_OP_IMPL_VAR_LEN(templateClass, ...) \
    ...
    do { \
        ...
        REGIST_MATMUL_OBJ(&tPipe, GetSysWorkSpacePtr(), op.bmm1, bmm1tiling, op.bmm1Nz, bmm1tiling, op.bmm2, \
                          bmm2tiling); \
        op.UnpackInit(query, key, value, attenMask, actualSeqLengths, actualSeqLengthsKV, queryRope, keyRope, \
...
#define INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(templateClass, ...) \
    ...
    do { \
        ...
        templateClass<__VA_ARGS__> op; \
        op.UnpackInit(query, key, value, attenMask, actualSeqLengths, actualSeqLengthsKV, queryRope, keyRope, \
                      blocktable, learnableSink, attentionOut, softmaxLse, user, tilingData, &tPipe); \
        op.Process(); \
    } while (0)
#define INVOKE_PFA_GENERAL_OP_IMPL_BASE_API(templateClass, ...) \
    do { \
        ...
        op.Init(query, key, value, pseShift, attenMask, actualSeqLengths, actualSeqLengthsKV, blocktable, \
                queryPaddingSize, kvPaddingSize, keySharedPrefix, valueSharedPrefix, actualSharedPrefixLen, \
                attentionOut, softmaxLse, user, tiling_data, tiling, nullptr, deq_scale1, quant_scale1, deq_scale2, \
                quant_scale2, quant_offset2, nullptr); \
    } while (0)
#define INVOKE_PFA_GENERAL_OP_IMPL_MLA(templateClass, ...) \
    do { \
        ...
        op.Init(query, key, value, pseShift, attenMask, actualSeqLengths, actualSeqLengthsKV, blocktable, \
                queryPaddingSize, kvPaddingSize, keySharedPrefix, valueSharedPrefix, actualSharedPrefixLen, \
                attentionOut, softmaxLse, user, tiling_data, tiling, nullptr, nullptr); \
    } while (0)
...
inline __aicore__ void prompt_flash_attention_FIAS_arch22(
    ...
    __gm__ uint8_t *dequantScaleQuery, __gm__ uint8_t *learnableSink, __gm__ uint8_t *attentionOut,
    __gm__ uint8_t *softmaxLse, __gm__ uint8_t *workspace, __gm__ uint8_t *tiling)
{
    ...
    if (maskByteNum == FLOAT16BYTENUM) {
        INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionBNSTillingNSWithBNSDNoTail, half, half, CubeFormat::ND, half);
    } else {
        INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionBNSTillingNSWithBNSDNoTail, half, bool, CubeFormat::ND, half);
    }
    ...
    INVOKE_PFA_GENERAL_OP_IMPL_BASE_API(
        PromptFlashAttentionBaseApiHighPrecisionNoMask,
        PFAHighPrecisionBaseType<PromptFlashAttentionBaseApiTilingData, float, half, half, half, half, float>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_BNSD_HIGHPERFORMANCE_BASICAPI_CUBEVECTORDIFF_NEWTILING
    INVOKE_PFA_GENERAL_OP_IMPL_BASE_API(
        PromptFlashAttentionBaseApiHighPerformance,
        PFATypeNew<PromptFlashAttentionBaseApiTilingData, half, half, half, half, half, float, half>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, half, bool, half, half, OptimizationMode::HighPrecision>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(
        ...
        PFAType<PFALayout::BSH, half, bool, half, half, OptimizationMode::HighPrecision, MatMulType::MM_PA>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, half, half, OptimizationMode::HighPrecision, MatMulType::MM_MDL, true>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, half, half, OptimizationMode::HighPrecision, MatMulType::MM_IBSHARE_NORM>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BNSD, half, bool, half, half, OptimizationMode::HighPrecision>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, half, half, OptimizationMode::HighPrecision, MatMulType::MM_PA>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, half, half, OptimizationMode::HighPrecision, MatMulType::MM_MDL, true>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_BNSD_HIGHPRECISION_HIGHLEVELAPI_NORMAL_CUBEVECTORDIFF_NEWTILING
    // BNSD layout HighPrecision
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, half, half, OptimizationMode::HighPrecision, MatMulType::MM_NORM>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_BNSD_HIGHPRECISION_HIGHLEVELAPI_PREFIX_NORMAL_CUBEVECTORDIFF_NEWTILING
    // Prefix BNSD layout HighPrecision
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, half, half, OptimizationMode::HighPrecision, MatMulType::MM_NORM, true>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_BNSD_HIGHPRECISION_HIGHLEVELAPI_CUBEVECTORDIFF_NEWTILING
    // BNSD layout HighPrecision
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, half, half, OptimizationMode::HighPrecision, MatMulType::MM_IBSHARE_NORM>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, half, half, OptimizationMode::HighPerformance, MatMulType::MM_PA>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, half, uint8_t, half, half, OptimizationMode::HighPerformance,
    ...
    if (maskByteNum == FLOAT16BYTENUM) {
        INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BSH, half, half, half, half, OptimizationMode::HighPerformance,
                                           MatMulType::MM_MDL, true>);
    } else {
        INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BSH, half, bool, half, half, OptimizationMode::HighPerformance,
                                           MatMulType::MM_MDL, true>);
    }
    ...
    if (maskByteNum == FLOAT16BYTENUM) {
        INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BNSD, half, half, half, half, OptimizationMode::HighPerformance,
                                           MatMulType::MM_MDL, true>);
    } else {
        INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BNSD, half, uint8_t, half, half,
                                           OptimizationMode::HighPerformance, MatMulType::MM_MDL, true>);
    }
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_BNSD_HIGHPERFORMANCE_HIGHLEVELAPI_NORMAL_CUBEVECTORDIFF_NEWTILING
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, uint8_t, half, half, OptimizationMode::HighPerformance, MatMulType::MM_NORM>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, uint8_t, half, half, OptimizationMode::HighPerformance, MatMulType::MM_PA>);
#elif TILING_KEY_VAR == \
    QFP16_KVFP16_OUTFP16_BNSD_HIGHPERFORMANCE_HIGHLEVELAPI_PREFIX_NORMAL_CUBEVECTORDIFF_NEWTILING // enable prefix
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BNSD, half, uint8_t, half, half, OptimizationMode::HighPerformance,
                                       MatMulType::MM_NORM, true>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL_BASE_API(PromptFlashAttentionBaseApiHighPrecisionV,
                                        PFATypeNew<PromptFlashAttentionBaseApiTilingData, half, half, half, float, half,
                                                   float, half, OptimizationMode::HighPrecision>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_BNSD_HIGHPRECISION_BASICAPI_MLA_CUBEVECTORDIFF_NEWTILING
    INVOKE_PFA_GENERAL_OP_IMPL_MLA(PromptFlashAttentionBaseMLAHighPrecision,
                                   PFAHighPrecisionMLAType<PromptFlashAttentionBaseApiTilingData, half, false>);
    ...
    // BSH layout HighPrecision
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BSH, half, bool, half, int8_t, OptimizationMode::HighPrecision>);
#elif TILING_KEY_VAR == QFP16_KVINT8_OUTFP16_BSH_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING
    // Prefix BSH layout HighPrecision
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, half, int8_t, OptimizationMode::HighPrecision, MatMulType::MM_MDL, true>);
#elif TILING_KEY_VAR == QFP16_KVINT8_OUTFP16_BNSD_HIGHPRECISION_HIGHLEVELAPI_MDL_CUBEVECTORDIFF_NEWTILING
    // BNSD layout HighPrecision
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BNSD, half, bool, half, int8_t, OptimizationMode::HighPrecision>);
#elif TILING_KEY_VAR == QFP16_KVINT8_OUTFP16_BNSD_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING
    // Prefix BNSD layout HighPrecision
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, half, int8_t, OptimizationMode::HighPrecision, MatMulType::MM_MDL, true>);
    ...
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, half, int8_t, OptimizationMode::HighPerformance, MatMulType::MM_MDL, true>);
    ...
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BNSD, half, bool, half, int8_t>);
    ...
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BNSD, half, bool, half, int8_t, OptimizationMode::HighPerformance,
                                           MatMulType::MM_MDL, true>);
#elif TILING_KEY_VAR == \
    QFP16_KVFP8_OUTFP16_BSH_HIGHPRECISION_HIGHLEVELAPI_MSD_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix, enable MSD
    // BSH layout fp16 cvdiff
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, half, bool, half, int8_t, OptimizationMode::HighPrecision,
                                       MatMulType::MM_MDL, true, MsdMode::MSD_ON>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, half, bool, half, int8_t, OptimizationMode::HighPrecision,
                                       MatMulType::MM_MDL, false, MsdMode::MSD_ON>);
    ...
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BSH, half, bool, half, int8_t, OptimizationMode::HighPerformance,
                                           MatMulType::MM_PA_ANTIQUANT>);
    ...
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, int8_t, half, OptimizationMode::HighPerformance, MatMulType::MM_PA>);
    ...
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, int8_t, half, OptimizationMode::HighPerformance, MatMulType::MM_MDL, true>);
    ...
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, int8_t, half, OptimizationMode::HighPerformance, MatMulType::MM_PA>);
    ...
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BNSD, half, bool, int8_t, half, OptimizationMode::HighPerformance,
                                    MatMulType::MM_MDL, true>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTINT8_BSH_HIGHPRECISION_HIGHLEVELAPI_MDL_CUBEVECTORDIFF_NEWTILING
    // no anti-quant path for CVDIFF-BSH, half in int8 out
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BSH, half, bool, int8_t, half, OptimizationMode::HighPrecision>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTINT8_BSH_HIGHPRECISION_HIGHLEVELAPI_PA_ND_MDL_CUBEVECTORDIFF_NEWTILING
    // no anti-quant path for CVDIFF-BSH, half in int8 out, enable PA
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, int8_t, half, OptimizationMode::HighPrecision, MatMulType::MM_PA>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTINT8_BSH_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING
    // Prefix no anti-quant path for CVDIFF-BSH, half in int8 out
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, int8_t, half, OptimizationMode::HighPrecision, MatMulType::MM_MDL, true>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTINT8_BNSD_HIGHPRECISION_HIGHLEVELAPI_MDL_CUBEVECTORDIFF_NEWTILING
    // no anti-quant path for CVDIFF-BNSD, half in int8 out
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BNSD, half, bool, int8_t, half, OptimizationMode::HighPrecision>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTINT8_BNSD_HIGHPRECISION_HIGHLEVELAPI_PA_ND_MDL_CUBEVECTORDIFF_NEWTILING
    // no anti-quant path for CVDIFF-BNSD, half in int8 out, enable PA
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, int8_t, half, OptimizationMode::HighPrecision, MatMulType::MM_PA>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTINT8_BNSD_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING
    // Prefix no anti-quant path for CVDIFF-BNSD, half in int8 out
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, int8_t, half, OptimizationMode::HighPrecision, MatMulType::MM_MDL, true>);
#endif
#endif
    ...
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BSH, half, bool, int8_t, int8_t>);
    ...
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BNSD, half, bool, int8_t, int8_t>);
    ...
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, int8_t, int8_t, OptimizationMode::HighPrecision>);
#elif TILING_KEY_VAR == QFP16_KVINT8_OUTINT8_BSH_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING
    // Prefix anti-quant path for CVDIFF-BSH, half in int8 out
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, half, bool, int8_t, int8_t, OptimizationMode::HighPrecision, MatMulType::MM_MDL, true>);
#elif TILING_KEY_VAR == QFP16_KVINT8_OUTINT8_BNSD_HIGHPRECISION_HIGHLEVELAPI_MDL_CUBEVECTORDIFF_NEWTILING
    // anti-quant path for CVDIFF-BNSD, half in int8 out
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, half, bool, int8_t, int8_t, OptimizationMode::HighPrecision>);
    ...
    INVOKE_PFA_KVANTIQUANT_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                                   PFAType<PFALayout::BNSD, half, bool, int8_t, int8_t, OptimizationMode::HighPrecision,
                                           MatMulType::MM_MDL, true>);
#elif TILING_KEY_VAR == \
    QFP16_KVFP8_OUTINT8_BSH_HIGHPRECISION_HIGHLEVELAPI_MSD_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix, enable MSD
    ...
    TILING_KEY_IS(
        QFP16_KVFP16_OUTFP16_TND_ENABLEATTNETIONMASK_HIGHPERFORMANCE_PA_ND_MDL_NOTAIL_CUBEVECTORDIFF_OLDTILING);
    TILING_KEY_IS(QFP16_KVFP16_OUTFP16_NTD_TND_HIGHPERFORMANCE_PA_ND_MDL_CUBEVECTORDIFF_OLDTILING);
    TILING_KEY_IS(
        QFP16_KVFP16_OUTFP16_NTD_TND_ENABLEATTNETIONMASK_HIGHPERFORMANCE_PA_ND_MDL_NOTAIL_CUBEVECTORDIFF_OLDTILING);
    TILING_KEY_IS(QFP16_KVFP16_OUTFP16_TND_HIGHPERFORMANCE_PA_NZ_MDL_CUBEVECTORDIFF_OLDTILING);
    TILING_KEY_IS(
        QFP16_KVFP16_OUTFP16_TND_ENABLEATTNETIONMASK_HIGHPERFORMANCE_PA_NZ_MDL_NOTAIL_CUBEVECTORDIFF_OLDTILING);
    TILING_KEY_IS(QFP16_KVFP16_OUTFP16_NTD_TND_HIGHPERFORMANCE_PA_NZ_MDL_CUBEVECTORDIFF_OLDTILING);
    TILING_KEY_IS(
        QFP16_KVFP16_OUTFP16_NTD_TND_ENABLEATTNETIONMASK_HIGHPERFORMANCE_PA_NZ_MDL_NOTAIL_CUBEVECTORDIFF_OLDTILING);
    ...
    // BNSD layout, split NS no tail
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionBNSTillingNSWithBNSDNoTail, bfloat16_t, bool, CubeFormat::ND,
                               bfloat16_t);
#elif TILING_KEY_VAR == QBF16_KVFP16_OUTFP16_HIGHPERFORMANCE_HIGHLEVELAPI_MDL_NOTAIL_BNSD_NEWTILING
    // BNSD layout, split NS with tail
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionBNSTillingNSWithBNSDTail, bfloat16_t, bool, CubeFormat::ND,
                               bfloat16_t);
#elif TILING_KEY_VAR == QBF16_KVFP16_OUTBF16_BNSD_HIGHPERFORMANCE_BASICAPI_BASE_API_CUBEVECTORDIFF_NEWTILING
    INVOKE_PFA_GENERAL_OP_IMPL_BASE_API(
        PromptFlashAttentionBaseApiHighPrecisionNoMask,
        PFAHighPrecisionBaseType<PromptFlashAttentionBaseApiTilingData, float, bfloat16_t, bfloat16_t, bfloat16_t,
                                 bfloat16_t, float>);
#elif TILING_KEY_VAR == QBF16_KVFP16_OUTBF16_BNSD_HIGHPERFORMANCE_BASICAPI_CUBEVECTORDIFF_NEWTILING
    INVOKE_PFA_GENERAL_OP_IMPL_BASE_API(
        PromptFlashAttentionBaseApiHighPrecisionV,
        PFATypeNew<PromptFlashAttentionBaseApiTilingData, bfloat16_t, bfloat16_t, bfloat16_t, float, bfloat16_t, float,
                   bfloat16_t, OptimizationMode::HighPrecision>);
#elif TILING_KEY_VAR == QBF16_KVFP16_OUTBF16_BNSD_HIGHPERFORMANCE_BASICAPI_MLA_CUBEVECTORDIFF_NEWTILING
    INVOKE_PFA_GENERAL_OP_IMPL_MLA(PromptFlashAttentionBaseMLAHighPrecision,
                                   PFAHighPrecisionMLAType<PromptFlashAttentionBaseApiTilingData, bfloat16_t, true>);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_TND_HIGHPERFORMANCE_MDL_TAIL_OLDTILING
    INVOKE_PFA_GENERAL_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameAB, MLAGeneralTilingData,
                                       ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_TND, true, bfloat16_t,
                                       float);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_TND_ENABLEATTNETIONMASK_VALUED192_HIGHPERFORMANCE_MDL_NOTAIL_OLDTILING
    INVOKE_PFA_GENERAL_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameAB, MLAGeneralTilingData,
                                       ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_TND, false, bfloat16_t,
                                       float);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_TND_HIGHPERFORMANCE_MDL_CUBEVECTORDIFF_OLDTILING
    INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameABBaseApi, MLAGeneralTilingData,
                                          ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_TND, true, bfloat16_t,
                                          float, CubeFormat::NZ, MmPolicyType::NORMAL, false);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_TND_ENABLEATTNETIONMASK_HIGHPERFORMANCE_MDL_NOTAIL_CUBEVECTORDIFF_OLDTILING
    INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameABBaseApi, MLAGeneralTilingData,
                                          ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_TND, false,
                                          bfloat16_t, float, CubeFormat::NZ, MmPolicyType::NORMAL, false);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_NTD_TND_HIGHPERFORMANCE_MDL_CUBEVECTORDIFF_OLDTILING
    INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameABBaseApi, MLAGeneralTilingData,
                                          ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_NTD_TND, true,
                                          bfloat16_t, float, CubeFormat::NZ, MmPolicyType::NORMAL, false);
#elif TILING_KEY_VAR == \
    QFP16_KVFP16_OUTFP16_NTD_TND_ENABLEATTNETIONMASK_HIGHPERFORMANCE_MDL_NOTAIL_CUBEVECTORDIFF_OLDTILING
    INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameABBaseApi, MLAGeneralTilingData,
                                          ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_NTD_TND, false,
                                          bfloat16_t, float, CubeFormat::NZ, MmPolicyType::NORMAL, false);
#elif TILING_KEY_VAR == QFP16_KVFP16_OUTFP16_TND_HIGHPERFORMANCE_PA_ND_MDL_CUBEVECTORDIFF_OLDTILING
    INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameABBaseApi, MLAGeneralTilingData,
                                          ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_TND, true, bfloat16_t,
                                          float, CubeFormat::NZ, MmPolicyType::PA_ND, true);
#elif TILING_KEY_VAR == \
    QFP16_KVFP16_OUTFP16_TND_ENABLEATTNETIONMASK_HIGHPERFORMANCE_PA_ND_MDL_NOTAIL_CUBEVECTORDIFF_OLDTILING
    INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameABBaseApi, MLAGeneralTilingData,
                                          ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_TND, false,
                                          bfloat16_t, float, CubeFormat::NZ, MmPolicyType::PA_ND, true);
    ...
    INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameABBaseApi, MLAGeneralTilingData,
                                          ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_TND, true, bfloat16_t,
                                          float, CubeFormat::NZ, MmPolicyType::PA_NZ, true);
#elif TILING_KEY_VAR == \
    QFP16_KVFP16_OUTFP16_TND_ENABLEATTNETIONMASK_HIGHPERFORMANCE_PA_NZ_MDL_NOTAIL_CUBEVECTORDIFF_OLDTILING
    INVOKE_PFA_NO_KFC_MLA_OP_IMPL_VAR_LEN(PromptFlashAttentionVarLenScoreSameABBaseApi, MLAGeneralTilingData,
                                          ImplModeEnum::AA_HIGH_PRECISION, LayOutTypeEnum::LAYOUT_TND, false,
                                          bfloat16_t, float, CubeFormat::NZ, MmPolicyType::PA_NZ, true);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, bfloat16_t, bool, bfloat16_t, bfloat16_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_PA>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, bfloat16_t, bool, bfloat16_t, bfloat16_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_IBSHARE_NORM>);
#elif TILING_KEY_VAR == \
    QBF16_KVFP16_OUTBF16_BSH_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix
    // BSH layout bf16 cvdiff
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, bfloat16_t, bool, bfloat16_t, bfloat16_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_MDL, true>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BNSD, bfloat16_t, bool, bfloat16_t>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BNSD, bfloat16_t, bool, bfloat16_t, bfloat16_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_PA>);
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BNSD, bfloat16_t, bool, bfloat16_t, bfloat16_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_IBSHARE_NORM>);
#elif TILING_KEY_VAR == \
    QBF16_KVFP16_OUTBF16_BNSD_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix
    // BNSD layout bf16 cvdiff
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BNSD, bfloat16_t, bool, bfloat16_t, bfloat16_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_MDL, true>);
...
#if TILING_KEY_VAR == \
    QBF16_KVFP8_OUTBF16_BSH_HIGHPRECISION_HIGHLEVELAPI_MSD_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix, enable MSD
    // BSH layout bf16 cvdiff
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, bfloat16_t, bool, bfloat16_t, int8_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_MDL, true, MsdMode::MSD_ON>);
#elif TILING_KEY_VAR == \
    QBF16_KVFP8_OUTBF16_BNSD_HIGHPRECISION_HIGHLEVELAPI_MSD_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix, enable MSD
    // BNSD layout bf16 cvdiff
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BNSD, bfloat16_t, bool, bfloat16_t, int8_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_MDL, true, MsdMode::MSD_ON>);
#elif TILING_KEY_VAR == QBF16_KVFP8_OUTBF16_BSH_HIGHPRECISION_HIGHLEVELAPI_MDL_CUBEVECTORDIFF_NEWTILING
    // BSH layout bf16 cvdiff
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                               PFAType<PFALayout::BSH, bfloat16_t, bool, bfloat16_t, int8_t,
                                       OptimizationMode::HighPerformance, MatMulType::MM_MDL, false, MsdMode::MSD_ON>);
    ...
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BSH, bfloat16_t, bool, int8_t, bfloat16_t,
                                    OptimizationMode::HighPerformance, MatMulType::MM_PA>);
#elif TILING_KEY_VAR == \
    QBF16_KVFP16_OUTINT8_BSH_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix
    // BSH layout bf16 in int8 out cvdiff
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BSH, bfloat16_t, bool, int8_t, bfloat16_t,
                                    OptimizationMode::HighPerformance, MatMulType::MM_MDL, true>);
    ...
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BNSD, bfloat16_t, bool, int8_t, bfloat16_t,
                                    OptimizationMode::HighPerformance, MatMulType::MM_PA>);
#elif TILING_KEY_VAR == \
    QBF16_KVFP16_OUTINT8_BNSD_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix
    // BNSD layout bf16 in int8 out cvdiff
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BNSD, bfloat16_t, bool, int8_t, bfloat16_t,
                                    OptimizationMode::HighPerformance, MatMulType::MM_MDL, true>);
...
#if TILING_KEY_VAR == \
    QBF16_KVFP8_OUTINT8_BSH_HIGHPRECISION_HIGHLEVELAPI_MSD_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix, enable MSD,
                                                                                        // enable MSD
    // BSH layout bf16 in int8 out cvdiff
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BSH, bfloat16_t, bool, int8_t, int8_t, OptimizationMode::HighPerformance,
                                    MatMulType::MM_MDL, true, MsdMode::MSD_ON>);
#elif TILING_KEY_VAR == \
    QBF16_KVFP8_OUTINT8_BNSD_HIGHPRECISION_HIGHLEVELAPI_MSD_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix, enable MSD
    // BNSD layout bf16 in int8 out cvdiff
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BNSD, bfloat16_t, bool, int8_t, int8_t,
                                    OptimizationMode::HighPerformance, MatMulType::MM_MDL, true, MsdMode::MSD_ON>);
...
#elif TILING_KEY_VAR == \
    QINT8_KVFP16_OUTINT8_BNSD_HIGHPRECISION_HIGHLEVELAPI_PA_ND_MDL_CUBEVECTORDIFF_NEWTILING // enable PA
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, int8_t, bool, int8_t, int8_t, OptimizationMode::HighPerformance, MatMulType::MM_PA>);
#elif TILING_KEY_VAR == \
    QINT8_KVFP16_OUTINT8_BNSD_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BSH, int8_t, bool, int8_t, int8_t, OptimizationMode::HighPerformance,
                                    MatMulType::MM_MDL, true>);
...
#elif TILING_KEY_VAR == \
    QINT8_KVFP16_OUTINT8_HIGHPRECISION_HIGHLEVELAPI_PA_ND_MDL_CUBEVECTORDIFF_BNSD_NEWTILING // enable PA
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, int8_t, bool, int8_t, int8_t, OptimizationMode::HighPerformance, MatMulType::MM_PA>);
#elif TILING_KEY_VAR == \
    QINT8_KVFP16_OUTINT8_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_BNSD_NEWTILING // enable prefix
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BNSD, int8_t, bool, int8_t, int8_t, OptimizationMode::HighPerformance,
                                    MatMulType::MM_MDL, true>);
#endif
#endif
...
#elif TILING_KEY_VAR == \
    QINT8_KVFP16_OUTFP16_BNSD_HIGHPRECISION_HIGHLEVELAPI_PA_ND_MDL_CUBEVECTORDIFF_NEWTILING // enable PA
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BSH, int8_t, bool, half, int8_t, OptimizationMode::HighPerformance, MatMulType::MM_PA>);
...
#elif TILING_KEY_VAR == \
    QINT8_KVFP16_OUTFP16_HIGHPRECISION_HIGHLEVELAPI_PA_ND_MDL_CUBEVECTORDIFF_BNSD_NEWTILING // enable PA
    INVOKE_PFA_INT8_OP_IMPL(
        PromptFlashAttentionS1s2Bns1X910,
        PFAType<PFALayout::BNSD, int8_t, bool, half, int8_t, OptimizationMode::HighPerformance, MatMulType::MM_PA>);
#elif TILING_KEY_VAR == \
    QINT8_KVFP16_OUTFP16_BNSD_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_NEWTILING // enable prefix
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BSH, int8_t, bool, half, int8_t, OptimizationMode::HighPerformance,
                                    MatMulType::MM_MDL, true>);
#elif TILING_KEY_VAR == \
    QINT8_KVFP16_OUTFP16_HIGHPRECISION_HIGHLEVELAPI_PREFIX_MDL_CUBEVECTORDIFF_BNSD_NEWTILING // enable prefix
    INVOKE_PFA_INT8_OP_IMPL(PromptFlashAttentionS1s2Bns1X910,
                            PFAType<PFALayout::BNSD, int8_t, bool, half, int8_t, OptimizationMode::HighPerformance,
                                    MatMulType::MM_MDL, true>);
#endif
#endif
    ...
    INVOKE_PFA_NEW_GQA_OP_IMPL(PromptAttentionPrefill, PFATypeNZ<PFALayoutNZ::BNSD, half, int8_t>,
                               PrecType::BMM1_FP16_EXP_FP32); // 高性能
#elif TILING_KEY_VAR == QINT8_KVFP16_OUTINT8_BSH_HIGHLEVELAPI_MDL_310TILING
    INVOKE_PFA_NEW_GQA_OP_IMPL(PromptAttentionPrefill, PFATypeNZ<PFALayoutNZ::BSH, half, int8_t>,
                               PrecType::BMM1_FP16_EXP_FP32); // 高性能
    ...
    INVOKE_PFA_GENERAL_OP_IMPL(PromptFlashAttentionS1s2Bns1X310,
