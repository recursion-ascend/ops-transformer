...
template <PFALayout L, typename T, typename U, typename O = T, typename KV_T = T,
          OptimizationMode M = OptimizationMode::HighPerformance, const MatMulType MM_TYPE_TMP = MatMulType::MM_MDL,
          const bool F = false, const MsdMode MSD_MODE = MsdMode::MSD_OFF, typename... Args>
struct PFAType {
    using inputType = T;
    using maskType = U;
    using outputType = O;
...
public:
                                      ...
                                      int useK, int useN, const uint64_t tilingPtr, const uint64_t dataPtr)
    {
        ...
        uint64_t bmm1TensorBAddr =
            (static_cast<uint64_t>(bmm1TensorBAddr_high) << 32U) | static_cast<uint64_t>(bmm1TensorBAddr_low);
        uint64_t bmm1BlockTableAddr =
            (static_cast<uint64_t>(bmm1BlockTableAddr_high) << 32U) | static_cast<uint64_t>(bmm1BlockTableAddr_low);

        // In other scenarios, the tiling result in the tilingptr configured on the V side is used.
        __gm__ PromptFlashAttentionTilingData *tilingDataPtr =
            reinterpret_cast<__gm__ PromptFlashAttentionTilingData *>(tilingPtr);
        ...
        uint32_t s2BaseOffset =
            col * baseN; // The offset of the current useN block in the single block in the S2 direction.
        ...
        while (copyFinishRowCnt < useN) { // 1. useN <= blockSize : copy part of the blockSize 2. useN > blockSize:
                                          // Multiple copies of a callback 3. Tail block
            ...
            blockId =
                *(reinterpret_cast<__gm__ int32_t *>(bmm1BlockTableAddr) + bmm1BIdx * blockTableDim2 + blockTableIdx);
                                      ...
                                      int useK, int useN, const uint64_t tilingPtr, const uint64_t dataPtr)
    {
        ...
        uint64_t bmm2TensorBAddr =
            (static_cast<uint64_t>(bmm2TensorBAddr_high) << 32U) | static_cast<uint64_t>(bmm2TensorBAddr_low);
        uint64_t bmm2BlockTableAddr =
            (static_cast<uint64_t>(bmm2BlockTableAddr_high) << 32U) | static_cast<uint64_t>(bmm2BlockTableAddr_low);
        ...
        while (copyFinishRowCnt < useK) {
            ...
            blockRowOffsetInSingle = blockTableIdx * blockSize - s2SingleOffset;
            blockId =
                *(reinterpret_cast<__gm__ int32_t *>(bmm2BlockTableAddr) + bmm2BIdx * blockTableDim2 + blockTableIdx);
...
protected:
    ...
    __aicore__ inline void SoftmaxComputeFirstTail(LocalTensor<computeType> &mmResUb, LocalTensor<float> &softmaxMaxUb,
                                                   LocalTensor<float> &softmaxSumUb, uint32_t souterSize);

    __aicore__ inline void SoftmaxComputeTail(LocalTensor<computeType> &mmResUb, LocalTensor<float> &softmaxMaxUb,
    ...
    __aicore__ inline void SoftmaxLseCopyOut(LocalTensor<float> &softmaxSumUb, LocalTensor<float> &softmaxMaxUb)
    {
        ...
        if (this->isSoftmaxLseNeedUpdate) {
            SoftMaxShapeInfo softmaxShapeInfo = {static_cast<uint32_t>(souterSize), static_cast<uint32_t>(8),
        ...
        if constexpr (PFAT::msdMode == MsdMode::MSD_ON) {
            if (this->tilingData->promptAttentionBaseParams.keyAntiquantMode == 0 and this->msdIsKOffsetExist) {
                ...
                Muls(qRowSumUb, qRowSumUb, static_cast<float>(this->tilingData->promptAttentionBaseParams.scaleValue),
        ...
        if (likely(souterSize % 8 ==
                   0)) { // copyout element num is 32B aligned, can use DataCopy to acquire high performance
            GatherMaskParams gatherMaskParams{
                1, 0, 8, 0}; // src0BlockStride = 1, repeatTimes = 0, src0RepeatStride = 8, src1RepeatStride = 0
            ...
            DataCopyParams dataCopyParams{static_cast<uint16_t>(souterSize), 4, 0,
                                          0}; // blockCount = souterSize, blockLen = 4, srcStride = 0, dstStride = 0
    ...
    __aicore__ inline void PostQuant2PerChannelBF16(LocalTensor<computeType> &bmm2ResUb,
                                                    LocalTensor<int8_t> &outputQuantRes);

    __aicore__ inline void PostQuant2PerChannelFP16(LocalTensor<computeType> &bmm2ResUb,
                                                    LocalTensor<int8_t> &outputQuantRes);

    __aicore__ inline void PostQuant2PerChannelFP32(LocalTensor<computeType> &bmm2ResUb,
                                                    LocalTensor<int8_t> &outputQuantRes);
    ...
    __aicore__ inline void DataCopyTransposeOutBSH(LocalTensor<computeType> &bmm2ResUb)
    {
        ...
        if (this->preHeadParams->preTokensPerBatch < 0) {
            int64_t preTokenLength = this->preHeadParams->actualSeqLengthKVPerBatch + actualKVPrefixLen +
                                     this->preHeadParams->preTokensPerBatch;
        ...
        if (this->preHeadParams->sOuterOffset < this->preHeadParams->nextTokensPerBatch * (-1) &&
            (this->preHeadParams->sOuterOffset + this->preHeadParams->singleProcessSOuterSize) >
        ...
        int64_t multiSeqOffset = (this->tilingData->promptAttentionBaseParams.isQHasLeftPadding &&
                                  this->tilingData->promptAttentionBaseParams.isBSNDOut) ?
                                     this->preHeadParams->multiSeqOffsetBSNDOut :
                                     this->preHeadParams->multiSeqOffset;
        if constexpr (IsSameType<O, int8_t>::value) {
            ...
            if (isQuant2PerChn) { // per-channel
                if (isQuant2BF16) { // scale2 and offset2 is bf16，now qkv is also bf16，bmm2 output is fp32，scale2 and
                                    // offset2 need to cast to FP32.
                    PostQuant2PerChannelBF16(bmm2ResUb, outputQuantRes);
                } else if (isQuant2FP16) {
                    PostQuant2PerChannelFP16(bmm2ResUb, outputQuantRes);
                } else { // scale2 and offset2 is fp32. High performance requires casting to fp16，high-precision/bf16
                         // directly do quant.
                    PostQuant2PerChannelFP32(bmm2ResUb, outputQuantRes);
                }
            } else { // Perform per-channel quantiation, high performance requires conversion to fp16, high precision or
                     // bf16 directly quantizes.
        ...
        } else if constexpr (PFAT::calcMode == OptimizationMode::HighPrecision || IsSameType<T, bfloat16_t>::value) {
            ...
            DataCopyTranspose2<O>(
                attentionOutGm, FinalResUb[nextTokensOffset * tilingData->promptAttentionBaseParams.headSize],
                CopyTransposeType::TRANSPOSE_ND_UB_GM, transposeParams, transposeTilingData22, multiSeqOffset);
        } else {
    ...
    __aicore__ inline void DataCopyTransposeOutBNSD(LocalTensor<computeType> &bmm2ResUb)
    {
        ...
        int64_t nextTokensOffset = 0;
        uint64_t copySize =
            this->preHeadParams->singleProcessSOuterSize * tilingData->promptAttentionBaseParams.headSize;
        if (this->preHeadParams->preTokensPerBatch < 0) {
            int64_t preTokenLength = this->preHeadParams->actualSeqLengthKVPerBatch + actualKVPrefixLen +
                                     this->preHeadParams->preTokensPerBatch;
            if (this->preHeadParams->sOuterOffset < preTokenLength &&
    ...
    TEMPLATE_LAYOUT
    __aicore__ inline TYPENAME_BSH_VOID ComputeOffset(PFAComputeParam *params, uint32_t sInnerLoopIdx,
                                                      int32_t firstInnerMargin)
    {
        ...
        if constexpr (PFAT::enablePrefix) {
            if (!params->isPrefixInnerIter) {
                int64_t prefixIdxOffset = (actualKVPrefixLen + this->tailParams->singleProcessSInnerSize - 1) /
                                          this->tailParams->singleProcessSInnerSize;
                sInnerOffsetDataSize =
                    (((int64_t)sInnerLoopIdx - prefixIdxOffset) * this->tailParams->singleProcessSInnerSize +
                     firstInnerMargin);
                computeOffset = sInnerOffsetDataSize + this->actualKVPrefixLen;
                this->tailParams->tensorBOffset =
                    tensorBCoreOffset + (int64_t)sInnerOffsetDataSize * (int64_t)MultiHeadKV;
                this->tailParams->valueOffset = valueCoreOffset + (int64_t)sInnerOffsetDataSize * (int64_t)MultiHeadKV;
            } else {
                sInnerOffsetDataSize =
                    ((int64_t)sInnerLoopIdx * this->tailParams->singleProcessSInnerSize + firstInnerMargin);
                computeOffset = sInnerOffsetDataSize;
                this->tailParams->tensorBOffset =
                    tensorBPrefixCoreOffset + (int64_t)sInnerOffsetDataSize * (int64_t)MultiHeadKV;
                this->tailParams->valueOffset =
                    valuePrefixCoreOffset + (int64_t)sInnerOffsetDataSize * (int64_t)MultiHeadKV;
            }
        } else {
            sInnerOffsetDataSize =
                (int64_t)sInnerLoopIdx * this->tailParams->singleProcessSInnerSize + firstInnerMargin;
    ...
    TEMPLATE_LAYOUT
    __aicore__ inline TYPENAME_BNSD_VOID ComputeOffset(PFAComputeParam *params, uint32_t sInnerLoopIdx,
                                                       int32_t firstInnerMargin)
    {
        int64_t sInnerOffsetDataSize = 0;
        int64_t computeOffset = 0;
        if constexpr (PFAT::enablePrefix) {
            if (!params->isPrefixInnerIter) {
                ...
                this->tailParams->valueOffset =
                    valueCoreOffset +
                    (int64_t)sInnerOffsetDataSize * (int64_t)this->tilingData->promptAttentionBaseParams.headSize;
            } else {
                sInnerOffsetDataSize =
                    ((int64_t)sInnerLoopIdx * this->tailParams->singleProcessSInnerSize + firstInnerMargin);
                computeOffset = sInnerOffsetDataSize;
                this->tailParams->valueOffset =
                    valuePrefixCoreOffset +
                    (int64_t)sInnerOffsetDataSize * (int64_t)this->tilingData->promptAttentionBaseParams.headSize;
            }
        } else {
            sInnerOffsetDataSize =
    ...
    __aicore__ inline int64_t GetQueryLeftPaddingSize(int sIdx)
    {
        ...
        int64_t actSeqLensSize = tilingData->promptAttentionBaseParams.actualSeqLengthsSize == 1 ?
                                     this->actualSeqLengthsGm.GetValue(0) :
                                     this->actualSeqLengthsGm.GetValue(sIdx);
        int64_t leftPaddingSize = this->tilingData->promptAttentionBaseParams.seqSize - actSeqLensSize -
                                  rightPaddingSize; // this->actualSeqLengthsGm.GetValue(sIdx)
    ...
    __aicore__ inline int64_t GetKVLeftPaddingSize(int sIdx)
    {
        ...
        int64_t actSeqLensKVSize = tilingData->promptAttentionBaseParams.actualSeqLengthsKVSize == 1 ?
                                       this->actualSeqLengthsKVGm.GetValue(0) :
                                       this->actualSeqLengthsKVGm.GetValue(sIdx);
        int64_t leftPaddingSize =
            this->tilingData->promptAttentionBaseParams.seqInnerSize - actSeqLensKVSize - rightPaddingSize;
    ...
    __aicore__ inline void CalPrefixCoreOffset(PFAComputeParam *params)
    {
        if constexpr (PFAT::enablePrefix) {
            if (PFAT::layout == PFALayout::BNSD) {
                this->tensorBPrefixCoreOffset = (int64_t)params->batchNOffset / (int64_t)this->headNumRatio *
                ...
                this->tensorBPrefixCoreOffset = (int64_t)this->tailParams->batchNOffset / (int64_t)this->headNumRatio *
                                                (int64_t)this->tilingData->promptAttentionBaseParams.headSize;
            }
    ...
    __aicore__ inline TYPENAME_BSH_VOID LoopSOuterOffsetInit(int64_t seqListOffsetSize, int sIdx)
    {
        ...
        attenMaskCoreOffset = attenMaskBatchOffset +
                              (uint64_t)(this->tailParams->sOuterOffset + queryLeftpaddingSize) *
                                  (uint64_t)tilingData->promptAttentionBaseParams.maskKVsSize +
                              (uint64_t)kvLeftPaddingSize;

        tensorACoreOffset =
            (int64_t)seqListOffsetSize + (int64_t)this->tailParams->sOuterOffset * (int64_t)MultiHeadQ +
        ...
        if (this->tilingData->promptAttentionBaseParams.isKVHasLeftPadding) {
            ...
            // This is the offset required from the GM of KV to the starting address of each batch. Each batch needs to
            // be offset by the length of the entire previous batch.
            seqInnerOffsetSize =
        ...
        tensorBCoreOffset = (int64_t)seqInnerOffsetSize + this->tailParams->batchNOffset / headNumRatio *
        ...
        this->antiqParamBatchOffsetPerToken =
            (int64_t)sIdx * (int64_t)this->tilingData->promptAttentionBaseParams.seqInnerSize;
    }
    ...
    __aicore__ inline TYPENAME_BNSD_VOID LoopSOuterOffsetInit(int64_t seqListOffsetSize, int sIdx)
    {
        ...
        if (this->isKvContinuous == 1) {
            ...
            head_stride_kv = tilingData->promptAttentionBaseParams.headSize * s2InCurrentBatch;
        }
        ...
        int64_t kvLeftPaddingSize = GetKVLeftPaddingSize(sIdx);
        attenMaskCoreOffset = attenMaskBatchOffset +
                              ((uint64_t)this->tailParams->sOuterOffset + (uint64_t)queryLeftpaddingSize) *
                                  (uint64_t)tilingData->promptAttentionBaseParams.maskKVsSize +
                              (uint64_t)kvLeftPaddingSize;
                                                              ...
                                                              int32_t firstInnerMargin)
    {
        ...
        if constexpr (PFAT::enablePrefix) {
            if (params->isPrefixInnerIter) {
                return this->tensorBPrefixCoreOffset +
                       ((int64_t)sInnerLoopIdx * (int64_t)params->singleProcessSInnerSize + (int64_t)firstInnerMargin) *
                           (int64_t)this->MultiHeadKV;
            }
            prefixIdxOffset = (actualKVPrefixLen + (int64_t)params->singleProcessSInnerSize - 1) /
                              (int64_t)params->singleProcessSInnerSize;
        }
        return this->tensorBCoreOffset +
               (((int64_t)sInnerLoopIdx - prefixIdxOffset) * (int64_t)params->singleProcessSInnerSize +
    ...
    __aicore__ inline TYPENAME_MASKTYPE_HALF_VOID ElewiseCompute(LocalTensor<computeType> &mmResUb, uint32_t sOuterSize,
                                                                 uint32_t sInnerSize, uint32_t maskCopyInCol,
                                                                 bool useMask, event_t &copyIn, uint32_t type)
    {
                                                                 ...
                                                                 bool useMask, event_t &copyIn, uint32_t type)
    {
        if (useMask) {
            this->attenMaskUb =
                this->tempBmm2Queue.template DeQue<U>(); // Dequeue the attentio mask tensor from the queue.
            ...
            if (type == 0) {
                SelectWithBytesMask(mmResUb, mmResUb, scalar, this->attenMaskUb, selectSpace,
                       ...
                       1) { //  swape param 2 and param 3 of SelectWithBytesMask to compute attenMaskPre for band mode
                                                                 ...
                                                                 bool useMask, event_t &copyIn, uint32_t type)
    {
        if (useMask) {
            ...
            if (type == 0) {
                ...
                SelectWithBytesMask(mmResUb, scalar, mmResUb, this->attenMaskUb, selectSpace,
                                    selectWithBytesMaskShapeInfo); // swape param 2 and param 3 of SelectWithBytesMask
                                                                   // to compute attenMaskPre for band mode
            }
            PipeBarrier<PIPE_V>();
            tempBmm2Queue.FreeTensor(this->attenMaskUb);
        }
    ...
    __aicore__ inline void ComputeAttenMaskOffset(int64_t sInnerOffsetDataSize)
    {
        ...
        if (attentionMaskType == 2 || attentionMaskType == 3 ||
            attentionMaskType ==
                4) { // 2:leftUp mode of sparseMode, 3:rightdown mode of sparseMode, 4:band mode of sparseMode
            if (attentionMaskType == 2) {
                ...
                delta = this->tailParams->sOuterOffset - sInnerOffsetDataSize + this->tailParams->nextTokensPerBatch;
            }
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::PostQuant2PerChannelBF16(
    LocalTensor<computeType> &bmm2ResUb, LocalTensor<int8_t> &outputQuantRes)
{
    if constexpr (IsSameType<computeType, float>::value) {
        ...
        DataCopy(quantScale2Ub, quantScale2BF16Gm[(uint64_t)this->preHeadParams->batchNOffset * perChannelQuantUBSize],
                 perChannelQuantUBSize);
        if (isQuantOffset2Exist) {
            quantOffset2Ub = quantOffset2Size16Ub.Get<bfloat16_t>(perChannelQuantUBSize);
            DataCopy(quantOffset2Ub,
                     quantOffset2BF16Gm[(uint64_t)this->preHeadParams->batchNOffset * perChannelQuantUBSize],
                     perChannelQuantUBSize);
        }
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::PostQuant2PerChannelFP16(
    LocalTensor<computeType> &bmm2ResUb, LocalTensor<int8_t> &outputQuantRes)
{
    if constexpr (IsSameType<computeType, float>::value) {
        LocalTensor<half> quantScale2Ub = quantScale2Size16Ub.Get<half>(perChannelQuantUBSize);
        LocalTensor<half> quantOffset2Ub;
        DataCopy(quantScale2Ub, quantScale2FP16Gm[(uint64_t)this->preHeadParams->batchNOffset * perChannelQuantUBSize],
                 perChannelQuantUBSize);
        if (isQuantOffset2Exist) {
            quantOffset2Ub = quantOffset2Size16Ub.Get<half>(perChannelQuantUBSize);
            DataCopy(quantOffset2Ub,
                     quantOffset2FP16Gm[(uint64_t)this->preHeadParams->batchNOffset * perChannelQuantUBSize],
                     perChannelQuantUBSize);
        }
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::PostQuant2PerChannelFP32(
    LocalTensor<computeType> &bmm2ResUb, LocalTensor<int8_t> &outputQuantRes)
{
    ...
    DataCopy(quantScale2UbTmp, quantScale2FP32Gm[(uint64_t)this->preHeadParams->batchNOffset * perChannelQuantUBSize],
             perChannelQuantUBSize);
    if (isQuantOffset2Exist) {
        DataCopy(quantOffset2UbTmp,
                 quantOffset2FP32Gm[(uint64_t)this->preHeadParams->batchNOffset * perChannelQuantUBSize],
                 perChannelQuantUBSize);
    }
    auto quantParamCast = GetTPipePtr()->FetchEventID(HardEvent::MTE2_V);
    SetFlag<HardEvent::MTE2_V>(quantParamCast);
    ...
    PipeBarrier<PIPE_V>();
    if (isQuantOffset2Exist) {
        AscendQuant(outputQuantRes, bmm2ResUb, quantScale2Ub, quantOffset2Ub, perChannelQuantUBSize,
                    perChannelQuantUBSize, bmm2ResUb.GetSize());
    } else {
        AscendQuant(outputQuantRes, bmm2ResUb, quantScale2Ub, static_cast<computeType>(0), perChannelQuantUBSize,
                    bmm2ResUb.GetSize());
    }
}
...
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::CalPseShiftOffset(int sIdx)
{
    ...
    pseShiftCoreOffset = pseShiftBatchOffset + (uint64_t)this->tailParams->batchNOffset * pseShiftS1 * pseShiftS2 +
                         ((uint64_t)this->tailParams->sOuterOffset + queryLeftpaddingSize) * pseShiftS2 +
                         kvLeftPaddingSize;
}

// quant: add quant functions
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::QuantCompute(LocalTensor<int8_t> quantResUb,
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::InitScale2InQuant(__gm__ uint8_t *scale2,
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::InitOffset2InQuant(__gm__ uint8_t *scale2,
                                                                                      __gm__ uint8_t *offset2)
{
    if (offset2 != nullptr) {
                                                                             ...
                                                                             __gm__ uint8_t *offset2)
{
    ...
    // Whether the per-tensor supports scale2 and offset input BF16? The current modification has removed this feature
    // and now has a rollback function.
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::InitKvAntiquant(
    __gm__ uint8_t *antiq_scale, __gm__ uint8_t *antiq_offset, __gm__ uint8_t *key_antiquant_scale,
    __gm__ uint8_t *key_antiquant_offset, __gm__ uint8_t *value_antiquant_scale, __gm__ uint8_t *value_antiquant_offset)
{
    if constexpr (PFAT::msdMode != MsdMode::MSD_ON) {
        PFA_InitQueue(kvAntiquantSrcQueue, 1,
                      tilingData->promptAttentionTensorSizeRect.kvAntiquantUbSize * sizeof(int8_t));
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::InitMsd(__gm__ uint8_t *key_antiquant_scale,
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::Init(
    ...
    const PromptFlashAttentionTilingData *__restrict tiling, __gm__ uint8_t *gmTiling, TPipe *tPipe)
{
    ...
    if constexpr (PFAT::msdMode == MsdMode::MSD_ON) {
        ...
        PFA_InitBuffer(msdAMaxTmpBuff,
                       tilingData->promptAttentionTensorSizeRect.msdAMaxTmpBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        PFA_InitBuffer(msdAMaxResBuff[0],
                       tilingData->promptAttentionTensorSizeRect.msdAMaxResBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        PFA_InitBuffer(msdAMaxResBuff[1],
                       tilingData->promptAttentionTensorSizeRect.msdAMaxResBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        PFA_InitBuffer(msdQRowSumBuff[0],
                       tilingData->promptAttentionTensorSizeRect.msdQRowSumBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        PFA_InitBuffer(msdQRowSumBuff[1],
                       tilingData->promptAttentionTensorSizeRect.msdQRowSumBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        PFA_InitBuffer(msdSoftmaxResAmaxBuff[0],
                       tilingData->promptAttentionTensorSizeRect.msdSoftmaxResAmaxBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        PFA_InitBuffer(msdSoftmaxResAmaxBuff[1],
                       tilingData->promptAttentionTensorSizeRect.msdSoftmaxResAmaxBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        PFA_InitBuffer(
            msdSoftmaxRowSumScaleBuff[0],
            tilingData->promptAttentionTensorSizeRect.msdSoftmaxRowSumScaleBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        PFA_InitBuffer(
            msdSoftmaxRowSumScaleBuff[1],
            tilingData->promptAttentionTensorSizeRect.msdSoftmaxRowSumScaleBuffSize + PFA_BUFFER_SIZE_BYTE_256B);
        ...
        PFA_InitQueue(msdOutQueue, 1,
                      tilingData->promptAttentionTensorSizeRect
                          .msdOutQueueSize); // 8k， msd模式下，新增ub,大小为：max_qs * d * sizeof(bf16)
    }
...
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::InitOutputSingleCore()
{
    ...
    if (tailSize > 0) {
        ...
        if constexpr (IsSameType<O, int8_t>::value) {
            ...
            InitOutput<half>(attentionOutInitGm[tmp_block_idx * (int64_t)initParams.singleCoreSize / 2],
...
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::SoftmaxBasicComputeFirstNoTail(
    LocalTensor<computeType> &mmResUb, LocalTensor<float> &softmaxMaxUb, LocalTensor<float> &softmaxSumUb,
    uint32_t souterSize)
{
    ...
    if (this->headParams->isInnerTail) {
        softmaxShapeInfo = {
            static_cast<uint32_t>(souterSize), static_cast<uint32_t>(this->headParams->singleProcessSInnerSizeNow),
            static_cast<uint32_t>(souterSize), static_cast<uint32_t>(this->headParams->singleProcessSInnerBmmTail)};
    } else {
        softmaxShapeInfo = {static_cast<uint32_t>(souterSize), // translate into type uint32_t
                            static_cast<uint32_t>(this->headParams->singleProcessSInnerSizeNow),
                            static_cast<uint32_t>(souterSize),
                            static_cast<uint32_t>(this->headParams->singleProcessSInnerSizeNow)};
    }
    SoftmaxFlashV2<computeType, false, true, true>(mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, null, softmaxSumUb,
                                                   softmaxMaxUb, softmaxFlashTilingData, softmaxShapeInfo);

    if (this->isSoftmaxResNeedUpdate) {
        this->isSoftmaxResNeedUpdate =
            AdjustSoftMaxRes<computeType, float>(mmResUb, softmaxMaxUb, this->negativeScalar, 0.0, softmaxShapeInfo);
        this->isSoftmaxLseNeedUpdate = true;
    }
}

template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::SoftmaxBasicComputeNoTail(
    LocalTensor<computeType> &mmResUb, LocalTensor<float> &softmaxMaxUb, LocalTensor<float> &softmaxSumUb,
    LocalTensor<computeType> &softmaxExpUb, uint32_t souterSize)
{
    SoftMaxShapeInfo softmaxShapeInfo;
    ...
    SoftmaxFlashV2<computeType, true, true, true>(mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::SoftmaxComputeFirstTail(
    LocalTensor<computeType> &mmResUb, LocalTensor<float> &softmaxMaxUb, LocalTensor<float> &softmaxSumUb,
    uint32_t souterSize)
{
    LocalTensor<computeType> null;
    ...
    SoftmaxFlashV2<computeType, false, true, false>(mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, null, softmaxSumUb,
                                                    softmaxMaxUb, softmaxFlashTilingData, softmaxShapeInfo);
    if (this->isSoftmaxResNeedUpdate) {
        this->isSoftmaxResNeedUpdate =
            AdjustSoftMaxRes<computeType, float>(mmResUb, softmaxMaxUb, this->negativeScalar, 0.0, softmaxShapeInfo);
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::SoftmaxComputeTail(
    LocalTensor<computeType> &mmResUb, LocalTensor<float> &softmaxMaxUb, LocalTensor<float> &softmaxSumUb,
    LocalTensor<computeType> &softmaxExpUb, uint32_t souterSize)
{
    SoftMaxShapeInfo softmaxShapeInfo;
    ...
    SoftmaxFlashV2<computeType, true, true, false>(mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
                                                   softmaxSumUb, softmaxMaxUb, softmaxFlashTilingData,
                                                   softmaxShapeInfo);
    if (this->isSoftmaxResNeedUpdate) {
        this->isSoftmaxResNeedUpdate =
            AdjustSoftMaxRes<computeType, float>(mmResUb, softmaxMaxUb, this->negativeScalar, 0.0, softmaxShapeInfo);
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::Bmm2UpdateDivNoTail(
    LocalTensor<computeType> &bmm2ResPreUb, LocalTensor<float> &softmaxSumUb)
{
    ...
    if constexpr (IsSameType<computeType, half>::value) {
        ...
        for (int i = 0; i < loop; i++) {
            Div(bmm2ResPreUb[i * REPEAT_DATA_NUM], bmm2ResPreUb[i * REPEAT_DATA_NUM], tmpHalfBuffer, REPEAT_DATA_NUM,
                params->singleProcessSOuterSize, repeatParams);
        }
        if (remain) {
            Div(bmm2ResPreUb[loop * REPEAT_DATA_NUM], bmm2ResPreUb[loop * REPEAT_DATA_NUM], tmpHalfBuffer, remain,
                params->singleProcessSOuterSize, repeatParams);
        }
...
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::GetSingleCoreParam(int sIdx)
{
    ...
    if (isActualLenDimsNull) {
        ...
        actualSeqLengthPerBatch = (tilingData->promptAttentionBaseParams.actualSeqLengthsSize == actualSeqQMin) ?
                                      actualSeqLengthsGm.GetValue(0) :
                                      actualSeqLengthsGm.GetValue(sIdx);
    }
    if (isActualLenDimsKVNull) {
        actualSeqLengthKVPerBatch =
            (isKvContinuous == 1) ? tilingData->promptAttentionBaseParams.seqInnerSize : s2InCurrentBatch;
    } else {
        actualSeqLengthKVPerBatch = (tilingData->promptAttentionBaseParams.actualSeqLengthsKVSize == actualSeqKVmin) ?
    ...
    if (attentionMaskType != 4) {
        ...
        if (isKvContinuous == 1) {
            actualSeqLengthPerBatch =
                ((int64_t)actualSeqLengthPerBatch > (int64_t)actualSeqLengthKVPerBatch + actualKVPrefixLen +
        ...
        this->tailParams->preTokensPerBatch = (int64_t)tilingData->promptAttentionBaseParams.preTokens -
                                              actualSeqLengthKVPerBatch - actualKVPrefixLen + actualSeqLengthPerBatch;
        this->tailParams->nextTokensPerBatch = (int64_t)tilingData->promptAttentionBaseParams.nextTokens +
                                               actualSeqLengthKVPerBatch + actualKVPrefixLen - actualSeqLengthPerBatch;
        if (isKvContinuous == 1) {
            actualSeqLengthPerBatch =
    ...
    this->tailParams->unalignSInner = (actualSeqLengthKVPerBatch % this->tailParams->singleProcessSInnerSize != 0) ?
                                          ...
                                          this->tailParams->singleProcessSInnerSize;
    uint32_t maxInnerLoopPromptTimes = (actualSeqLengthKVPerBatch + this->tailParams->singleProcessSInnerSize - 1) /
                                       this->tailParams->singleProcessSInnerSize;
    ...
    if (pseShiftTypeByteNum != 0) {
        this->tailParams->pseShiftInnerTailAlign =
            (this->tailParams->unalignSInner + pseShiftTypeByteNum - 1) / pseShiftTypeByteNum * pseShiftTypeByteNum;
    ...
    if constexpr (PFAT::enablePrefix) {
        this->tailParams->unalignSInnerPrefix = (actualKVPrefixLen % this->tailParams->singleProcessSInnerSize != 0) ?
                                                    actualKVPrefixLen % this->tailParams->singleProcessSInnerSize :
                                                    this->tailParams->singleProcessSInnerSize;
        maxInnerLoopPrefixTimes = (actualKVPrefixLen + this->tailParams->singleProcessSInnerSize - 1) /
                                  this->tailParams->singleProcessSInnerSize;
        ...
        this->tailParams->padPrefixSize =
            this->tailParams->maskInnerPrefixTailAlign - this->tailParams->unalignSInnerPrefix;
        if (pseShiftTypeByteNum != 0) {
            this->tailParams->pseShiftInnerPrefixTailAlign =
                (this->tailParams->unalignSInnerPrefix + pseShiftTypeByteNum - 1) / pseShiftTypeByteNum *
                pseShiftTypeByteNum;
            this->tailParams->pseShiftPadPrefixSize =
                this->tailParams->pseShiftInnerPrefixTailAlign - this->tailParams->unalignSInnerPrefix;
        }
    }
...
template <typename PFAT>
__aicore__ inline void PromptFlashAttentionS1s2Bns1X910Base<PFAT>::GetSparseParam(int64_t *preTokens,
    ...
    if (attentionMaskType == 4) {
        ...
        if (isActualLenDimsNull) {
            ...
            actualSeqLengthPerBatchUser =
                (tilingData->promptAttentionBaseParams.actualSeqLengthsSize == actualSeqQMin) ?
                    actualSeqLengthsGm.GetValue(0) :
                    actualSeqLengthsGm.GetValue(sIdx);
        }
        if (isActualLenDimsKVNull) {
            ...
            actualSeqLengthKVPerBatchUser =
                (tilingData->promptAttentionBaseParams.actualSeqLengthsKVSize == actualSeqKVmin) ?
                    actualSeqLengthsKVGm.GetValue(0) :
                    actualSeqLengthsKVGm.GetValue(sIdx);
        }
        *preTokens = (int64_t)tilingData->promptAttentionBaseParams.preTokens - actualSeqLengthKVPerBatchUser -
                     actualKVPrefixLen + actualSeqLengthPerBatchUser;
        *nextTokens = (int64_t)tilingData->promptAttentionBaseParams.nextTokens + actualSeqLengthKVPerBatchUser +
                      actualKVPrefixLen - actualSeqLengthPerBatchUser;
    }
