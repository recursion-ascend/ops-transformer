            ...
            if (nIdx < nEnd) {
                for (uint32_t splitIdx = 0; splitIdx < sBlockStack && nIdx + splitIdx < nEnd; splitIdx++) {
                    ...
                    for (uint32_t kIdx = 0; kIdx < loopQK; kIdx++) {
                        ...
                        if (qkM == 1) {
                            AscendC::DataCopy(
                                l1qBufAddrTensor[offset], qGmTensor[qOffsetk],
                            ...
                            if (strideQo < STRIDE_LIMIT) {
                                AscendC::DataCopy(l1qBufAddrTensor[offset], qGmTensor[qOffsetk],
                        ...
                        if (qkM == 1) {
                            AscendC::LoadData(l0aBufTensor[offset], l1qBufAddrTensor[offset],
                        ...
                        if (strideK < STRIDE_LIMIT) {
                            AscendC::DataCopy(l1kBufAddrTensor[offset], kGmTensor[kOffsetk],
                                              AscendC::Nd2NzParams(1,        // ndNum
                                                                   qkN,      // nValue
                                                                   qkK,      // dValue
                                                                   0,        // srcNdMatrixStride, unused
                        ...
                        AscendC::LoadData(
                            ...
                            AscendC::LoadData2dParams(0, qkRoundK * qkRoundN / CUBE_MATRIX_SIZE, 1, 0, 0, false, 0));
                ...
                AscendC::Fixpipe<half, float, AscendC::CFG_ROW_MAJOR>(
                    sGmTensor[(uint64_t)GetBlockIdx() * TMP_SIZE + nIdx % vectMod * TMP_SIZE / vectMod], l0cBufTensor,
                    intriParams);
                AscendC::SetFlag<AscendC::HardEvent::FIX_M>((EVENT_ID0));
                AscendC::SetFlag<AscendC::HardEvent::FIX_M>((EVENT_ID1));
                ...
                for (uint32_t kIdx = 0; kIdx < loopV; kIdx++) {
                    ...
                    if (strideV < STRIDE_LIMIT) {
                        ...
                        for (uint32_t i = 0; i < svN; ++i) {
                            AscendC::DataCopy(l1vBufAddrTensor[i * BLOCK_SIZE_COPY], vGmTensor[vOffsetk][i * strideV],
                                              AscendC::Nd2NzParams(1,        // ndNum
                                                                   1,        // nValue
                                                                   qkK,      // dValue
                                                                   0,        // srcNdMatrixStride, unused
                    ...
                    for (uint32_t l0bLoadIdx = 0; l0bLoadIdx < svRoundN / BLOCK_SIZE; ++l0bLoadIdx) {
                        AscendC::LoadData(
                            l0bBufTensor[l0bLoadIdx * qkRoundK * BLOCK_SIZE],
                            l1vBufAddrTensor[l0bLoadIdx * CUBE_MATRIX_SIZE],
                            AscendC::LoadData2dParams(0, qkRoundK / BLOCK_SIZE, svRoundN / BLOCK_SIZE, 0, 0, true, 0));
                    }
                            ...
                            if (svRoundN < STRIDE_LIMIT) {
                                ...
                                for (uint32_t i = 0; i < qkM; ++i) {
                                    AscendC::DataCopy(
                                        ...
                                        pGmTensor[(uint64_t)GetBlockIdx() * TMP_SIZE +
                                                  (nIdx - launchDelay) % vectMod * TMP_SIZE / vectMod][i * strideQo],
                                        AscendC::Nd2NzParams(1,        // ndNum
                                                             1,        // nValue
                                                             qkM,      // dValue
                                                             0,        // srcNdMatrixStride, unused
    ...
    const typename PFAT::tilingType *__restrict tiling, __gm__ uint8_t *gmTiling, TPipe *tPipe)
{
    ...
    AscendC::LocalTensor<half> maskValueUbufTensor =
        buf.GetBuffer<BufferType::ASCEND_UB, half>(11 * UB_UINT8_BLOCK_SIZE);
    AscendC::LocalTensor<uint8_t> maskU8UbufTensor =
            ...
            if (nIdx < nEnd) {
                ...
                if (subM > 0 && maskType != 0 && longSeq == 0) {
                    if (qkN <= ppNScalar) {
                        ...
                        AscendC::DataCopyPad(maskUbufTensor[offset],
                        ...
                        AscendC::DataCopyPad(maskUbufTensor,
                                             maskGmTensor[maskOffset + (uint64_t)subblockIdx * qkM / 2 * maxSeqlen],
                                             AscendC::DataCopyExtParams(subM, qkN * 2, (maxSeqlen - qkN) * 2, 0, 0),
                                             AscendC::DataCopyPadExtParams<half>(false, 0, 0, 0));
                ...
                if (qkN <= VECTOR_SIZE) {
                    ...
                    if (subM > 0) {
                        ...
                        AscendC::DataCopy(
                            ...
                            AscendC::DataCopyParams(subM, qkRoundN / BLOCK_SIZE, 0, 0));
                        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID0));
                        AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID0));
                        // *** ls = tor * ls
                        AscendC::Muls<half, false>(lsUbufTensor[offset], lsUbufTensor[offset], tor, (uint64_t)0,
                                                   (subM * qkRoundN + VECTOR_SIZE - 1) / VECTOR_SIZE,
                                                   AscendC::UnaryRepeatParams(1, 1, STRIDE_8, STRIDE_8));
                        AscendC::PipeBarrier<PIPE_V>();
                        if (isClamp == 1) {
                            // get min(clampMin，ls_ubuf)
                            AscendC::Maxs<half, false>(lsUbufTensor[offset], lsUbufTensor[offset], clampMin,
                                                       (uint64_t)0, (subM * qkRoundN + VECTOR_SIZE - 1) / VECTOR_SIZE,
                                                       AscendC::UnaryRepeatParams(1, 1, STRIDE_8, STRIDE_8));
                            AscendC::PipeBarrier<PIPE_V>();

                            // get max(clampMin，ls_ubuf)
                            AscendC::Mins<half, false>(lsUbufTensor[offset], lsUbufTensor[offset], clampMax,
                                                       (uint64_t)0, (subM * qkRoundN + VECTOR_SIZE - 1) / VECTOR_SIZE,
                                                       AscendC::UnaryRepeatParams(1, 1, STRIDE_8, STRIDE_8));
                            AscendC::PipeBarrier<PIPE_V>();
                        }
                        ...
                        if (maskType != 0) {
                            if (longSeq == 0) {
                                ...
                                AscendC::Add<half, false>(
                                    lsUbufTensor[offset], lsUbufTensor[offset], maskUbufTensor[offset], (uint64_t)0,
                                    (subM * qkRoundN + VECTOR_SIZE - 1) / VECTOR_SIZE,
                                ...
                                AscendC::Add<half, false>(lsUbufTensor[offset + FLOAT_VECTOR_SIZE],
                                                          lsUbufTensor[offset + FLOAT_VECTOR_SIZE], maskUbufTensor,
                                                          (uint64_t)0, subM,
                                                          AscendC::BinaryRepeatParams(1, 1, 1, qkRoundN / BLOCK_SIZE,
                                                                                      qkRoundN / BLOCK_SIZE, STRIDE_8));
                            } else if (nIdx == nEnd - 1) {
                                __set_mask(qkN);
                                AscendC::Add<half, false>(lsUbufTensor[offset], lsUbufTensor[offset], maskUbufTensor,
                                                          (uint64_t)0, subM,
                                                          AscendC::BinaryRepeatParams(1, 1, 1, qkRoundN / BLOCK_SIZE,
                                                                                      qkRoundN / BLOCK_SIZE, STRIDE_8));
                            }
                        ...
                        if (qkN <= VECTOR_SIZE) {
                            ...
                            AscendC::BlockReduceMax<half, false>(lmUbufTensor, tvUbufTensor.ReinterpretCast<half>(),
                                                                 (subM * BLOCK_SIZE + VECTOR_SIZE - 1) / VECTOR_SIZE, 0,
                                                                 1, 1, STRIDE_8);
                            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
                        } else {
                            AscendC::BlockReduceMax<half, false>(tvUbufTensor.ReinterpretCast<half>(),
                        ...
                        if (nIdx == 0) {
                            ...
                            AscendC::Sub<half, false>(
                                dmUbufTensor[(nIdx / sBlockStack) % 4 * UB_HALF_LINE_SIZE], gmUbufTensor, hmUbufTensor,
                                (uint64_t)0, subMD128,
                                AscendC::BinaryRepeatParams(1, 1, 1, STRIDE_8, STRIDE_8, STRIDE_8));
                            AscendC::PipeBarrier<PIPE_V>();
                        }
                        ...
                        for (uint32_t vsubIdx = 0; vsubIdx < qkN / VECTOR_SIZE; ++vsubIdx) {
                            AscendC::Sub<half, false>(
                                ...
                                lsUbufTensor[offset + vsubIdx * VECTOR_SIZE], tvUbufTensor.ReinterpretCast<half>(),
                                (uint64_t)0, subM,
                        ...
                        AscendC::PipeBarrier<PIPE_V>();
                        // *** ls = castfp16to32(ls)
                        AscendC::Cast<float, half, false>(ls32UbufTensor, lsUbufTensor[offset],
                        ...
                        AscendC::PipeBarrier<PIPE_V>();
                        // *** lp = castfp32to16(ls)
                        AscendC::Cast<half, float, false>(lpUbufTensor[offset], ls32UbufTensor,
                                                          AscendC::RoundMode::CAST_NONE, (uint64_t)0,
                                                          (subM * qkRoundN + FLOAT_VECTOR_SIZE - 1) / FLOAT_VECTOR_SIZE,
                        ...
                        if (qkN <= FLOAT_VECTOR_SIZE) {
                            ...
                            AscendC::RepeatReduceSum<float, false>(
                                llUbufTensor[(nIdx / sBlockStack) % 4 * UB_FLOAT_LINE_SIZE], ls32UbufTensor, subM, 0, 0,
                        ...
                        AscendC::PipeBarrier<PIPE_V>();
                        if (nIdx == 0) {
                            AscendC::DataCopy(glUbufTensor, llUbufTensor[(nIdx / sBlockStack) % 4 * UB_FLOAT_LINE_SIZE],
                                              AscendC::DataCopyParams(1, roundSubM / FLOAT_BLOCK_SIZE, 0, 0));
                        } else {
                            AscendC::Cast<float, half, false>(
                                tvUbufTensor, dmUbufTensor[(nIdx / sBlockStack) % 4 * UB_HALF_LINE_SIZE],
                            ...
                            // *** gl = ll + gl
                            AscendC::Add<float, false>(
                                glUbufTensor, glUbufTensor, llUbufTensor[(nIdx / sBlockStack) % 4 * UB_FLOAT_LINE_SIZE],
                                (uint64_t)0, subMD64,
                                AscendC::BinaryRepeatParams(1, 1, 1, STRIDE_8, STRIDE_8, STRIDE_8));
                            AscendC::PipeBarrier<PIPE_V>();
                        }
                        ...
                        AscendC::DataCopy(
                            pGmTensor[(uint64_t)GetBlockIdx() * TMP_SIZE + nIdx % vectMod * TMP_SIZE / vectMod +
                                      (uint64_t)subblockIdx * qkM / 2 * qkRoundN],
                            lpUbufTensor[offset], AscendC::DataCopyParams(1, subM * qkRoundN / BLOCK_SIZE, 0, 0));
                    ...
                    if (subM > 0) {
                        ...
                        AscendC::DataCopy(
                            ...
                            sGmTensor[(uint64_t)GetBlockIdx() * TMP_SIZE + nIdx % vectMod * TMP_SIZE / vectMod +
                                      (uint64_t)subblockIdx * qkM / 2 * qkRoundN],
                            AscendC::DataCopyParams(mSlice, qkRoundN / BLOCK_SIZE, 0, 0));
                        if (subM > mSlice) {
                            if (mEnd > 1) {
                            ...
                            AscendC::DataCopy(
                                ...
                                AscendC::DataCopyParams(subM - mSlice, qkRoundN / BLOCK_SIZE, 0, 0));
                        }
                        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID0));
                        AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID0));
                        // *** ls = tor * ls
                        ...
                        if (maskType != 0) {
                            if (longSeq == 0) {
                                ...
                                AscendC::Add<half, false>(lsUbufTensor[ppNScalar], lsUbufTensor[ppNScalar],
                        ...
                        if (qkN != SOFTMAX_MAX_LENGTH) {
                            AscendC::DataCopy(ls32UbufTensor.ReinterpretCast<half>(), lsUbufTensor,
                                              AscendC::DataCopyParams(1, VECTOR_SIZE / BLOCK_SIZE,
                                                                      (qkRoundN - VECTOR_SIZE) / BLOCK_SIZE, 0));
                            AscendC::PipeBarrier<PIPE_V>();
                            __set_mask(qkN - VECTOR_SIZE);
                            AscendC::Max<half, false>(
                            ...
                            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
                            AscendC::BlockReduceMax<half, false>(tvUbufTensor.ReinterpretCast<half>(),
                                                                 ls32UbufTensor.ReinterpretCast<half>(), subM, 0, 2, 1,
                                                                 STRIDE_8);
                            AscendC::PipeBarrier<PIPE_V>();
                            __set_vcg_mask_mla(VECTOR_SIZE / BLOCK_SIZE);
                            AscendC::BlockReduceMax<half, false>(lmUbufTensor, tvUbufTensor.ReinterpretCast<half>(),
                            ...
                            AscendC::BlockReduceMax<half, false>(ls32UbufTensor.ReinterpretCast<half>(), lsUbufTensor,
                                                                 subM * qkRoundN / VECTOR_SIZE, 0, 1, 1, STRIDE_8);
                            AscendC::PipeBarrier<PIPE_V>();
                            AscendC::BlockReduceMax<half, false>(lmUbufTensor, ls32UbufTensor.ReinterpretCast<half>(),
                                                                 (subM * BLOCK_SIZE + VECTOR_SIZE - 1) / VECTOR_SIZE, 0,
                                                                 1, 1, STRIDE_8);
                        }
                        AscendC::PipeBarrier<PIPE_V>();
                        if (nIdx == 0) {
                            ...
                            // *** dm = gm - hm
                            AscendC::Sub<half, false>(
                                dmUbufTensor[(nIdx / sBlockStack) % launchDelay * UB_HALF_LINE_SIZE], gmUbufTensor,
                                hmUbufTensor, (uint64_t)0, 1,
                                AscendC::BinaryRepeatParams(1, 1, 1, STRIDE_8, STRIDE_8, STRIDE_8));
                            AscendC::PipeBarrier<PIPE_V>();
                            // *** gm = hm
                            AscendC::DataCopy(gmUbufTensor, hmUbufTensor,
                        ...
                        for (uint32_t vsubIdx = 0; vsubIdx < qkN / VECTOR_SIZE; ++vsubIdx) {
                            AscendC::Sub<half, false>(
                                lsUbufTensor[vsubIdx * VECTOR_SIZE], lsUbufTensor[vsubIdx * VECTOR_SIZE],
                                tvUbufTensor.ReinterpretCast<half>(), (uint64_t)0, subM,
                                AscendC::BinaryRepeatParams(1, 1, 0, qkRoundN / BLOCK_SIZE, qkRoundN / BLOCK_SIZE, 1));
                        }
                        if (qkN % VECTOR_SIZE > 0) {
                            ...
                            AscendC::Sub<half, false>(
                                ...
                                lsUbufTensor[qkN / VECTOR_SIZE * VECTOR_SIZE], tvUbufTensor.ReinterpretCast<half>(),
                                (uint64_t)0, subM,
                                AscendC::BinaryRepeatParams(1, 1, 0, qkRoundN / BLOCK_SIZE, qkRoundN / BLOCK_SIZE, 1));
                            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
                        }
                        ...
                        for (uint32_t splitIdx = 0; splitIdx < mEnd; splitIdx++) {
                            ...
                            if (qkN % FLOAT_VECTOR_SIZE > 0) {
                                __set_mask(qkN % FLOAT_VECTOR_SIZE);
                                AscendC::Add<float, false>(
                                    ls32UbufTensor, ls32UbufTensor, ls32UbufTensor[qkNReduceSum], (uint64_t)0, mSplit,
                                    AscendC::BinaryRepeatParams(1, 1, 1, qkRoundN / FLOAT_BLOCK_SIZE,
                                                                qkRoundN / FLOAT_BLOCK_SIZE,
                                                                qkRoundN / FLOAT_BLOCK_SIZE));
                                AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
                        ...
                        if (nIdx == 0) {
                            ...
                            AscendC::Cast<float, half, false>(
                                tvUbufTensor, dmUbufTensor[(nIdx / sBlockStack) % launchDelay * UB_HALF_LINE_SIZE],
                                AscendC::RoundMode::CAST_NONE, (uint64_t)0, subMD64,
                                AscendC::UnaryRepeatParams(1, 1, STRIDE_8, STRIDE_4));
                            AscendC::PipeBarrier<PIPE_V>();
                            // *** dm = exp(dm)
                            ...
                            AscendC::Add<float, false>(
                                ...
                                llUbufTensor[(nIdx / sBlockStack) % launchDelay * UB_FLOAT_LINE_SIZE], (uint64_t)0,
                                subMD64, AscendC::BinaryRepeatParams(1, 1, 1, STRIDE_8, STRIDE_8, STRIDE_8));
                            AscendC::PipeBarrier<PIPE_V>();
                        }
                    }
                }
                    ...
                    if (subM > 0) {
                        ...
                        if (nIdx + sBlockStack > nEnd + launchDelay - 1) {
                            ...
                            AscendC::Cast<half, float, false>(
                                ...
                                (uint64_t)0, (subM * qkRoundK + FLOAT_VECTOR_SIZE - 1) / FLOAT_VECTOR_SIZE,
                                AscendC::UnaryRepeatParams(1, 1, STRIDE_4, STRIDE_8));
                            AscendC::PipeBarrier<PIPE_V>();
                            ...
                            for (uint32_t vdiv_idx = 0; vdiv_idx < qkK / VECTOR_SIZE; ++vdiv_idx) {
                                AscendC::Div<half, false>(goUbufTensor.ReinterpretCast<half>()[vdiv_idx * VECTOR_SIZE],
                                                          goUbufTensor.ReinterpretCast<half>()[vdiv_idx * VECTOR_SIZE],
                                                          tvUbufTensor.ReinterpretCast<half>(), (uint64_t)0, subM,
                                                          AscendC::BinaryRepeatParams(1, 1, 0, qkRoundK / BLOCK_SIZE,
                                                                                      qkRoundK / BLOCK_SIZE, 1));
                            }
                            if (qkK % VECTOR_SIZE > 0) {
                                ...
                                AscendC::Div<half, false>(
                                    ...
                                    goUbufTensor.ReinterpretCast<half>()[qkK / VECTOR_SIZE * VECTOR_SIZE],
                                    tvUbufTensor.ReinterpretCast<half>(), (uint64_t)0, subM,
                                    AscendC::BinaryRepeatParams(1, 1, 0, qkRoundK / BLOCK_SIZE, qkRoundK / BLOCK_SIZE,
                                                                1));
                                AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
                            }
                            AscendC::PipeBarrier<PIPE_V>();
                            ...
                            AscendC::DataCopyPad(oGmTensor[oOffsetk + (uint64_t)subblockIdx * qkM / 2 * strideQo],
                                                 ...
                                                 AscendC::DataCopyExtParams(subM, qkK * 2, 0, (strideQo - qkK) * 2, 0));
                            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>((EVENT_ID2));
                        } else {
                            AscendC::PipeBarrier<PIPE_V>();
...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                ...
                                TPipe *tPipe, __gm__ uint8_t *alibi_coeff);
};

template <typename PFAT>
__aicore__ inline void PromptFlashAttentionBaseMLA<PFAT>::Init(
    __gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value, __gm__ uint8_t *pseShift,
    __gm__ uint8_t *attenMask, __gm__ uint8_t *actualSeqLengths, __gm__ uint8_t *actualSeqLengthsKV,
    __gm__ uint8_t *blocktable, __gm__ uint8_t *queryPaddingSize, __gm__ uint8_t *kvPaddingSize,
    __gm__ uint8_t *keySharedPrefix, __gm__ uint8_t *valueSharedPrefix, __gm__ uint8_t *actualSharedPrefixLen,
    __gm__ uint8_t *attentionOut, __gm__ uint8_t *softmaxLse, __gm__ uint8_t *workspace,
    const typename PFAT::tilingType *__restrict tiling, __gm__ uint8_t *gmTiling, TPipe *tPipe,
    __gm__ uint8_t *alibi_coeff)
{
#ifdef __DAV_C220_CUBE__
    unpad_flashattention_mla<PFAT>(query, key, value, pseShift, attenMask, actualSeqLengths, actualSeqLengthsKV,
                                   blocktable, queryPaddingSize, kvPaddingSize, keySharedPrefix, valueSharedPrefix,
                                   actualSharedPrefixLen, attentionOut, softmaxLse, workspace, tiling, gmTiling, tPipe);
