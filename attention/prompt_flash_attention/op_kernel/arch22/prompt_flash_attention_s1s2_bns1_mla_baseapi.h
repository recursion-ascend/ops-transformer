...
protected:
    static constexpr bool BMM1_OUT_ISFIXED_ND =
        false; // false: 根据S2是否64字节对齐决定bmm1输出为ND/NZ. 对齐输出ND，否则NZ. true: 强制bmm1输出ND
    ...
    __aicore__ inline void InitInput(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                     __gm__ uint8_t *queryRope, __gm__ uint8_t *keyRope, __gm__ uint8_t *attenMask,
...
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                                __gm__ uint8_t *attenMask, __gm__ uint8_t *blockTable,
                                                __gm__ uint8_t *attentionOut, __gm__ uint8_t *softmaxLse,
                                                __gm__ uint8_t *workspace, const TILING_TYPE *__restrict tiling,
                                                TPipe *tPipe)
{
    this->InitInput(query, key, value, nullptr, nullptr, attenMask, nullptr, nullptr, attentionOut, softmaxLse,
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::InitInput(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::SetTiling(const TILING_TYPE *__restrict tilingData)
{
    // copy base params
...
template <typename TILING_TYPE, ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasAtten, typename INPUT_T,
          typename T, CubeFormat bmm1Format, MmPolicyType mmPolicyType, bool pageAttention>
__aicore__ inline void MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format,
                                                  mmPolicyType, pageAttention>::InitBuffer()
{
    if ASCEND_IS_AIV {
        ...
        // 输入buffer
        this->pipe->InitBuffer(this->pseTBuf,
                               V1_BASE_TILE_SZ * sizeof(uint8_t)); // pse/second to,e atten mask/cast stage1 result, 8k
        ...
        this->pipe->InitBuffer(
            this->softmaxSumBuf[0],
            vecS1BaseSize * sizeof(float) * this->softmaxReduceSize); // 1K if cubeS1BaseSize > 256 else 4K
        this->pipe->InitBuffer(
            this->softmaxSumBuf[1],
            vecS1BaseSize * sizeof(float) * this->softmaxReduceSize); // 1K if cubeS1BaseSize > 256 else 4K
        this->pipe->InitBuffer(
            this->softmaxMaxBuf[0],
            vecS1BaseSize * sizeof(float) * this->softmaxReduceSize); // 1K if cubeS1BaseSize > 256 else 4K
        this->pipe->InitBuffer(
            this->softmaxMaxBuf[1],
            vecS1BaseSize * sizeof(float) * this->softmaxReduceSize); // 1K if cubeS1BaseSize > 256 else 4K
        this->pipe->InitBuffer(
            this->softmaxExpBuf[0],
            vecS1BaseSize * sizeof(float) * this->softmaxReduceSize); // 1K if cubeS1BaseSize > 256 else 4K
        this->pipe->InitBuffer(
            this->softmaxExpBuf[1],
            vecS1BaseSize * sizeof(float) * this->softmaxReduceSize); // 1K if cubeS1BaseSize > 256 else 4K

        if (this->hasSink) {
            this->pipe->InitBuffer(this->sinkBuf, vecS1BaseSize * sizeof(float) *
                                                      this->softmaxReduceSize); // 1K if cubeS1BaseSize > 256 else 4K
        ...
        this->pipe->InitBuffer(
            this->stage1PongBuf,
            (V1_BASE_TILE_SZ + (V1_BASE_TILE_SZ / 16)) *
                sizeof(
                    T)); // load bmm1 result, 32K + 32K/16 = 34K +32k/16 是为了在Nz-Nd间搬运时间隔一个bank，避免bank冲突
        this->pipe->InitBuffer(this->maskTBufPing,
                               (V1_BASE_TILE_SZ + (V1_BASE_TILE_SZ / 8)) * sizeof(uint8_t)); // atten mask 8K +8K/8 = 9K
        ...
        this->softmaxLseTmpUb = this->commonTBuf.template Get<
            T>()[V1_BASE_TILE_SZ - vecS1BaseSize -
                 (vecS1BaseSize * ONE_BLK_SIZE /
                  sizeof(
                      T))]; // 公用commTBuf，位置放在最后，避免和bmm2用到的commTBuf部分冲突。分为两半，分别用于计算log和广播后搬出结果
    } else {
...
template <typename TILING_TYPE, ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasAtten, typename INPUT_T,
          typename T, CubeFormat bmm1Format, MmPolicyType mmPolicyType, bool pageAttention>
__aicore__ inline void MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format,
                                                  mmPolicyType, pageAttention>::AllocEvent()
{
    if ASCEND_IS_AIV {
...
template <typename TILING_TYPE, ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasAtten, typename INPUT_T,
          typename T, CubeFormat bmm1Format, MmPolicyType mmPolicyType, bool pageAttention>
__aicore__ inline void MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format,
                                                  mmPolicyType, pageAttention>::FreeEvent()
{
    if ASCEND_IS_AIV {
                                                  ...
                                                  mmPolicyType, pageAttention>::ComputeConstexpr()
{
    ...
    if ASCEND_IS_AIC {
        matmulService.InitParams(dSize, valueDSize, ropeHeadSize, this->mm1Ka, this->mm1Kb, this->mm1RopeKa,
                                 this->mm1RopeKb, this->mm2Kb);
    }
}
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::GetS1LoopRange(int64_t &multiCoreInnerOffset, int64_t &multiCoreInnerLimit)
{
    // 计算sparse场景下s1的循环范围
...
template <typename TILING_TYPE, ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasAtten, typename INPUT_T,
          typename T, CubeFormat bmm1Format, MmPolicyType mmPolicyType, bool pageAttention>
__aicore__ inline void MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format,
                                                  mmPolicyType, pageAttention>::GetS2LoopRange()
{
...
template <typename TILING_TYPE, ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasAtten, typename INPUT_T,
          typename T, CubeFormat bmm1Format, MmPolicyType mmPolicyType, bool pageAttention>
__aicore__ inline void MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format,
                                                  mmPolicyType, pageAttention>::Process()
{}
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::ComputeAxisIdx(int64_t multiCoreInnerIdx)
{
    // 计算轴的idx
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::SetExtraInfo(SplitSameABExtraInfo &extraInfo, int64_t taskId,
...
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::CalcBmm1TensorAGmOffset(const SplitSameABExtraInfo &extraInfo)
{
    // 计算gm上的offset
    int64_t bOffset = 0;
...
__aicore__ inline uint64_t
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::CalcBmm1QRopeGmOffset(const SplitSameABExtraInfo &extraInfo)
{
...
__aicore__ inline uint64_t
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::CalcBmm1TensorBGmOffset(const SplitSameABExtraInfo &extraInfo)
{
    // 计算gm上的offset
    int64_t bOffset = 0;
    int64_t n2Offset = 0;
...
__aicore__ inline uint64_t
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::CalcBmm1KRopeGmOffset(const SplitSameABExtraInfo &extraInfo)
{
    int64_t ropeBOffset = 0;
...
template <typename TILING_TYPE, ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasAtten, typename INPUT_T,
          typename T, CubeFormat bmm1Format, MmPolicyType mmPolicyType, bool pageAttention>
__aicore__ inline void MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format,
                                                  mmPolicyType, pageAttention>::NdToNz(SplitSameABExtraInfo &extraInfo,
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::ProcessVec1(SplitSameABExtraInfo &extraInfo)
{
    ...
    for (int32_t loopIdx = 0; loopIdx < extraInfo.realSplitN; ++loopIdx) {
        ...
        if constexpr (hasAtten) {
            ...
            if (this->attenMaskComputeMode == AttenMaskComputeMode::PRE_AND_NEXT_MODE ||
                ...
                maskType = 1;
                this->ComputeAttenMask(shapeInfo, this->v1CalcUb, secondTimeMaskUb, maskType,
                                       this->v1PseMaskMte2EndEvent);
            }
            SetFlag<HardEvent::V_MTE2>(this->v1ReadyToInPseMaskEvent);
        }
        ...
        if constexpr (!IsSameType<T, INPUT_T>::value) {
            ...
            if constexpr (!BMM2_A_ISFIXED_ND) {
                ...
                DataCopy(this->stage1Res[extraInfo.taskIdMod2]
                                        [(extraInfo.vecCoreOffset + loopIdx * extraInfo.vec1S1BaseSize) *
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::CopyInAttenMask(SplitSameABExtraInfo &extraInfo, int64_t loopIdx,
    ...
    if constexpr (hasAtten) {
        ...
        if (unlikely(this->notSplitG)) {
            for (int64_t loop = 0; loop < extraInfo.gBaseSize; ++loop) {
                ...
                BoolCopyIn(dst, this->attenMaskGmInt, maskOffset, extraInfo.vec1S1RealSize / extraInfo.gBaseSize,
                           extraInfo.s2RealSize, s2StrideSize, 64);
            }
        } else {
...
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::ComputeAttenMaskOffset(SplitSameABExtraInfo &extraInfo, int64_t loopIdx)
{
    if constexpr (hasAtten) {
        if (this->tilingData->PFAinputParams.attenMaskCompressMode ==
            static_cast<uint8_t>(AttenMaskCompressMode::NO_COMPRESS_MODE)) {
        ...
        if constexpr (InputLayoutIsTNDLike()) {
            ...
            int64_t s1Offset = extraInfo.s1oIdx * this->cubeS1BaseSize +
                               (extraInfo.vecCoreOffset + loopIdx * extraInfo.vec1S1BaseSize) / extraInfo.gBaseSize;
            ...
            if (this->tilingData->PFAinputParams.attenMaskCompressMode ==
                ...
                int64_t maskOffsetPre =
                    ComputeOffsetForCausal(deltaPre, extraInfo.vec1S1BaseSize / extraInfo.gBaseSize,
                                           this->s2BaseNRatioSize, this->tilingData->PFAinputParams.attenMaskS2Size);
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::GetAttenMaskComputeMode(int64_t deltaCausalOrNext, int64_t deltaPre,
...
__aicore__ inline int64_t
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::ComputeOffsetForNoCompress(SplitSameABExtraInfo &extraInfo, int64_t loopIdx)
{
    if constexpr (hasAtten) {
        int64_t bOffset = 0;
        int64_t n2Offset = 0;
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::GetBmm1Result(SplitSameABExtraInfo &extraInfo, LocalTensor<T> &bmm1ResUb,
                                                         int64_t loopIdx)
{
    if constexpr (bmm1Format == CubeFormat::NZ) {
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::ComputeAttenMask(SelectWithBytesMaskShapeInfo &shapeInfo,
                                                            LocalTensor<T> &bmm1ResUb,
                                                            LocalTensor<uint8_t> &attenMaskUb, const uint8_t maskType,
                                                            event_t vWaitMte2)
{
    if constexpr (hasAtten) {
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::SoftMaxCheckResCompress(SplitSameABExtraInfo &extraInfo,
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::InvalidLineSplitS2Process(SplitSameABExtraInfo &extraInfo,
                                                                     LocalTensor<T> &srcTensor, LocalTensor<T> &maxUb,
                                                                     int64_t loopIdx)
{
    if constexpr (implMode == ImplModeEnum::AA_HIGH_PRECISION) {
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::SoftMaxCompute(SplitSameABExtraInfo &extraInfo, LocalTensor<T> &srcTensor,
                                                          int64_t loopIdx)
{
    ...
    if (extraInfo.s2LastLoop) {
        if constexpr (hasAtten == false) {
            ...
            Duplicate<T>(srcTensor[extraInfo.s2RealSizeFloorAlign8], *((float *)&MLA_NEGATIVE_INF_VALUE_FP32), mask,
                         vec1S1RealSize, 1, extraInfo.s2RealSizeAlign64 * sizeof(T) / blockBytes);
        } else if (this->tilingData->PFAinputParams.attenMaskCompressMode !=
                       static_cast<uint8_t>(AttenMaskCompressMode::NO_COMPRESS_MODE) ||
    ...
    if (loopIdx == extraInfo.realSplitN - 1 && extraInfo.s2LoopCount == extraInfo.s2LoopLimit) {
        if (unlikely(hasSink)) {
            ...
            if (likely(!this->notSplitG)) {
                ...
                for (int64_t loop = extraInfo.gBaseSize - 1; loop >= 0; --loop) {
                    ...
                    Duplicate(this->sinkUb, ToFloat(this->sinkGm.GetValue(n1Offset)),
                              extraInfo.s1RealSize * (loop + 1));
                    PipeBarrier<PIPE_V>();
                }
            }
        ...
        if (unlikely(this->hasSparse4InvalidLine)) {
            SoftMaxShapeInfo softmaxFullShapeInfo{
                static_cast<uint32_t>(extraInfo.s1RealSize), static_cast<uint32_t>(fp32BaseSize),
                static_cast<uint32_t>(extraInfo.s1RealSize), static_cast<uint32_t>(fp32BaseSize)};
            AdjustSoftMaxRes<T, T, false, 1>(softmaxTemp, maxTensor, this->negativeIntScalar, 3e+99,
                                             softmaxFullShapeInfo); // 3e+99: 代表NAN
            PipeBarrier<PIPE_V>();
        }
...
__aicore__ inline uint64_t
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::CalcBmm2TensorBGmOffset(const SplitSameABExtraInfo &extraInfo)
{
    int64_t bOffset = 0;
    int64_t n2Offset = 0;
    int64_t s2Offset = 0;
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::ProcessVec2(SplitSameABExtraInfo &extraInfo)
{
    if (extraInfo.s1RealSize == 0) {
        return;
    }
    ...
    for (int64_t s1oIdx = 0; s1oIdx < vec2LoopLimit; ++s1oIdx) {
        ...
        int64_t mm2ResOffset =
            (extraInfo.vecCoreOffset + s1oIdx * extraInfo.vec2S1BaseSize) * this->valueDSizeAlign16 * this->gBaseSize;
        WaitFlag<HardEvent::V_MTE2>(this->v2VecFreeEvent);
        ...
        if constexpr (!BMM2_OUT_ISFIXED_ND) {
            ...
            DataCopy2D(curBmm2ResTensor, this->mm2Res[extraInfo.taskIdMod2][bmmResOffset],
        ...
        if (likely(extraInfo.s2LoopCount < extraInfo.s2LoopLimit)) {
            ...
            if (unlikely(extraInfo.s2LoopCount == 0)) {
                DataCopy(this->vec2Res[extraInfo.multiCoreInnerIdxMod2][mm2ResOffset], curBmm2ResTensor,
                         mm2ResCalcSize);
                SetFlag<HardEvent::V_MTE2>(this->v2VecFreeEvent);
                SetFlag<HardEvent::MTE3_MTE2>(eventIdMte3ToMte2);
                WaitFlag<HardEvent::MTE3_MTE2>(eventIdMte3ToMte2);
            } else {
                DataCopy(this->vec2Res[extraInfo.multiCoreInnerIdxMod2][mm2ResOffset], this->bmm2ResOutUb,
                         mm2ResCalcSize);
            }
        } else {
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::Bmm2ResultMul(SplitSameABExtraInfo &extraInfo, LocalTensor<T> &bmm2ResUb,
                                                         int64_t s1oIdx)
{
    PipeBarrier<PIPE_V>();
    ...
    if (this->softmaxReduceSize == 1) {
        ...
        if (remain) {
            Mul(bmm2ResUb[loop * repeatMaxSize], softmaxTemp[softmaxTempOffset], bmm2ResUb[loop * repeatMaxSize],
                remain, gsAxisSize, repeatParams);
        }
    } else {
        for (int i = 0; i < loop; ++i) {
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::Bmm2ResultDiv(SplitSameABExtraInfo &extraInfo, LocalTensor<T> &srcBmm2ResUb,
                                                         int64_t s1oIdx)
{
    ...
    if (this->softmaxReduceSize == 1) {
        ...
        for (int i = 0; i < loop; ++i) {
            Div(bmm2ResOutUb[i * repeatMaxSize], srcBmm2ResUb[i * repeatMaxSize],
                softmaxTemp[s1oIdx * extraInfo.vec2S1BaseSize * 8], repeatMaxSize,
                extraInfo.vec2S1RealSize * extraInfo.gBaseSize, repeatParams);
        }
        if (likely(remain)) {
            Div(bmm2ResOutUb[loop * repeatMaxSize], srcBmm2ResUb[loop * repeatMaxSize],
                softmaxTemp[s1oIdx * extraInfo.vec2S1BaseSize * 8], remain,
                extraInfo.vec2S1RealSize * extraInfo.gBaseSize, repeatParams);
        }
    } else {
        if constexpr (IsSameType<T, half>::value) {
...
__aicore__ inline void
MlaS1s2Bn2gs1SameABBaseApi<TILING_TYPE, implMode, layOutType, hasAtten, INPUT_T, T, bmm1Format, mmPolicyType,
                           pageAttention>::Bmm2DataCopyOut(SplitSameABExtraInfo &extraInfo, int64_t s1oIdx,
        ...
        if (likely(dstStride <= 65535)) {
            ...
            for (int32_t i = 0; i < extraInfo.vec2S1RealSize; i++) {
                DataCopyPad(this->attentionOutGm[vCoreOffset +
                                                 (s1oIdx * extraInfo.vec2S1BaseSize + extraInfo.vecCoreOffset) *
                                                     attenOutOffset +
                                                 i * datacopyOffset],
            ...
            for (int32_t j = 0; j < extraInfo.vec2S1RealSize; j++) {
                DataCopyPad(
                    this->attentionOutGm[vCoreOffset +
                                         (s1oIdx * extraInfo.vec2S1BaseSize + extraInfo.vecCoreOffset) *
                                             attenOutOffset +
                                         i * this->valueDSize + j * datacopyOffset],
                    attenOut[i * dataCopyParams.blockCount * this->valueDSizeAlign16 + j * this->valueDSizeAlign16],
                    dataCopyParams);
            }
        }
    }
