...
public:
    ...
    __aicore__ __attribute__((always_inline)) inline void LoadDataToCa(
        AscendC::LocalTensor<typename PFAT::inDataType> dstTensor,
        AscendC::LocalTensor<typename PFAT::inDataType> srcTensor, uint32_t roundK, uint32_t qkRoundM, uint32_t qkM)
    {
        if (qkM == 1) {
            AscendC::LoadData(dstTensor, srcTensor,
            ...
            for (uint32_t l0aLoadIdx = 0; l0aLoadIdx < qkRoundM / BLOCK_SIZE; ++l0aLoadIdx) {
                AscendC::LoadData(dstTensor[l0aLoadIdx * roundK * BLOCK_SIZE], srcTensor[l0aLoadIdx * CUBE_MATRIX_SIZE],
                                  AscendC::LoadData2dParams(0,                     // baseIdx
    ...
    __aicore__ __attribute__((always_inline)) inline void Run()
    {
        ...
        for (uint32_t process = GetBlockIdx(); process < processNum; process = nextProcess) {
            while (process >= curTotalQBlkNum * qHeads) {
                ...
                curTotalQBlkNum +=
                    (qSeqlen != ZERO_PRECISION) ? ((qSeqlen + ppMScalar - ONE_PRECISION) / ppMScalar) : ZERO_PRECISION;
            }
            nextProcess = process + GetBlockNum();
            if (isTriuMask) {
                uint32_t currIter = process / GetBlockNum();
                nextProcess = currIter % 2 == 1 ?
                                  (currIter + 1) * GetBlockNum() + GetBlockIdx() :
                                  (currIter + 2) * GetBlockNum() - 1 - GetBlockIdx(); // 2 is the later num
            }
            // get tiling args
                    ...
                    for (uint32_t splitIdx = 0; splitIdx < sBlockStack && nIdx + splitIdx < nEnd; splitIdx++) {
                        ...
                        for (uint32_t kIdx = 0; kIdx < loopQK; kIdx++) {
                            ...
                            AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(
                                (pingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_FIVE_PRECISION));
                            if (qkM == 1) {
                                AscendC::DataCopy(
                                    l1qBufAddrTensor[offset], qGmTensor[qQffsetk],
                                ...
                                if (strideQo < STRIDE_LIMIT) {
                                    AscendC::DataCopy(l1qBufAddrTensor[offset], qGmTensor[qQffsetk],
                            ...
                            if (strideK < STRIDE_LIMIT) {
                                AscendC::DataCopy(l1kBufAddrTensor[offset], kGmTensor[kQffsetk],
                                                  AscendC::Nd2NzParams(1,        // ndNum
                                                                       qkN,      // nValue
                                                                       qkK,      // dValue
                                                                       0,        // srcNdMatrixStride, unused
                            ...
                            AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(
                                (pingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_FOUR_PRECISION));
                            AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE1>(
                                (pingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_FOUR_PRECISION));
                            AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(
                                (pingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_TWO_PRECISION));
                            AscendC::LoadData(l0bBufTensor[offset], l1kBufAddrTensor[offset],
                                              AscendC::LoadData2dParams(0, qkRoundK * qkRoundN / CUBE_MATRIX_SIZE, 1, 0,
                                                                        0, false, 0));
                            ...
                            AscendC::Mmad(l0cBufTensor[splitIdx * qkRoundM * ppNScalar], l0aBufTensor[offset],
                                          l0bBufTensor[offset], AscendC::MmadParams(qkM, qkN, qkK, 0, false, initc));
                ...
                if (nIdx >= launchDelay) {
                    ...
                    for (uint32_t kIdx = 0; kIdx < loopV; kIdx++) {
                        ...
                        if (strideV < STRIDE_LIMIT) {
                            AscendC::DataCopy(l1vBufAddrTensor, vGmTensor[vOffsetk],
                                              AscendC::Nd2NzParams(1,        // ndNum
                                                                   svN,      // nValue
                                                                   qkK,      // dValue
                                                                   0,        // srcNdMatrixStride, unused
                        ...
                        for (uint32_t l0bLoadIdx = 0; l0bLoadIdx < svRoundN / BLOCK_SIZE; ++l0bLoadIdx) {
                            AscendC::LoadData(l0bBufTensor[l0bLoadIdx * qkRoundK * BLOCK_SIZE],
                                              ...
                                              AscendC::LoadData2dParams(0, qkRoundK / BLOCK_SIZE, svRoundN / BLOCK_SIZE,
                                                                        0, 0, true, 0));
                        }
                        ...
                        intriParams.quantPre = QuantMode_t::NoQuant;
                        AscendC::Fixpipe<float, float, AscendC::CFG_ROW_MAJOR>(
                            oTmpGmTensor[(uint64_t)GetBlockIdx() * TMP_SIZE * LOCAL_SIZE + kIdx * TMP_SIZET +
                                         (nIdx - launchDelay) % vectMod * TMP_SIZE / vectMod * loopV],
...
private:
    ...
    AscendC::LocalTensor<IN_DATA_TYPE> l1qBufAddrTensor =
        buf.GetBuffer<BufferType::ASCEND_CB, IN_DATA_TYPE>(l1qBufAddrOffset);
    AscendC::LocalTensor<IN_DATA_TYPE> l1kBufAddrTensor =
        buf.GetBuffer<BufferType::ASCEND_CB, IN_DATA_TYPE>(l1kBufAddrOffset);
    AscendC::LocalTensor<IN_DATA_TYPE> l1pBufAddrTensor =
        buf.GetBuffer<BufferType::ASCEND_CB, IN_DATA_TYPE>(l1pBufAddrOffset);
    AscendC::LocalTensor<IN_DATA_TYPE> l1vBufAddrTensor =
        buf.GetBuffer<BufferType::ASCEND_CB, IN_DATA_TYPE>(l1vBufAddrOffset);
        ...
        for (uint32_t process = GetBlockIdx(); process < processNum; process = nextProcess) {
            ...
            if (longSeq == 0) {
                ...
                AscendC::DataCopy(mask16UbufTensor,
                                  ...
                                  AscendC::DataCopyParams(subM, VECTOR_SIZE / BLOCK_SIZE, 0, 0));
                ...
                AscendC::Cast<float, IN_DATA_TYPE, false>(
                    ...
                    subM * VECTOR_SIZE / FLOAT_VECTOR_SIZE, AscendC::UnaryRepeatParams(1, 1, STRIDE_8, STRIDE_4));
                ...
                if (nIdx < nEnd) {
                    ...
                    if (qkN <= VECTOR_SIZE) {
                        if (subM > ZERO_PRECISION && maskType != MASK_TYPE_NONE_PRECISION) {
                            ...
                            } else if (maskType == MASK_TYPE_ALIBI_COMPRESS_PRECISION &&
                                       alibiCompressOffset > ALIBI_COMPRESS_OFFSET_DISABLE_PRECISION) {
                                if (nIdx == nEnd - ONE_PRECISION) {
                            ...
                            if (longSeq == 0) {
                                ...
                                AscendC::DataCopyPad(
                                    ...
                                    AscendC::DataCopyExtParams(subM, qkN * TWO_PRECISION,
                                                               (maxSeqlen - qkN) * TWO_PRECISION, 0, 0),
                        ...
                        if (subM > 0) {
                            ...
                            AscendC::DataCopy(
                                ...
                                AscendC::DataCopyParams(subM, qkRoundN / FLOAT_BLOCK_SIZE, 0, 0));
                            AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID0));
                            AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID0));
                            // *** ls = tor * ls
                            AscendC::Muls<float, false>(lsUbufTensor, lsUbufTensor, tor, (uint64_t)0,
                                                        (subM * qkRoundN + FLOAT_VECTOR_SIZE - 1) / FLOAT_VECTOR_SIZE,
                                                        AscendC::UnaryRepeatParams(1, 1, STRIDE_8, STRIDE_8));
                            AscendC::PipeBarrier<PIPE_V>();
                            ...
                            if (maskType != MASK_TYPE_NONE_PRECISION) {
                                if (longSeq == ZERO_PRECISION) {
                                    AscendC::Add<float, false>(
                                        ...
                                        AscendC::BinaryRepeatParams(1, 1, 1, STRIDE_8, STRIDE_8, STRIDE_8));
                                    AscendC::SetFlag<AscendC::HardEvent::V_MTE2>((EVENT_ID1));
                                } else if (ppNScalar == FLOAT_VECTOR_SIZE &&
                                           sBlockStack == SBLOCK_STACK_DOUBLE_PRECISION &&
                                           nIdx == nEnd - TWO_PRECISION) {
                            ...
                            if (qkN <= FLOAT_VECTOR_SIZE) {
                                ...
                                AscendC::PipeBarrier<PIPE_V>();
                                AscendC::BlockReduceMax<float, false>(
                                    lmUbufTensor, tvUbufTensor, roundSubM * EIGHT_PRECISION / SIXTY_FOUR_PRECISION, 0,
                                    1, 1, STRIDE_8);
                                AscendC::PipeBarrier<PIPE_V>();
                                for (uint32_t rowMaxIdx = 1; rowMaxIdx < qkN / FLOAT_VECTOR_SIZE; ++rowMaxIdx) {
                                    AscendC::BlockReduceMax<float, false>(tvUbufTensor,
                                                                          lsUbufTensor[rowMaxIdx * FLOAT_VECTOR_SIZE],
                                                                          subM, 0, 1, 1, qkRoundN / EIGHT_PRECISION);
                                    AscendC::PipeBarrier<PIPE_V>();
                                    AscendC::BlockReduceMax<float, false>(
                                ...
                                if (qkN % FLOAT_VECTOR_SIZE > 0) {
                                    ...
                                    AscendC::BlockReduceMax<float, false>(
                                        ...
                                        0, 1, 1, qkRoundN / EIGHT_PRECISION);
                                    AscendC::PipeBarrier<PIPE_V>();
                            ...
                            if (nIdx == 0) {
                                ...
                                AscendC::DataCopy(hmUbufTensor, lmUbufTensor,
                                                  AscendC::DataCopyParams(1, roundSubM / FLOAT_BLOCK_SIZE, 0, 0));
                                AscendC::PipeBarrier<PIPE_V>();
                            } else {
                                // *** hm = MAX(lm, gm)
                                AscendC::Max<float, false>(
                                ...
                                AscendC::Sub<float, false>(
                                    dmUbufTensor[(nIdx / sBlockStack) % 4 *
                                                 UB_FLOAT_LINE_SIZE], // 4 is the BlockStack num
                                    gmUbufTensor, hmUbufTensor, (uint64_t)0, subMD64,
                                    AscendC::BinaryRepeatParams(1, 1, 1, STRIDE_8, STRIDE_8, STRIDE_8));
                                AscendC::PipeBarrier<PIPE_V>();
                                // *** dm = exp(dm)
                                AscendC::Exp<float, false>(dmUbufTensor[(nIdx / sBlockStack) % 4 *
                                                                        UB_FLOAT_LINE_SIZE], // 4 is the BlockStack num
                                                           dmUbufTensor[(nIdx / sBlockStack) % 4 *
                                                                        UB_FLOAT_LINE_SIZE], // 4 is the BlockStack num
                                                           (uint64_t)0, subMD64,
                            ...
                            if (IS_BF16) {
                                AscendC::Cast<IN_DATA_TYPE, float, false>(
                                    lpUbufTensor.ReinterpretCast<IN_DATA_TYPE>(), ls32UbufTensor,
                                ...
                                if constexpr (std::is_same<float, float>::value &&
                                              std::is_same<IN_DATA_TYPE, __bf16>::value) {
                                    AscendC::Cast<IN_DATA_TYPE, float, false>(
                                        lpUbufTensor.ReinterpretCast<IN_DATA_TYPE>(), ls32UbufTensor,
                                        AscendC::RoundMode::CAST_RINT, (uint64_t)0,
                                        (subM * qkRoundN + FLOAT_VECTOR_SIZE - 1) / FLOAT_VECTOR_SIZE,
                            ...
                            AscendC::DataCopy(
                                ...
                                AscendC::DataCopyParams(1, subM * qkRoundN / BLOCK_SIZE, 0, 0));
                            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>((EVENT_ID0));
                            // *** ll = rowsum(ls32)
                            if (qkN <= FLOAT_VECTOR_SIZE) {
                                __set_mask(qkN);
                                AscendC::RepeatReduceSum<float, false>(llUbufTensor, ls32UbufTensor, subM, 0, 0,
                                                                       1,                          // dstRepeatStride
                                                                       1,                          // srcBlockStride
                                ...
                                if (alibiCoeffGm != nullptr) {
                                    ...
                                    AscendC::DataCopy(
                                        ...
                                        maskGmTensor[maskOffset +
                                                     (subBlockIdx * qkM / TWO_PRECISION + splitIdx * mSlice) *
                                                         SOFTMAX_MAX_LENGTH],
                                        AscendC::DataCopyParams(mSplit, qkRoundN / BLOCK_SIZE,
                                                                (SOFTMAX_MAX_LENGTH - qkRoundN) / BLOCK_SIZE, 0));
                                } else if (maskType == MASK_TYPE_ALIBI_COMPRESS_PRECISION &&
                                           alibiCompressOffset > ALIBI_COMPRESS_OFFSET_DISABLE_PRECISION) {
                                    deltaUint = mIdx * ppMScalar - nIdx * ppNScalar;
                                    maskOffset =
                                    ...
                                    AscendC::DataCopy(
                                        ...
                                        AscendC::DataCopyParams(mSplit,
                                                                ...
                                                                0, (qkRoundN - VECTOR_SIZE) / BLOCK_SIZE));
                                    AscendC::DataCopy(
                                        mask16UbufTensor[VECTOR_SIZE],
                                        maskGmTensor[maskOffsetTail + (subBlockIdx * qkM / 2 + splitIdx * mSlice) *
                                                                          VECTOR_SIZE], // 2 is the CV_RATIO
                                        AscendC::DataCopyParams(mSplit, (qkRoundN - VECTOR_SIZE) / BLOCK_SIZE,
                                    ...
                                    AscendC::DataCopyPad(
                                        ...
                                        maskGmTensor[maskOffset +
                                                     (subBlockIdx * qkM / TWO_PRECISION + splitIdx * mSlice) *
                                                         maxSeqlen],
                                        AscendC::DataCopyExtParams(mSplit, qkN * TWO_PRECISION,
                                                                   (maxSeqlen - qkN) * TWO_PRECISION, 0, 0),
                                        AscendC::DataCopyPadExtParams<IN_DATA_TYPE>(false, 0, 0, 0));
                                }
                                AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID1));
                            ...
                            if (subM > 0) {
                                if (mSplit > 0) {
                                            ...
                                            if (nIdx != nEnd - TWO_PRECISION) {
                                                ...
                                                if (alibiLeftAlign == 1) {
                                                    AscendC::Adds<float, false>(
                                                        ...
                                                        maskUbufTensor, (float)-baseY, (uint64_t)0, mSplit,
                                                        AscendC::UnaryRepeatParams(1, 1, qkRoundN / EIGHT_PRECISION,
                                                                                   qkRoundN / EIGHT_PRECISION));
                                                    AscendC::Adds<float, false>(
                                                        maskUbufTensor[128 +
                                                                       FLOAT_VECTOR_SIZE], // 128 is the Tensor length
                                                    ...
                                                    AscendC::Adds<float, false>(
                                                        maskUbufTensor[128], // 128 is the Tensor length
                                                        maskUbufTensor, baseY, (uint64_t)0, mSplit,
                                                        AscendC::UnaryRepeatParams(1, 1, qkRoundN / EIGHT_PRECISION,
                                                                                   qkRoundN / EIGHT_PRECISION));
                                                    AscendC::Adds<float, false>(
                                                        maskUbufTensor[128 +
                                                                       FLOAT_VECTOR_SIZE], // 128 + FLOAT_VECTOR_SIZE is
                                                                                           // the Tensor length
                                                        maskUbufTensor[FLOAT_VECTOR_SIZE], baseY, (uint64_t)0, mSplit,
                                                        AscendC::UnaryRepeatParams(1, 1, qkRoundN / EIGHT_PRECISION,
                                                                                   qkRoundN / EIGHT_PRECISION));
                                                }
                                                AscendC::PipeBarrier<PIPE_V>();
                                    ...
                                    AscendC::DataCopy(
                                        ...
                                        AscendC::DataCopyParams(mSplit, qkRoundN / FLOAT_BLOCK_SIZE, 0, 0));
                                    AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID0));
                                    AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>((EVENT_ID0));
                                    // *** ls = tor * ls
                                    AscendC::Muls<float, false>(
                                    ...
                                    if (nIdx == 0) {
                                        ...
                                        AscendC::DataCopy(
                                            ...
                                            AscendC::DataCopyParams(1, roundMSplit / FLOAT_BLOCK_SIZE, 0, 0));
                                        AscendC::PipeBarrier<PIPE_V>();
                                    } else {
                                        // *** hm = MAX(lm, gm)
                                        AscendC::Max<float, false>(
                                    ...
                                    AscendC::PipeBarrier<PIPE_V>();
                                    // *** gm = hm
                                    AscendC::DataCopy(gmUbufTensor[splitIdx * mSlice], hmUbufTensor[splitIdx * mSlice],
                                                      AscendC::DataCopyParams(1, roundMSplit / FLOAT_BLOCK_SIZE, 0, 0));
                                    AscendC::PipeBarrier<PIPE_V>();
                                    // *** hm_block = expand_to_block(hm), 存放于 tv
                                    AscendC::Brcb(tvUbufTensor.ReinterpretCast<uint32_t>(),
                                    ...
                                    AscendC::DataCopy(pGmTensor[(uint64_t)GetBlockIdx() * TMP_SIZE +
                                                                nIdx % vectMod * TMP_SIZE / vectMod +
                                                                ((uint64_t)subBlockIdx * qkM / 2 + splitIdx * mSlice) *
                                                                    qkRoundN], // 2 is the CV_RAITO
                                                      lpUbufTensor.ReinterpretCast<IN_DATA_TYPE>(),
                                                      AscendC::DataCopyParams(mSplit, qkRoundN / BLOCK_SIZE, 0, 0));
                                    AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>((EVENT_ID0));
                                    // *** ll = rowsum(ls32)
                                    for (uint32_t rowsumIdx = 1; rowsumIdx < qkN / FLOAT_VECTOR_SIZE; ++rowsumIdx) {
                                        AscendC::Add<float, false>(
                                    ...
                                    AscendC::RepeatReduceSum<float, false>(
                                        llUbufTensor, ls32UbufTensor, mSplit, 0, 0,
                                        1,                          // dstRepeatStride
                                        1,                          // srcBlockStride
                                        qkRoundN / FLOAT_BLOCK_SIZE // srcRepeatStride
                                    );
                        ...
                        if (subM > 0) {
                            if (nIdx == launchDelay) {
                                ...
                                AscendC::Brcb(
                                    ...
                                    dmUbufTensor[((nIdx - launchDelay) / sBlockStack % 4) * UB_FLOAT_LINE_SIZE]
                                ...
                                for (uint32_t vmulIdx = 0; vmulIdx < qkK / FLOAT_VECTOR_SIZE; ++vmulIdx) {
                                    AscendC::Mul<float, false>(
                                        ...
                                        AscendC::BinaryRepeatParams(1, 1, 0, qkRoundK / FLOAT_BLOCK_SIZE,
                                                                    qkRoundK / FLOAT_BLOCK_SIZE, 1));
                                }
                                if (qkK % FLOAT_VECTOR_SIZE > 0) {
                                    __set_mask(qkK % FLOAT_VECTOR_SIZE);
                                    AscendC::Mul<float, false>(
                                        goUbufTensor[qkK / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE],
                                        goUbufTensor[qkK / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], tvUbufTensor,
                                        (uint64_t)0, subM,
                                        AscendC::BinaryRepeatParams(1, 1, 0, qkRoundK / FLOAT_BLOCK_SIZE,
                                                                    qkRoundK / FLOAT_BLOCK_SIZE, 1));
                                    AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
                            ...
                            if (nIdx + sBlockStack > nEnd + launchDelay - 1) {
                                ...
                                if (IS_BF16) {
                                    AscendC::Cast<IN_DATA_TYPE, float, false>(
                                        goUbufTensor.ReinterpretCast<IN_DATA_TYPE>(), goUbufTensor,
                                        AscendC::RoundMode::CAST_RINT, (uint64_t)0,
                                        (subM * qkRoundK + FLOAT_VECTOR_SIZE - 1) / FLOAT_VECTOR_SIZE,
                                        AscendC::UnaryRepeatParams(1, 1, STRIDE_4, STRIDE_8));
                                } else {
                                ...
                                AscendC::DataCopyPad(
                                    oGmTensor[oOffsetk + (uint64_t)subBlockIdx * qkM / TWO_PRECISION * strideQo],
                                    goUbufTensor.ReinterpretCast<IN_DATA_TYPE>(),
                                    AscendC::DataCopyExtParams(subM, qkK * TWO_PRECISION, 0,
                                                               (strideQo - qkK) * TWO_PRECISION, 0));
                                AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>((EVENT_ID2));
                            } else {
                                AscendC::PipeBarrier<PIPE_V>();
                                ...
                                AscendC::DataCopy(
                                    upoGmTensor[(uint64_t)GetBlockIdx() * TMP_SIZE + kIdx * TMP_SIZET +
                                                (uint64_t)subBlockIdx * qkM / 2 * qkRoundK], // 2 is the CV_RATIO
                                    goUbufTensor, AscendC::DataCopyParams(1, subM * qkRoundK / FLOAT_BLOCK_SIZE, 0, 0));
                                AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>((EVENT_ID2));
                            }
                        }
                    }
...
private:
    ...
    AscendC::LocalTensor<IN_DATA_TYPE> mask16UbufTensor =
        buf.GetBuffer<BufferType::ASCEND_UB, IN_DATA_TYPE>(mask16UbufOffset);
...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                ...
                                TPipe *tPipe, __gm__ uint8_t *alibiCoeff)
    {
#ifdef __DAV_C220_CUBE__
        FlashAttentionEncoderHighPrecisionMLA<PFAT> fa_cube(
            ...
            workspace, tiling, gmTiling, tPipe);
        fa_cube.Run();
#elif __DAV_C220_VEC__
        FlashAttentionEncoderHighPrecisionMLAVec<PFAT> fa_vec(
            query, key, value, pseShift, attenMask, actualSeqLengths, actualSeqLengthsKV, blocktable, queryPaddingSize,
            kvPaddingSize, keySharedPrefix, valueSharedPrefix, actualSharedPrefixLen, attentionOut, softmaxLse,
            workspace, tiling, gmTiling, tPipe, alibiCoeff);
        fa_vec.Run();
#endif
    }
};
