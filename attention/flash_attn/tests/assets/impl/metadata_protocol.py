...
# -----------------------------------------------------------------------------------------------------------
"""Self-contained TTK manual-data sidecar protocol for the flash_attn assets.

This is a behavior-equivalent copy of the generic sidecar protocol used by the
sparse_flash_mla assets.  It is intentionally environment- and file-based: it
does not import TTK and does not depend on a runner/context object.  TTK
continues to own its case files; the assets only keep a compact sidecar outside
each case directory and replace an existing metadata input atomically after a
