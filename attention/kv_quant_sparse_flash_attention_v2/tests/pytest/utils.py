...
def get_np_dtype(type_str):
    ...
    torch_dtype = torch_dtype_map[type_str]
    if torch_dtype in torch_to_np:
        return torch_to_np[torch_dtype]
    return np.float32
    ...
    if tensors is not None:
        ...
        if torch.is_tensor(tensors):
            if tensor_dtype == "bf16":
                out_tensor = tensors.detach().cpu().to(torch.float32).numpy()
            else:
...
def qsfa_v2_run_npu(test_data, testcase_name=None, device_id=0, result_path=None):
    from batch import kv_quant_sparse_flash_attention_v2_process
    ...
    softmax_sum_percent = None
    try:
        npu_result = kv_quant_sparse_flash_attention_v2_process.call_npu(
        ...
        if params.get("return_softmax_lse", False):
