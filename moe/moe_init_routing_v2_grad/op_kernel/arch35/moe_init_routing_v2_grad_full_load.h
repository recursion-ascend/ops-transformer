...
private:
    ...
    __aicore__ inline void CopyIn(int64_t indexOffset, int64_t inputOffset, int64_t currentN, int64_t currentH,
                                  int64_t currentHAlign)
    {
        ...
        for (int64_t nIdx = 0; nIdx < currentN; nIdx++) {
            ...
            for (int64_t kIdx = 0; kIdx < td_->k; kIdx++) {
                ...
                DataCopyPad(inputUb[nInputOffset + kIdx * currentHAlign], inputGm[curInputOffset], copyInParams,
