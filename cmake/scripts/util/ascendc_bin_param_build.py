...
class BinParamBuilder(opdesc_parser.OpDesc):
    ...
    def gen_input_json(
        self: any, auto_gen_path: str, kernel_template_input: str, bisheng_flags: str
    ):
        key_map = {}
...
"""
    ...
    def _write_build_cmd(
        ...
        build_cmd_var += "start_time=$(date +%s.%N)\n"
        ...
        check_result = self._generate_check_result(
            enable_tiling_keys, bin_file, ci_mode_flag
        )
        build_cmd_var += check_result
        build_cmd_var += "end_time=$(date +%s.%N)\n"
        build_cmd_var += 'duration=$(awk "BEGIN { print $end_time - $start_time }")\n'
        ...
        build_cmd_var += 'kernel_size=$(stat -c%s "$kernel_path")\n'
        build_cmd_var += (
...
def get_bisheng_flags(input_options):
    ...
    for elem in args.argv:
        ...
        # 使用集合操作来找出有效标志
        valid_parts = {
            part.strip() for part in parts if part.strip() in bisheng_flags_option
        }
        if valid_parts:
