...
def process_class_fields(fields_str, class_name):
    ...
    var_pattern = re.compile(
        r"^\s*(?P<type>[\w:<>]+)\s+(?P<name>\w+)\s*(?:=\s*[^;]*)?;", re.MULTILINE
    )
    # 逐行处理，保持顺序
        ...
        for entry in fields:
            if entry[0] == "normal":
                ...
                if field_type in [
                    ...
                    output.append(
                        f"TILING_DATA_FIELD_DEF_STRUCT({field_type}, {field_name});"
                    )
            elif entry[0] == "array":
                _, field_type, field_name, field_len = entry
                output.append(
                    f"TILING_DATA_FIELD_DEF_ARR({field_type}, {field_len}, {field_name});"
                )
...
class Process:
    ...
    @classmethod
    def _get_tiling_source(
        cls, ori_file: Path, isTemplateTilingKey: bool = False
    ) -> str:
        """
        获取 TilingData 定义源码
        ...
        for line in lines:
            ...
            if line.startswith("BEGIN_TILING_DATA_DEF"):
                ...
                struct_src += "{\n"
                struct_src += (
                    "  (void)memcpy_s(const_data, sizeof({struct_name}), "
    ...
    def _gen_tiling_h(cls, ori_file: Path, gen_dir: Path):
        ...
        if not gen_file.exists():
            if not flag:
                ...
                def_src = cls._get_tiling_source(
                    ori_file=ori_file, isTemplateTilingKey=flag
                )
    ...
    def main(cls):
        ...
        parser = argparse.ArgumentParser(
            description="TilingData Generator", epilog="Best Regards!"
        )
        parser.add_argument(
            "-o",
            "--operator",
