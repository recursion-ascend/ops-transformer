...
def generate_symbol(args):
    ...
    if not os.path.exists(library_file):
        raise FileExistsError(
            f"generate_symbol input library error, file <{library_file}> not exists."
        )
...
def parser_generate_symbol(subparsers):
    generate_symbol_parser = subparsers.add_parser(
        name="GenerateSymbol", help="Generate Symbol file of input library"
    )
...
class CompileOpStaticLib:
    def __init__(
        self, ops_compile_files: Dict, out_path: str, dist_index: int, arch: str
    ):
    ...
    def compile_link_o(self, out_path, file_path, is_need_path=True):
        ...
        if is_need_path and file_path.name.endswith(".json"):
            with open(file_path, "r", encoding="UTF-8") as json_fd:
                ...
                json_dict["filePath"] = os.path.join(
                    soc, str(file_path).split("/bin/")[-1].split("/kernel/")[-1]
                )
    ...
    def compile_ops_part_o(self, out_path):
        ...
        if self.cpu_arch == Const.x86 or (
            self.cpu_arch == Const.arm and platform.machine() != Const.x86
        ):
            shell_exec(
                [
                    "bash",
                    "-c",
                    f"cd {out_path} && ld -r {path_data_o} -o {path_part_o}",
                ],
                shell=False,
            )
        if self.cpu_arch == Const.arm and platform.machine() == Const.x86:
            shell_exec(
                [
                    ...
                    "-c",
                    f"cd {out_path} && "
                    f"aarch64-linux-gnu-ld -r {path_data_o} -o {path_part_o}",
                ],
                shell=False,
            )
        return
        ...
        for op in self.ops_compile_files:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=job_num) as executor:
                ...
                for file in json_files:
                    executor.submit(
                        self.compile_link_o, op_out_path, file.resolve(), False
                    )
...
def compile_static_library(args):
    ...
    ops_compile_files = GenOpResourceIni(
        args.soc_version, args.build_dir, args.jit
    ).analyze_ops_files()

    csl = CompileOpStaticLib(
        ops_compile_files,
        os.path.join(args.build_dir, f"bin_tmp/{args.soc_version}"),
        index_num,
        cpu_aarch,
    )
...
class GenOpResourceIni:
    def __init__(self, soc_version: str, build_dir: str, build_with_package: bool):
        ...
        if build_with_package and opp_path:
            ...
            ops_info = (
                opp_path / "built-in/op_impl/ai_core/tbe/config" / self._soc_version
            )
            ops_info = list(
                ops_info.glob(f"aic-{self._soc_version}-ops-info-transformer.json")
            )
            ...
            ops_info = (
                self._build_dir
                / "custom/op_impl/ai_core/tbe/config"
                / self._soc_version
            )
            ops_info = list(ops_info.glob(f"aic-{self._soc_version}-ops-info*.json"))
            self._ops_info = ops_info[0] if len(ops_info) != 0 else None
        self._op_resource_path = (
            self._build_dir / "autogen" / self._soc_version / "aclnnop_resource"
        )
...
"""
    ...
    @staticmethod
    def _extract_op_symbol_pair(
        symbol_file: str, search_key: str, prefix: str, suffix: str
    ):
        ...
        for symbol in symbol_ret.splitlines():
            ...
            if not (symbol_name.startswith(prefix) and symbol_name.endswith(suffix)):
                log.warning(
                    f"symbol not satisfied with the format:{prefix}<op_type>{suffix}, skip"
                )
                continue
            op_type = symbol_name
            if prefix:
...
"""
            reference_code += (
                f"{{_binary_{binary_name}_start, _binary_{binary_name}_end}},\n"
            )
...
"""
    ...
    def analyze_ops_files(self):
        ...
        for ops in ops_info_json:
            if "opFile" in ops_info_json[ops]:
                ...
                o_lists = list(
                    Path(self._binary_path).rglob(f"{self._soc_version}/**/*{ops}*.o")
                )
                if len(o_lists) == 0:
                    continue
                else:
            ...
            bin_json_file = (
                self._binary_path
                / op_json_content["binList"][0]["binInfo"]["jsonFilePath"].split(
                    "/", 1
                )[1]
            )
            if "opp/built-in/" in str(self._binary_path):
        ...
        for kb_json in list(
            Path(self._tuning_basic_path).rglob("*_AiCore_*_runtime_kb.json")
        ):
    ...
    def _analyze_symbols(self):
        ...
        for op_type, symbol in self._extract_op_symbol_pair(
            ophost_symbol, "BankParseInterf", "g_", "BankParseInterf"
        ):
    ...
    def _gen_register_resouce_code(self, op_type: str):
        ...
        tiling_declaration = (
            self.TILING_REG_DECL_FMT.format_map(symbol_map) if func_name else ""
        )
        tiling_reg_func = (
            self.TILING_REG_RES_FUNC_FMT.format_map(symbol_map) if func_name else ""
        )
        ...
        for symbol in self._op_res[op_type].extend_register:
            ...
            if func_name:
                extend_declaration += self.EXTEND_REG_DECL_FMT.format(
                    namespace=namespace, func_name=func_name
                )
                reference_code_list.append(reference_code)
        reference_code = ", ".join(reference_code_list)
        extend_reg_func = self.EXTLEND_REG_RES_FUNC_FMT.format(
            op_type=op_type, reference_code=reference_code
        )

        # InferShape
        namespace, func_name, reference_code = self._extract_register_symbol(
            self._op_res[op_type].infer_shape_register
        )
        symbol_map = {
            "op_type": op_type,
            "namespace": namespace,
        ...
        infer_shape_declaration = (
            self.INFER_SHAPE_REG_DECL_FMT.format_map(symbol_map) if func_name else ""
        )
        infer_shape_reg_func = (
            self.INFER_SHAPE_REG_RES_FUNC_FMT.format_map(symbol_map)
            if func_name
            else ""
        )
    ...
    def _gen_tuning_register_resouce_code(self, op_type: str):
        ...
        namespace, func_name, tuning_bank_parse_ref_code = (
            self._extract_register_symbol(
    ...
    def _gen_binary_resource_code(self, op_type: str) -> str:
        ...
        kernel_files_declaration, kernel_files_ref_code = self._gen_binary_res_code(
            self._op_res[op_type].kernel_files
        )
        kernel_resource = (
        ...
        # 知识库
        tuning_kb_declaration, tuning_kb_ref_code = self._gen_binary_res_code(
            self._op_res[op_type].runtime_kb_files
        )
