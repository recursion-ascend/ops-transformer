...
{
    m.def(
        "MoeDistributeDispatchV2(Tensor x, Tensor expert_ids, Tensor mc2_context, str group_ep, int ep_world_size, "
                                    ...
                                    at::Tensor &expand_scales, MoeDistributeDispatchV2Info tilingData)
{
    ...
    INIT_ACL_PROF_TENSOR_INFO("moe_distribute_dispatch_v2", "MoeDistributeDispatchV2", tilingData.aivNum, MIX_AIV,
                              tensorInfo, stream, INPUT(x), INPUT(expert_ids), OUTPUT(expand_x), OUTPUT(dynamic_scales),
                              OUTPUT(assist_info_forcombine), OUTPUT(expert_token_nums), OUTPUT(ep_recv_counts),
                              OUTPUT(expand_scales));
                          ...
                          const c10::optional<at::Tensor> &performance_info)
{
    ...
    tilingData.isTokenMask =
        (x_active_mask.has_value() && x_active_mask->dim() == DIM_ONE); // input active mask 1dims or not
    tilingData.isExpertMask =
        (x_active_mask.has_value() && x_active_mask->dim() == DIM_TWO); // input active mask 2dims or not
    ...
    tilingData.expertTokenNumsType =
        expert_token_nums_type; // expert token nums type, support 0: cumsum mode, 1: count mode
    tilingData.zeroComputeExpertNum =
        zero_expert_num + copy_expert_num + const_expert_num; // sum of zero, copy and const expert nums
    if (scales.has_value()) {
        TORCH_CHECK(scales.value().dim() == 2, "scales should be 2D, but got dim=", scales.value().dim());
    ...
    int64_t copy_expert_num, int64_t const_expert_num)
{
    ...
    ValidateMoeDistributeDispatchV2Input(x, expert_ids, scales, x_active_mask, expert_scales, performance_info,
                                         validate_params);
    ...
    calculate_tilingdata(tilingData, ep_world_size, ep_rank_id, moe_expert_num, total_winsize_ep, expert_shard_type,
                         shared_expert_num, shared_expert_rank_num, global_bs, bs, h, k, a, quant_mode, zero_expert_num,
                         copy_expert_num, const_expert_num, expert_token_nums_type, scales, x_active_mask,
                         performance_info);
    ...
    auto acl_call = [=]() mutable -> int {
        moe_distribute_dispatch_v2_api(stream, is_fullmesh_v2, x, expert_ids, new_workspace, mc2_context, group_ep,
                                       ep_world_size, ep_rank_id, moe_expert_num, scales, x_active_mask, expert_scales,
                                       performance_info, expand_x, dynamic_scales, assist_info_forcombine,
                                       expert_token_nums, ep_recv_counts, expand_scales, tilingData);
        return 0;
    };
    at_npu::native::OpCommand::RunOpApiV2("moeDistributeDispatchV2", acl_call);

    return std::tie(expand_x, dynamic_scales, assist_info_forcombine, expert_token_nums, ep_recv_counts, expand_scales);
}
    ...
    int64_t copy_expert_num, int64_t const_expert_num)
{
    ...
    bool is_valid_shared = ((shared_expert_num > 0) && ((shared_expert_rank_num / shared_expert_num) > 0) &&
