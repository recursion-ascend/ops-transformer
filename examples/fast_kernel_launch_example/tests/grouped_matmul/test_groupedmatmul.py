...
def test_grouped_matmul_interface_exist():
    ...
    assert hasattr(torch.ops.ascend_ops, "grouped_matmul"), (
        "The 'grouped_matmul' operator is not registered in the 'torch.ops.ascend_ops' namespace."
    )
...
def test_grouped_matmul_operator(config, data_type):
    ...
    try:
        expected_result = calculate_pytorch_grouped_matmul(
            x_list, weight_list, group_list, M, E, data_type
        )
