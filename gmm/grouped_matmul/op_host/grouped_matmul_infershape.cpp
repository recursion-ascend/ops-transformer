...
static ge::graphStatus MultiInMultiOutWithoutGroupList(gert::InferShapeContext *context)
{
    ...
    OP_CHECK_IF(
        !success,
        OP_LOGE(context->GetNodeName(), "x tensorList's length[%zu] != weight tensor's first dim[%ld] and length[%zu]",
...
static ge::graphStatus CheckAttrs(gert::InferShapeContext *context, GMMAttrs &gmmAttrs)
{
    const gert::RuntimeAttrs *attrs = context->GetAttrs();
    OP_CHECK_NULL_WITH_CONTEXT(context, attrs);
    OP_CHECK_IF(CheckSplitItem(gmmAttrs.splitItem) != GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "Invalid splitItem, which can only be one of 0/1/2/3."),
                return GRAPH_FAILED);
...
static ge::graphStatus IsGmmAntiQuantEmpty(gert::InferShapeContext *context)
{
    OP_CHECK_IF(!IsTensorListNullOrEmpty(context, GMM_INDEX_IN_ANTIQUANT_SCALE),
                OP_LOGE(context->GetNodeName(), "antiquantScale is not null or empty!"), return GRAPH_FAILED);
    OP_CHECK_IF(!IsTensorListNullOrEmpty(context, GMM_INDEX_IN_ANTIQUANT_OFFSET),
                OP_LOGE(context->GetNodeName(), "antiquantOffset is not null or empty!"), return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}
...
static ge::graphStatus IsGmmQuantEmpty(gert::InferShapeContext *context)
{
    OP_CHECK_IF(!IsTensorListNullOrEmpty(context, GMM_INDEX_IN_SCALE),
                OP_LOGE(context->GetNodeName(), "scale is not null or empty!"), return GRAPH_FAILED);
    OP_CHECK_IF(!IsTensorListNullOrEmpty(context, GMM_INDEX_IN_OFFSET),
                OP_LOGE(context->GetNodeName(), "offset is not null or empty!"), return GRAPH_FAILED);
    ...
    OP_CHECK_IF(IsNonEmpty(pertokenQuantScale0Shape),
                OP_LOGE(context->GetNodeName(), "pertokenQuant scale is not null or empty!"), return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}
...
static ge::graphStatus GetGroupSize(const gert::InferShapeContext *context, GMMParamsInfo &paramsInfo)
{
    ...
    OP_CHECK_IF(groupNum > maxGroupNum,
                OP_LOGE(context->GetNodeName(), "groupNum[%zu] is larger than %zu.", groupNum, maxGroupNum),
                return GRAPH_FAILED);
                                             ...
                                             const gert::Shape *tensorShape, const std::string &tensorType)
{
    ...
    if (isAntiquantInt4) {
        if (tensorDimNum == expectedDimNum) {
            ...
            OP_CHECK_IF(!(perGroupNum > 0 && weightKDimValue % perGroupNum == 0),
                        OP_LOGE(context->GetNodeName(),
                                "perGroupNum must be larger than 0, and can be evenly divided "
                                "by K[%ld] in A16W4-pergroup case, but now perGroupNum is %ld.",
                                weightKDimValue, perGroupNum),
                        return GRAPH_FAILED);
        } else {
            OP_CHECK_IF(tensorDimNum != expectedDimNum - 1,
                        OP_LOGE(context->GetNodeName(),
                                "%s Dim must be %zu for in perchannel case or "
        ...
        OP_CHECK_IF(tensorDimNum != expectedDimNum - 1,
                    OP_LOGE(context->GetNodeName(), "%s Dim must be %zu, but now is %zu.", tensorType.c_str(),
                                               ...
                                               size_t nodeIdx)
{
    ...
    OP_CHECK_IF(tensorSize != weightGroupedSize,
                OP_LOGE(context->GetNodeName(), "%s size[%lu] must be equal with weight size[%lu].", tensorType.c_str(),
                        tensorSize, weightGroupedSize),
                return GRAPH_FAILED);
    ...
    if (isSingleWeight) { // In this case, nodeIdx should have only single tensor, its dim should be 2.
        OP_CHECK_IF(IsTensorListNullOrEmpty(context, nodeIdx),
                    OP_LOGE(context->GetNodeName(), "%s must not be nullptr or empty, but now is nullptr or empty.",
                            tensorType.c_str()),
                    return GRAPH_FAILED);
        ...
        OP_CHECK_IF(static_cast<size_t>(tensor0Shape->GetDim(0)) != groupNum,
                    OP_LOGE(context->GetNodeName(),
                            "%s batch size[%ld] should be "
                            "equal with groupList length[%lu].",
                            tensorType.c_str(), tensor0Shape->GetDim(0), groupNum),
                    return GRAPH_FAILED);
        ...
        OP_CHECK_IF(tensorNDimValue != weightNDimValue,
                    OP_LOGE(context->GetNodeName(), "NDim[%ld] of %s should be equal with NDim[%ld] of weight.",
        ...
        for (uint64_t i = 0; i < groupNum; i++) {
            ...
            OP_CHECK_IF(tensorShape == nullptr,
                        OP_LOGE(context->GetNodeName(), "%s[%lu] must not be nullptr, but now is nullptr.",
                                tensorType.c_str(), i),
                        return GRAPH_FAILED);
            ...
            OP_CHECK_IF(tensorNDimValue != weightNDimValue,
                        OP_LOGE(context->GetNodeName(),
                                "NDim[%ld] of %s[%lu] should be equal with "
                                "NDim[%ld] of weight[%lu].",
...
static ge::graphStatus CheckPerTokenScale(const gert::InferShapeContext *context, const GMMParamsInfo &paramsInfo)
{
    ...
    if ((xGroupedSize == 1UL) && (yGroupedSize == 1UL)) {
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE(context->GetNodeName(), "MDim[%ld] of perTokenScaleOptional should be equal with MDim[%ld] of x.",
                    tensorMDimValue, xMDimValue),
            return GRAPH_FAILED);
    } else {
        OP_LOGE(context->GetNodeName(),
                "per-token quant case is only supported "
                                               ...
                                               const GMMParamsInfo &paramsInfo)
{
    OP_CHECK_IF(paramsInfo.platform == PlatformID::ASCEND310P,
                OP_LOGE(context->GetNodeName(), "quant cases are not supported on Ascend310P."), return GRAPH_FAILED);
    OP_CHECK_IF(gmmAttrs.groupType == GMM_SPLIT_K,
                OP_LOGE(context->GetNodeName(), "quant cases do not support split along the K axis."),
                return GRAPH_FAILED);
    OP_CHECK_IF(!IsTensorListNullOrEmpty(context, GMM_INDEX_IN_OFFSET),
    ...
    if (gmmAttrs.outputDtype != GMM_OUT_DTYPE_INT32) { // output dtype is int32, this scene does not need scale
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE(context->GetNodeName(), "Invalid scale."), return GRAPH_FAILED);
    }
    ...
    if (isPerTokenQuant) {
        OP_CHECK_IF(CheckPerTokenScale(context, paramsInfo) != GRAPH_SUCCESS,
                    OP_LOGE(context->GetNodeName(), "Check perTokenScale failed!"), return GRAPH_FAILED);
    }
    OP_CHECK_IF(IsGmmAntiQuantEmpty(context) != GRAPH_SUCCESS,
...
static ge::graphStatus CheckA8W4AsymQuantParams(gert::InferShapeContext *context, const GMMParamsInfo &paramsInfo)
{
    ...
    OP_CHECK_IF(IsGmmAntiQuantEmpty(context) != GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "antiquant inputs is not empty!"), return GRAPH_FAILED);
    OP_CHECK_IF(biasDimNum != GMM_A8W4_BIAS_DIM_NUM || biasShape->GetDim(0) != e || biasShape->GetDim(1) != n,
                OP_LOGE(context->GetNodeName(), "bias shape is invalid, must be (e,n)."), return GRAPH_FAILED);
...
{
    OP_CHECK_IF(paramsInfo.platform == PlatformID::ASCEND310P,
                OP_LOGE(context->GetNodeName(), "antiquant cases are not supported on Ascend310P."),
                return GRAPH_FAILED);
    OP_CHECK_IF(gmmAttrs.groupType == GMM_SPLIT_K,
                OP_LOGE(context->GetNodeName(), "antiquant cases do not support split along the K axis."),
                return GRAPH_FAILED);
    OP_CHECK_IF(IsTensorListNullOrEmpty(context, GMM_INDEX_IN_ANTIQUANT_SCALE),
                OP_LOGE(context->GetNodeName(),
                        "antiquantScale must not be nullptr in antiquant, but now is nullptr or empty."),
                return GRAPH_FAILED);
    // check antiquantScale and antiquantOffset's tensor shape
    OP_CHECK_IF(CheckOptionalTensorList(context, "antiquantScale", paramsInfo, gmmAttrs,
                                        GMM_INDEX_IN_ANTIQUANT_SCALE) != GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "Invalid antiquantScale"), return GRAPH_FAILED);
    auto w0Desc = context->GetDynamicInputDesc(GMM_INDEX_IN_WEIGHT, 0);
    ...
    OP_CHECK_IF(w0Desc->GetDataType() != DT_INT4 && !hasAntiquantOffset,
                OP_LOGE(context->GetNodeName(),
                        "antiquantOffset must not be nullptr in antiquant, but now is nullptr or empty."),
                return GRAPH_FAILED);
    if (hasAntiquantOffset) {
        OP_CHECK_IF(CheckOptionalTensorList(context, "antiquantOffset", paramsInfo, gmmAttrs,
                    ...
                    OP_LOGE(context->GetNodeName(), "Invalid antiquantOffset"), return GRAPH_FAILED);
    }
    // check perGroupSize
    ...
    OP_CHECK_IF(IsGmmQuantEmpty(context) != GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "Detected antiquant, but quant inputs is not empty!"),
                return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}
                                                   ...
                                                   GMMParamsInfo &paramsInfo)
{
    ...
    OP_CHECK_IF(CheckQuantParams(context, gmmAttrs, paramsInfo) != GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "CheckQuantParams failed!"), return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}
    ...
    for (size_t i = 0; i < tensorListLength; ++i) {
        ...
        if (numWeight > 1) {
            ...
            OP_CHECK_IF(
                ...
                OP_LOGE(context->GetNodeName(), "weight[%lu] dimNum is %lu , but only support 2 when weight separated.",
                        i, weightDimNum),
                return GRAPH_FAILED);
        }
                                                     ...
                                                     const int64_t innerAxisDimId)
{
    ...
    if (wDtype == DataType::DT_INT4) {
        for (size_t i = 0; i < weightSize; ++i) {
            ...
            OP_CHECK_IF(
                n % 2 != 0,
                OP_LOGE(context->GetNodeName(), "w[%zu] dim %ld value %ld should be even when weight is int4 dtype.", i,
                        innerAxisDimId, n),
                return GRAPH_FAILED);
        }
    }
                                        ...
                                        const GMMParamsInfo &paramsInfo)
{
    ...
    OP_CHECK_IF(xSize != paramsInfo.numY,
                OP_LOGE(context->GetNodeName(), "When y is separated, size of x %lu should equal to size of y %lu.",
                        xSize, paramsInfo.numY),
                return GRAPH_FAILED);
    OP_CHECK_IF(weightSize != 1 && xSize != weightSize,
                OP_LOGE(context->GetNodeName(),
                        "When x and weight are separated, "
                        "size of x %lu should equal to size of weight %lu.",
                        xSize, weightSize),
                return GRAPH_FAILED);
    // check dimension
    ...
    DataType wDtype = w0Desc->GetDataType();
    // 2: an even factor
    OP_CHECK_IF(wDtype == DataType::DT_INT4 && weightNDimValue % 2 != 0,
                OP_LOGE(context->GetNodeName(), "w[0] dim %lu value %ld should be even when weight is int4 dtype.",
                        wNDimIdx, weightNDimValue),
                return GRAPH_FAILED);
    for (size_t i = 0; i < xSize; i++) {
        ...
        if (!(ret == GRAPH_SUCCESS && GmmDavidSupportSoc.count(platformInfo.str_info.short_soc_version) > 0)) {
            OP_CHECK_IF(xKDimValue > GMM_MAX_INNER_AXIS,
                        OP_LOGE(context->GetNodeName(), "x[%lu] dim %lu value %ld should be less than or equal to %ld.",
        ...
        if (weightSize > 1UL) {
            ...
            OP_CHECK_IF(
                ...
                OP_LOGE(context->GetNodeName(), "w[%lu] dim %lu value %ld should be even when weight is int4 dtype.", i,
                        wNDimIdx, weightNDimValue),
                return GRAPH_FAILED);
        }
        OP_CHECK_IF(
            xKDimValue != weightKDimValue,
            OP_LOGE(context->GetNodeName(), "x[%lu] dim %lu value %ld should equal to weight[%lu] dim 0 value %ld.", i,
        ...
        if (!(ret == GRAPH_SUCCESS && GmmDavidSupportSoc.count(platformInfo.str_info.short_soc_version) > 0)) {
            OP_CHECK_IF(!transposeWeight && weightNDimValue > GMM_MAX_INNER_AXIS,
                        OP_LOGE(context->GetNodeName(), "w[%zu] dim %zu value %ld should be less than or equal to %ld.",
    ...
    for (size_t i = 0; i < checkNum; i++) {
        ...
        if (!(ret == GRAPH_SUCCESS && GmmDavidSupportSoc.count(platformInfo.str_info.short_soc_version) > 0)) {
            OP_CHECK_IF(innerAxisValue > GMM_MAX_INNER_AXIS,
                        OP_LOGE(context->GetNodeName(),
                                "Dim %ld value of %zu-th shape should be less than or equal to %ld, "
                                "but now is %ld.",
                                innerAxisDimId, i, GMM_MAX_INNER_AXIS, innerAxisValue),
                        return GRAPH_FAILED);
        }
    }
    return GRAPH_SUCCESS;
                                                      ...
                                                      const std::vector<std::string> tensorType, uint64_t groupNum)
{
    ...
    OP_CHECK_IF(TensorType2NodeId(tensorType, nodeIdx) != GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "TensorType2NodeId failed."), return GRAPH_FAILED);
                                                      ...
                                                      const std::vector<std::string> tensorType, uint64_t groupNum)
{
    ...
    if (tensorType[2] == "true" && innerAxisdimId > -1) {
        ...
        if (!(ret == GRAPH_SUCCESS && GmmDavidSupportSoc.count(platformInfo.str_info.short_soc_version) > 0)) {
            OP_CHECK_IF(dimValue > GMM_MAX_INNER_AXIS,
                        OP_LOGE(context->GetNodeName(),
                                "Dim %ld value of %s[0] should be less than or equal to %ld, but now is %ld.",
    ...
    for (uint64_t i = 0; i < groupNum; i++) {
        ...
        OP_CHECK_IF(dimValueLong != dimValueSingle,
                    OP_LOGE(context->GetNodeName(),
                            "Dim %lu value of %s[%lu] %ld should be equal to dim %lu value of %s[0] %ld.", dimIds[0],
                                                        ...
                                                        const GMMParamsInfo &paramsInfo)
{
    ...
    OP_CHECK_IF(CheckShapeSameLengthTensorList(context, {1, kAxisOfWeight}, innerAxisDimId, tenorXAndWeight,
                ...
                OP_LOGE(context->GetNodeName(), "k dim value of x and weight is not matched."), return GRAPH_FAILED);
    innerAxisDimId = specialWeightFormat ? static_cast<int64_t>(weightAxis.n) : (!transposeWeight ? 2 : -1);
    OP_CHECK_IF(
        CheckInnerAxisOfTensorList(context, GMM_INDEX_IN_WEIGHT, innerAxisDimId, paramsInfo.numWeight) != GRAPH_SUCCESS,
    ...
    OP_CHECK_IF(CheckGroupListCommonTensor(
                ...
                OP_LOGE(context->GetNodeName(), "Invalid groupList."), return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}
                                                              ...
                                                              const GMMParamsInfo &paramsInfo)
{
    ...
    OP_CHECK_IF(xSize != weightSize,
                OP_LOGE(context->GetNodeName(),
                        "When x and weight are separated, size of x %lu should equal to size of weight %lu.", xSize,
                        weightSize),
                return GRAPH_FAILED);
    // check dimension
                                       ...
                                       const GMMParamsInfo &paramsInfo)
{
    ...
    if ((xSize == 1UL) && (weightSize > 1UL) && (ySize == 1UL)) {
        OP_CHECK_IF(weightSize != paramsInfo.groupNum,
                    OP_LOGE(context->GetNodeName(), "weight Size [%zu] does not equal with groupNum %zu", weightSize,
                            paramsInfo.groupNum),
                    return GRAPH_FAILED);
    ...
    if ((xSize == 1UL) && (weightSize > 1UL) && (ySize > 1UL)) {
        ...
        OP_CHECK_IF(groupListTensor == nullptr || groupListTensor->GetData<int64_t>() == nullptr,
                    OP_LOGE(context->GetNodeName(), "Failed to obtain necessary data from groupListTensor. "
                                                    "When grouplist is an invalid tensor, split m, single x, separated "
                                                    "weight, separated y cases do not support."),
                    return GRAPH_FAILED);
                                       ...
                                       const GMMParamsInfo &paramsInfo)
{
    ...
    if (xSize == 1UL) {
        ...
        OP_CHECK_IF(
            CheckInnerAxisOfTensorList(context, GMM_INDEX_IN_WEIGHT, innerAxisDimId, weightSize) != GRAPH_SUCCESS,
            OP_LOGE(context->GetNodeName(), "inner axis size of weight is larger than %ld!", GMM_MAX_INNER_AXIS),
            return GRAPH_FAILED);
        return GRAPH_SUCCESS;
    }
                                                    ...
                                                    const GMMParamsInfo &paramsInfo)
{
    ...
    OP_CHECK_IF(transposeX && transposeWeight,
                OP_LOGE(context->GetNodeName(), "x and weight can not be transposed at the same time."),
                return GRAPH_FAILED);
    auto groupTensorOptionalShape = context->GetOptionalInputShape(GMM_INDEX_IN_GROUP_LIST);
    const gert::RuntimeAttrs *attrs = context->GetAttrs();
    OP_CHECK_NULL_WITH_CONTEXT(context, attrs);
    ...
    if (groupType == GMM_NO_SPLIT) {
        OP_CHECK_IF(transposeX,
                    OP_LOGE(context->GetNodeName(), "When x, weight and y are all separated, x can not be transposed."),
                    return GRAPH_FAILED);
        ...
        OP_CHECK_IF(transposeX, OP_LOGE(context->GetNodeName(), "When groupType is 0, x can not be transposed."),
                    return GRAPH_FAILED);
                                            ...
                                            size_t xDimM)
{
    ...
    if (groupListTensor != nullptr) {
        ...
        OP_CHECK_IF(MultiInMultiOutWithoutGroupList(context) != GRAPH_SUCCESS,
                    OP_LOGE(context->GetNodeName(), "Failed to process multi-in-multi-out case without GroupList."),
                    return GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}
                                         ...
                                         const gert::Shape *w0Shape)
{
    ...
    if (isSingleX && !isSingleY) {
        if (gmmAttrs.groupType != GMM_SPLIT_K) {
            ...
            OP_CHECK_IF(MultiWeightMultiOutWithoutGroupList(context) != GRAPH_SUCCESS,
                        OP_LOGE(context->GetNodeName(), "Failed to process multi-in-multi-out case without GroupList."),
                        return GRAPH_FAILED);
        }
        // X单 Y单
                                             ...
                                             const DataType xDtype, const DataType weightDtype)
{
    ...
    if (GmmDavidSupportSoc.count(platformInfo.str_info.short_soc_version) > 0) {
        ...
        if (biasDtype != DT_UNDEFINED) {
            OP_CHECK_IF(
                std::find(BIAS_DTYPE_SUPPORT_LIST.begin(), BIAS_DTYPE_SUPPORT_LIST.end(), biasDtype) ==
...
static graphStatus CheckGroupedMatmulAntiQuantForDtype(gert::InferDataTypeContext *context)
{
    ...
    OP_CHECK_IF(CheckTensorListDataType(context, GMM_INDEX_IN_ANTIQUANT_SCALE, xDtype) != GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "antiquantScale dtype does not match with x dtype[%s].",
                        TypeUtils::DataTypeToAscendString(xDtype).GetString()),
                return GRAPH_FAILED);
    OP_CHECK_IF(CheckTensorListDataType(context, GMM_INDEX_IN_ANTIQUANT_OFFSET, xDtype) != GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "antiquantOffset dtype does not match with x dtype[%s].",
                        TypeUtils::DataTypeToAscendString(xDtype).GetString()),
                return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}
...
static graphStatus CheckFunctionParamsForDtype(gert::InferDataTypeContext *context)
{
    ...
    if (platform == PlatformID::ASCEND310P) {
        ...
        OP_CHECK_IF(!isAllInputFP16,
                    OP_LOGE(context->GetNodeName(), "Only float16 is supported on Ascend310P platforms."),
                    return GRAPH_FAILED);
        ...
        OP_CHECK_IF(biasDtype != ge::DT_UNDEFINED && biasDtype != DataType::DT_FLOAT16,
                    OP_LOGE(context->GetNodeName(), "only bias float16 is supported on Ascend310P platforms."),
                    return GRAPH_FAILED);
    }
    if (xDtype == DataType::DT_INT8 && weightDtype == DataType::DT_INT4) {
        return GRAPH_SUCCESS;
    ...
    if (xDtype == DataType::DT_INT8 && weightDtype == DataType::DT_INT8) {
        ...
        OP_CHECK_IF(CheckFunctionQuantParams(context) != GRAPH_SUCCESS,
                    OP_LOGE(context->GetNodeName(), "CheckFunctionQuantParams failed."), return GRAPH_FAILED);
        return GRAPH_SUCCESS;
    }
    if ((xDtype == DataType::DT_BF16 || xDtype == DataType::DT_FLOAT16) &&
        (weightDtype == DataType::DT_INT8 || weightDtype == DataType::DT_INT4)) {
                                         ...
                                         const DataType yDtype)
{
    ...
    if (isPerTokenQuant) {
        ...
        OP_CHECK_IF(!isOutputBF16 && !isOutputFloat16,
                    OP_LOGE(context->GetNodeName(),
                            "per-token quant case only supports scale data type bfloat16 with "
                            ...
                            i, TypeUtils::DataTypeToAscendString(scale0Dtype).GetString(),
        ...
        OP_CHECK_IF(!isOutputInt8 && !isOutputBF16 && !isOutputFP16,
                    OP_LOGE(context->GetNodeName(),
                            "per-channel quant case only supports scale with data type uint64 "
    ...
    if (isPerTokenQuant) {
        OP_CHECK_IF(perTokenScale0Dtype != DataType::DT_FLOAT,
                    OP_LOGE(context->GetNodeName(),
                            "pertoken quant case only support perTokenScale with dtype float32,"
                            "but perTokenScale[%zu] has data type %s!",
                            i, TypeUtils::DataTypeToAscendString(perTokenScale0Dtype).GetString()),
                    return GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}
...
static graphStatus InferDataType4GroupedMatmul(gert::InferDataTypeContext *context)
{
    ...
    if (isQuantCase && outputDtype != nullptr) {
        ...
        OP_CHECK_IF(it == GMM_OUTPUT_DTYPE_MAP.end(),
                    OP_LOGE(context->GetNodeName(), "value of attr dtype only supports -1/0/1/2, but now is %ld.",
                            *outputDtype),
                    return GRAPH_FAILED);
        yDtype = it->second;
        if (*outputDtype != GMM_OUT_DTYPE_INT32) { // output dtype is int32, this scene does not need scale
