...
/*!
 * \file test_aclnn_grouped_matmul_v5_quant_gb.cpp
 * \brief G-B（pergroup-perblock）量化示例（Ascend 950PR/950DT / A5 设备）
 *        x=FLOAT8_E5M2, weight=FLOAT8_E5M2, scale=FLOAT32, perTokenScale=FLOAT32, out=BF16
 *        groupType=0, splitItem=3, groupListType=0
 *
 *        G-B 量化：gsN=gsK=128 固定。
 *        scale shape = (E, ceil(K/128), ceil(N/128))，3 维。
 *        perTokenScale shape = (M, ceil(K/128))，2 维。
 *        bias 必须为空。
 */
...
int aclnnGroupedMatmulTest(int32_t deviceId, aclrtStream &stream)
{
    ...
    // 参数维度
    int64_t m = 512L;
    int64_t k = 256L;   // K 须为 128 的倍数（gsK=128）
    int64_t n = 256L;   // N 须为 128 的倍数（gsN=128）
    int64_t groupnum = 2L;
    int64_t gs = 128L;  // G-B block size

    std::vector<std::vector<int64_t>> xShape = {{m, k}};
    std::vector<std::vector<int64_t>> weightShape = {{groupnum, k, n}};
    std::vector<std::vector<int64_t>> scaleShape = {{groupnum, k / gs, n / gs}};   // 3 维 pergroup-perblock
    std::vector<std::vector<int64_t>> pertokenShape = {{m, k / gs}};                // 2 维
    ...
    // Host 数据构造
    // FLOAT8_E5M2 以 int8_t 存储（每个字节为一个 FP8 值）
    std::vector<int8_t> xHostData(m * k, 10);
    std::vector<int8_t> weightHostData(groupnum * k * n, 10);
    std::vector<float> scaleHostData(groupnum * (k / gs) * (n / gs), 1.0f);
    std::vector<float> pertokenHostData(m * (k / gs), 1.0f);
    std::vector<uint16_t> yHostData(m * n, 0);
    std::vector<int64_t> groupListData = {256L, 512L};

    // 创建 x aclTensorList (FLOAT8_E5M2)
    ...
    // 创建 weight aclTensorList (FLOAT8_E5M2)
    ...
    // 创建 scale aclTensorList (FLOAT32, 3 维 pergroup-perblock)
    ...
    // 创建 perTokenScale aclTensorList (FLOAT32, 2 维)
    ...
    // bias 为 nullptr（G-B 量化要求 bias 为空）
