...
/*!
 * \file test_aclnn_grouped_matmul_v5_quant_dynamic.cpp
 * \brief 全量化-动态量化 K-C（pertoken-perchannel）示例（Ascend 950PR/950DT / A5 设备）
 *        x=INT8, weight=INT8, scale=FLOAT32, perTokenScale=FLOAT, out=BF16
 *        groupType=0, splitItem=3, groupListType=0
 *
 *        动态量化：perTokenScale 非空（与静态量化的关键区别）。
 *        K-C 模式：scale shape=(E, N) 为 perchannel，perTokenScale shape=(M,) 为 pertoken。
 */
...
int aclnnGroupedMatmulTest(int32_t deviceId, aclrtStream &stream)
{
    ...
    std::vector<std::vector<int64_t>> scaleShape = {{groupnum, n}};          // T-C perchannel
    std::vector<std::vector<int64_t>> pertokenShape = {{m}};                 // K-C pertoken
    ...
    std::vector<float> scaleHostData(groupnum * n, 1.0f);                   // perchannel scale
    std::vector<float> pertokenHostData(m, 1.0f);                           // pertoken scale
    ...
    // 创建 perTokenScale aclTensorList (FLOAT, pertoken)
    // 动态量化的关键：perTokenScale 非空
