...
/*!
 * \file test_aclnn_grouped_matmul_v5_a8w8.cpp
 * \brief 全量化 A8W8 示例（Atlas A2/A3 设备）
 *        x=INT8, weight=INT8, scale=UINT64, out=BF16
...
int aclnnGroupedMatmulTest(int32_t deviceId, aclrtStream &stream)
{
    ...
    std::vector<uint64_t> scaleHostData(groupnum * n, 1);
    std::vector<int32_t> biasHostData(groupnum * n, 1);
    ...
    // 创建 bias aclTensorList
    ret = CreateAclTensorList(biasHostData, biasShape, &biasDeviceAddr, aclDataType::ACL_INT32, &bias);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
