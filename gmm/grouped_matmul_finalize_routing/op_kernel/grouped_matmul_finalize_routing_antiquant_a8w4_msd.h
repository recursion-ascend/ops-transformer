...
public:
    __aicore__ inline GMMA8W4MSDCompute(typename mmType::MT &matmul)
        : mm(matmul)
    {}
    __aicore__ inline void Init(const MMInitParams &initParams, const GroupMatmulFRTilingData *tilingData,
                                TPipe *tPipeIn);
                                                       ...
                                                       const GroupMatmulFRTilingData *tilingData, TPipe *tPipeIn)
{
    ...
    if (withOffset == uint32_t(1)) {
        ...
        mmOutGm.SetGlobalBuffer(
            reinterpret_cast<__gm__ cT::T *>(initParams.workspace + xRowSumCount * sizeof(float))); // add xRowSumGm
    ...
    if (tiling->deterministicFlag == 1) {
        mmQuantOutGm.SetGlobalBuffer(
            reinterpret_cast<__gm__ float *>(initParams.workspace + xRowSumCount * sizeof(float) +
                                             tiling->parallNum * tiling->coreNum * MM_BASE_BLOCK_OFFSET *
                                                 sizeof(float) * EIGHT)); // add userWorkspaceSize
    }
...
__aicore__ inline void GMMA8W4MSDCompute<mmType>::InitUbBuffer()
{
    ...
    pipe->InitBuffer(perTokenScaleInQueue, BUFFER_NUM_BSD,
                     Ceil(tiling->vBaseM * sizeof(float) * 2, UB_ALIGN_LEN) * UB_ALIGN_LEN);
...
__aicore__ inline void GMMA8W4MSDCompute<mmType>::Process()
{
    ...
    for (uint32_t groupIdx = 0, preCount = 0; groupIdx < tiling->groupNum; ++groupIdx) {
        int32_t groupRemainingLogical = static_cast<int32_t>(groupTokensGm.GetValue(groupIdx));
        if (groupRemainingLogical <= 0) {
            continue;
        }
        if constexpr (mmType::groupListType) {
            groupRemainingLogical -= mnConfig.offsetM / 2;
        }
        while (groupRemainingLogical > 0) {
            uint32_t chunkLogical = static_cast<uint32_t>(groupRemainingLogical);
            if (tiling->deterministicFlag == 1 && chunkLogical > syncConfig.windowSize) {
                chunkLogical = static_cast<uint32_t>(syncConfig.windowSize);
            }
            mnConfig.m = chunkLogical * 2; // 2: int8 has been split in 2 int4
            ...
            groupRemainingLogical -= chunkLogical;
        }
    }
    if ASCEND_IS_AIV {
        if (tiling->deterministicFlag == 1) {
...
template <typename mmType>
__aicore__ inline void GMMA8W4MSDCompute<mmType>::CopyMMOutLocal(LocalTensor<cT::T> &mmOutTensor, uint32_t dstOffset,
                                                                 uint64_t srcAddr, uint32_t curVecBaseM,
                                                                 uint32_t curVecBaseN)
{
    DataCopy2DDimParams copyDimParams{static_cast<uint32_t>(curVecBaseM), static_cast<uint32_t>(curVecBaseN),
                                      static_cast<uint32_t>(curVecBaseN * static_cast<uint32_t>(2))};
    if constexpr (mmType::BT::format == CubeFormat::ND) {
        DataCopyPad2DA8W4ND(mmOutTensor[dstOffset], mmOutGm[srcAddr], copyDimParams); // 2: 2 lines int4 to 1 line int8
    } else {
        DataCopyPad2DA8W4(mmOutTensor[dstOffset], mmOutGm[srcAddr], copyDimParams); // 2: 2 lines int4 to 1 line int8
    }
}

template <typename mmType>
__aicore__ inline void GMMA8W4MSDCompute<mmType>::VectorAtomicProcess(uint32_t curVecBaseM, uint32_t curVecBaseN,
                                                                      uint32_t alignBaseN, uint32_t rowIndexOffset,
                                                                      uint64_t yGmOffset, const SyncConfig &syncConfig)
{
    LocalTensor<DTYPE_OUT> yLocal = vecOutQueue.DeQue<DTYPE_OUT>();
    if (tiling->deterministicFlag == 1) {
        DataCopy2DDimParams copyDimParams{static_cast<uint32_t>(curVecBaseM), static_cast<uint32_t>(curVecBaseN),
                                          static_cast<uint32_t>(alignBaseN)};
        DataCopyPad2DA8W4(mmQuantOutGm[rowIndexOffset * tiling->n + yGmOffset -
                                                                ...
                                                                const SyncConfig &syncConfig)
{
    ...
    ComputeDequantAndActivate(mnConfig, vecAParams.curVecBaseM, vecAParams.alignBaseN, vecAParams.curVecBaseN,
                              vecAParams.offsetM);
    VectorAtomicProcess(vecAParams.curVecBaseM, vecAParams.curVecBaseN, vecAParams.alignBaseN, vecAParams.yGmOffset0,
                        vecAParams.yGmOffset1, syncConfig);
}
                                                                ...
                                                                SyncConfig &syncConfig)
{
    ...
    uint32_t vecBaseM =
        tiling->ubCalSize / (Ceil(mnConfig.baseN, uint32_t(8)) * 8); //  8: num int32_t in 32B ub block  32*256/256
    ...
    for (uint32_t offsetN = 0; offsetN < curCubeSingleN; offsetN += mnConfig.baseN) {
        ...
        for (uint32_t offsetM = 0; offsetM < curCubeSingleM; offsetM += vecBaseM, vecCount++) {
            ...
            VectorAtomicParams vecAParams{curVecBaseM,
                                          curVecBaseN,
                                          alignBaseN,
                                          offsetM,
                                          mGlobalOffset,
                                          mGlobalOffset + offsetM,
...
template <typename mmType>
__aicore__ inline void GMMA8W4MSDCompute<mmType>::ComputeDequantAndActivate(MNConfig &mnConfig, uint32_t curVecBaseM,
...
template <typename mmType>
__aicore__ inline void GMMA8W4MSDCompute<mmType>::DataCopyScale(uint32_t curBaseN, uint32_t alignBaseN,
...
template <typename mmType>
__aicore__ inline void GMMA8W4MSDCompute<mmType>::DataCopyOffset(uint32_t curBaseN, uint32_t alignBaseN,
...
template <typename mmType>
__aicore__ inline void GMMA8W4MSDCompute<mmType>::DataCopyPerTokenScaleAndBrcb(MNConfig &mnConfig, uint32_t curBaseM,
                                                                               uint32_t alignBaseN, uint32_t offsetM)
{
    uint64_t perTokenScaleOffset =
        mnConfig.offsetM / 2 + mnConfig.mIdx * mnConfig.singleM / 2 + offsetM; // 2: m方向两行合并为1行
...
template <typename mmType>
__aicore__ inline void GMMA8W4MSDCompute<mmType>::DataCopyAndBrcbOfRowSum(MNConfig &mnConfig, uint32_t curBaseM,
                                                                          uint32_t alignBaseN, uint32_t offsetM)
{
    const uint64_t xRowSumOffset =
        mnConfig.offsetM / 2 + mnConfig.mIdx * mnConfig.singleM / 2 + offsetM; // 2: m方向两行合并为1行
    ...
    BroadCast<float, 2, 1>(afterProcessOutBuffer, xRowSumLocal, xRowSumBroadCastDst, xRowSumBroadCastSrc,
                           tempRowSumBuffer);
...
__aicore__ inline void GMMA8W4MSDCompute<mmType>::VectorDeterSync(MNConfig &mnConfig, SyncConfig &syncConfig)
{
    ...
    uint64_t nextGroupM = (static_cast<uint64_t>(mnConfig.offsetM) + mnConfig.m) / 2;
    if (nextGroupM <= syncConfig.lowBoundM) {
        return;
    }
    syncConfig.curM = mnConfig.offsetM / 2;
    FinalizeRoutingDeterministic(syncConfig);
...
__aicore__ inline void GMMA8W4MSDCompute<mmType>::FinalizeRoutingDeterministic(SyncConfig &syncConfig)
{
    ...
    for (uint64_t mOffset = 0; mOffset < totalM; mOffset++) {
        auto outRow =
            static_cast<uint64_t>(tokenRanksGm.GetValue((syncConfig.lowBoundM - syncConfig.windowSize) + mOffset));
        ...
        for (uint64_t nOffset = 0; nOffset < n; nOffset += syncConfig.baseN) {
            ...
            DataCopy2DDimParams copyDimParams{static_cast<uint32_t>(1), static_cast<uint32_t>(curVecBaseN),
